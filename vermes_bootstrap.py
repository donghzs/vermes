"""Windows UTF-8 bootstrap for Vermes entry points.
...
should be imported at the very top of every Vermes entry point
(``Vermes``, ``Vermes-agent``, ``Vermes-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import vermes_bootstrap``
# (or ``from vermes_bootstrap import apply_windows_utf8_bootstrap``) at
