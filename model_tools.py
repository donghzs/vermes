...
#   - cli.py, vermes_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("VERMES_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("VERMES_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by VERMES_KANBAN_TASK and
        ...
        for toolset_name in effective_enabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                logger.info(f"⚠️  Unknown toolset: {toolset_name}")
    else:
    ...
    # This ensures that even if a composite toolset (like Vermes-cli)
    ...
    if not quiet_mode:
        if filtered_tools:
            ...
            logger.info(f"🛠️  Final tool selection ({len(filtered_tools)} tools): {', '.join(tool_names)}")
        else:
            logger.info("🛠️  No tools selected (all filtered out or unavailable)")
    ...
    # window). Core Vermes tools (toolsets._vermes_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from vermes_cli.config import load_config as _load
        cfg = _load() or {}
