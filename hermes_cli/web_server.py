...
# P1-10: Active stream tracking — allows frontend to notify backend on stop
_active_streams: Dict[str, asyncio.Event] = {}  # stream_id → cancel event
...
async def get_onboarding():
    ...
    Returns ``{configured: true}`` when a model provider and API key are
    present, or ``{configured: false, missing: [...]}`` otherwise.
    """
    ...
    # Check API key — try provider-specific env vars, then generic OPENAI_API_KEY
...
# ---------------------------------------------------------------------------
# P1-10: Stop generation endpoint — frontend notifies backend to abort stream
# ---------------------------------------------------------------------------
...
async def stop_generation(request: Request):
    ...
    Token-protected to prevent abuse."""
    _require_token(request)
    body = await request.json()
...
def _get_chat_credentials() -> tuple[str, str, str]:
    ...
    Supports multiple providers: reads provider-specific API keys from .env
    based on the 'provider' field in config.yaml.
    ...
    home = get_hermes_home()  # Uses VERMES_HOME or ~/.vermes
    cfg_path = home / "config.yaml"
    env_path = home / ".env"
    ...
    # Default base_url for each provider (if not in config)
    PROVIDER_BASE_URL = {
    ...
    # Use provider's default base_url if not in config
...
# ---------------------------------------------------------------------------
# Model-to-provider routing for WebChat (Agent-powered)
# ---------------------------------------------------------------------------
...
def _resolve_model_provider(model: str, explicit_provider: str | None = None) -> tuple:
    ...
    Uses explicit provider (from frontend) first, then prefix matching
    against MODEL_PROVIDER_MAP, then config.yaml default.
    """
    from hermes_constants import get_hermes_home
    home = get_hermes_home()
    env_path = home / ".env"

    # Priority 1: explicit provider from frontend
    provider = explicit_provider or ""

    # Priority 2: prefix matching
    if not provider:
...
async def chat_completions(req: ChatRequest):
    ...
    Creates a fresh AIAgent instance per request, configured with current
    Vermes settings (model, provider, api_key), and runs the full agent
    ...
    # ── 所有消息统一走 Agent 模式（支持 tool calling + 流式输出）──
    use_agent_mode = True
    agent = None
    try:
        agent = AIAgent(
    ...
    if req.stream:
        ...
        if agent is None:
            ...
            # Agent mode — 实时流式输出
            # P1-11: asyncio.Queue 替代 queue.Queue，消除 50ms 轮询延迟
            # P1-10: stream_id 追踪，支持前端 stop 通知后端
            ...
            def stream_callback(delta: str):
                """被agent调用，通过 loop.call_soon_threadsafe 安全放入 asyncio.Queue"""
                try:
            ...
            async def stream_generator():
                try:
                    # P1-10: 先发送 stream_id 给前端，前端 stop 时回传
