...
import hermes_cli.scholarforge.active_project as ap
...
class TestListProjectsMarksActive(unittest.TestCase):
    def test_active_marker_present(self):
        ...
        with patch("hermes_cli.scholarforge.database.list_projects", return_value=fake_projects):
    ...
    def test_no_projects_message(self):
        ap.set_active_project(0)
        with patch("hermes_cli.scholarforge.database.list_projects", return_value=[]):
            result = asyncio.run(_handle_scholarforge_list_projects({}))
...
class TestSetActiveProjectHandler(unittest.TestCase):
    ...
    def test_sets_active_and_reports(self):
        with patch("hermes_cli.scholarforge.database.get_project", return_value={"id": 99, "title": "X"}):
    ...
    def test_nonexistent_project_reports_error(self):
        with patch("hermes_cli.scholarforge.database.get_project", return_value=None):
...
class TestWriteMissingProjectId(unittest.TestCase):
    ...
    def test_missing_pid_emits_warning_not_silent(self):
        with patch("hermes_cli.scholarforge.tools._call_llm", return_value="# 引言\n这是正文内容。"):
...
class TestWriteFallsBackToActive(unittest.TestCase):
    ...
    def test_active_resolves_no_warning_and_saves(self):
        ...
        fake_ctx = "【项目】硕士论文上下文"
        with patch("hermes_cli.scholarforge.tools._call_llm", return_value="# 引言\n正文段落。") as _m, \
             patch("hermes_cli.scholarforge.project_context.auto_snapshot") as m_snap, \
             patch("hermes_cli.scholarforge.project_context.format_project_context_prompt", return_value=fake_ctx), \
             patch("hermes_cli.scholarforge.project_context.load_project_context", return_value={"title": "T", "paper_type": "硕士论文"}), \
             patch("hermes_cli.scholarforge.project_context.get_style_prompt", return_value=""), \
             patch("hermes_cli.scholarforge.project_context.save_section") as m_save, \
             patch("hermes_cli.scholarforge.quality_gate.run_quality_gate",
                   return_value=("# 引言\n正文段落。", "", False)):
            result = asyncio.run(_handle_scholarforge_write({"topic": "t", "section_type": "abstract"}))
...
class TestReplaceCitationsMissingProjectId(unittest.TestCase):
    ...
    def test_missing_pid_emits_warning_and_still_replaces(self):
        ...
        def run():
            with patch("hermes_cli.scholarforge.search.search_papers", fake_search_papers), \
                 patch("hermes_cli.scholarforge.tools._call_llm", side_effect=fake_llm), \
                 patch("hermes_cli.scholarforge.citation_verifier._fuzzy_verify", return_value=fake_verify):
