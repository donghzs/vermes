...
>
> **2026-08-12 第三轮**：收尾 1/2/3 已于 commit `bcf80312a` 落地（删 `tools.py` 重复副本 + 改 docstring + 强化 F-24 行为契约测试，R5 反向验证证明非真空）。**对外文案四条已定稿**（用户拍板：真实功能均写、需 key/账号者注明；唯一例外「百度学术」因 F-12 不可达 + F-13 名实不符，launch 前不得写，见 §第四梯队）。**至此 ScholarForge 审计全部 F-1~F-25 修复 + 收尾 + 文案定稿，Launch 阻断项清零。**
...
**文案硬约束（用户拍板，2026-08-12）**：真实功能就写；但需要账号/密码/key 的，**必须注明**。学术场景里「说了没做」比「没做」更危险。

按此原则逐条定稿（均为真实功能，故均可写，仅标注差异）：

| 原话 | 对外写法 | 需注明项 |
|---|---|---|
| 查重 | 「**文内重复度自查**」 | 标注"不比对外部已发表文献库"（无 key，纯本地 SimHash 自比，F-9） |
| AIGC 检测 | 「**AI 写作风格自检（启发式）**」 | 标注"不等同于任何期刊使用的 AIGC 检测服务"（无 key，启发式非模型，F-10） |
| 支持知网 | 「**支持知网检索（需配置知网 API key）**」 | 必须注明需 key；无 key 时降级 OpenAlex CN（行为诚实，F-14/`cnki_fetcher.py`） |
| 支持百度学术 | ⚠️ **暂不能写**（见下） | — |

**百度学术的例外（唯一不能按"真实功能"处理的）**：它同时满足两个硬伤——(a) `search/__init__.py:275-283` 的 registry 短路使 `baidu_scholar` 分支**永不执行**（F-12，死代码）；(b) 即使可达，`baidu_scholar_fetcher.py:4-11,253` 实为 **Crossref + OpenAlex 包装**，根本没碰百度学术（F-13）。即它不是"需 key 的真实功能"，而是"名字叫百度学术、实为英文库、且当前不可达"。所以：
- 在 F-12 修好前，不得出现"支持百度学术"；
- 修好后，建议**改名**为「多源学术检索（Crossref / OpenAlex 等）」而非继续用"百度学术"之名——否则仍属名实不符。

其余：`research_map` / `claim_audit` 等 LLM 推断类输出加显式免责：「以下为模型推断，未经文献验证，请勿直接引用」。

> 注：以上为对外文案约束，不要求改代码（除 F-12 百度学术可达性）。代码侧 `plagcheck.py:481,527` 的 `checked_sources` 已诚实标注"启发式"，与文案策略一致。
...
- 宣传口径四条 —— ✅ **已定稿**（2026-08-12 用户拍板）：真实功能均写，需 key/账号者注明；唯一例外「百度学术」因 F-12 不可达 + F-13 名实不符，launch 前不得写（见 §第四梯队）
