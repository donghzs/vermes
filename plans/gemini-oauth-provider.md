...
## Client ID
- Need to register a "Desktop app" OAuth client on a Vermes GCP project
...
## Token Lifecycle
- Store at `~/.vermes/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `vermes_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `vermes_cli/models.py` — Add Gemini model catalog
4. `vermes_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `vermes_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `vermes_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- Vermes GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via VERMES_GEMINI_CLIENT_ID env var
...
- vermes-agent Copilot OAuth: `vermes_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
