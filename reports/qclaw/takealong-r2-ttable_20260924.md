...
| 取长续期 | T8 `file_safety` | **L-014** `_guard_homes` multi-home | 5+3 passed |

**用户可见配额（本周）**：T16① 审批计数按本会话 + ③ 中间产物误标修复 — **≥1 条达标**（可在 GUI 感知）。

## 3. 新增取长/修复（§7c L-014~L-026）

| id | 来源 | 类型 | 人时 | 上游后续变更 |
|---|---|---|---|---|
| L-014 | `7c478ac257a3` | 重写 | ~0.5h | 2 |
| L-015 | — | 修复（自有） | ~0.4h | — |
...
**取长登记 12 → 28 行**（含拒绝/暂缓；验收线 15+ 已过）。每条新行均有「上游后续变更」+人时。

## 4. 验收

| 闸门 | 结果 |
|---|---|
| boundary | **未登记税 0** · 已登记偏离 66 · core 登记率 100% (9/9) |
