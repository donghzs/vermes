...
const SESSION_HEADER = "X-Vermes-Session-Token";
...
export async function fetchJSON<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  const token = window.__VERMES_SESSION_TOKEN__;
  if (token) {
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__VERMES_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Vermes dashboard server");
}
