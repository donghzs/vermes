...
import { VERMES_BASE_PATH } from "@/lib/api";
...
export class GatewayClient {
  ...
  async connect(token?: string): Promise<void> {
    ...
    const resolved = token ?? window.__VERMES_SESSION_TOKEN__ ?? "";
    if (!resolved) {
      this.setState("error");
      throw new Error(
        "Session token not available — page must be served by the Vermes dashboard",
      );
    }

    const scheme = location.protocol === "https:" ? "wss:" : "ws:";
    const ws = new WebSocket(
      `${scheme}//${location.host}${VERMES_BASE_PATH}/api/ws?token=${encodeURIComponent(resolved)}`,
    );
    this.ws = ws;
