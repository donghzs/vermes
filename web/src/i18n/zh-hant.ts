...
export const zhHant: Translations = {
  ...
  pluginsPage: {
    ...
    headline:
      "探索、安裝、啟用並更新 vbit Agent 外掛（對齊 `vermes plugins` CLI）。",
    ...
    removeConfirm: "從 ~/.vermes/plugins/ 移除此外掛？",
    removeHint: "僅可移除位於 ~/.vermes/plugins 下使用者安裝的外掛。",
  ...
  skills: {
    ...
    categories: "分類",
    filters: "篩選",
    noSkills: "找不到技能。技能由 ~/.vermes/skills/ 載入",
  ...
  achievements: {
    hero: {
      ...
      title: "vbit Agent 成就",
      subtitle:
        "從真實工作階段歷史中獲得的 vbit Agent 可收集徽章。已知尚未達成的成就會顯示為「已發現」；秘密成就在首次出現相符行為之前保持隱藏。",
      scan_subtitle:
        "正在掃描 vbit Agent 工作階段歷史。在歷史紀錄較多時，首次掃描可能需要 5–10 秒。",
    },
    actions: {
      rescan: "重新掃描",
    },
    stats: {
      ...
      highest_tier: "最高等級",
      highest_tier_hint: "銅 → 銀 → 金 → 鑽石 → 奧林匹亞",
      latest: "最新",
      latest_hint_empty: "多多使用 vbit Agent",
      none_yet: "尚無",
    },
    state: {
      unlocked: "已解鎖",
    ...
    guide: {
      ...
      secret_header: "秘密成就",
      secret_body:
        "秘密成就會隱藏其確切觸發條件。一旦 vbit Agent 偵測到相關訊號，卡片便會變為「已發現」並顯示其需求。",
      scan_status_header: "掃描狀態",
      scan_status_body:
        "vbit Agent 正在對本機歷史進行一次掃描，之後卡片會自動出現。即使需要幾秒鐘，也並未卡住。",
    ...
    share: {
      ...
      tweet_text: "剛剛解鎖了 {tier_part}\"{name}\" — vbit Agent ⚡",
    },
  },
  kanban: {
    loading: "正在載入看板…",
