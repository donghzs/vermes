...
export const zh: Translations = {
  ...
  chat: {
    newChat: "新对话",
    welcome: "你好！我是 vbit Agent，有什么可以帮你？",
    placeholder: "输入消息… (Enter 发送, Shift+Enter 换行)",
    powered: "本地智能助手",
    noConversation: "开始新对话",
  },
  ...
  pluginsPage: {
    ...
    headline: "发现、安装、启用和更新 vbit Agent 插件（对齐 `vermes plugins` CLI）。",
    ...
    removeConfirm: "从 ~/.vermes/plugins/ 删除此插件？",
    removeHint: "仅可移除用户安装在 ~/.vermes/plugins 下的插件。",
  ...
  skills: {
    ...
    categories: "分类",
    filters: "筛选",
    noSkills: "未找到技能。技能从 ~/.vermes/skills/ 加载",
  ...
  achievements: {
    hero: {
      ...
      title: "vbit Agent 成就",
      subtitle:
        "从真实会话历史中获得的 vbit Agent 可收集徽章。已知尚未达成的成就显示为「已发现」；秘密成就在首次出现匹配行为之前保持隐藏。",
      scan_subtitle:
        "正在扫描 vbit Agent 会话历史。在历史记录较多时，首次扫描可能需要 5–10 秒。",
    },
    actions: {
      rescan: "重新扫描",
    },
    stats: {
      ...
      highest_tier: "最高等级",
      highest_tier_hint: "铜 → 银 → 金 → 钻石 → 奥林匹亚",
      latest: "最新",
    ...
    guide: {
      ...
      secret_header: "秘密成就",
      secret_body:
        "秘密成就会隐藏其确切触发条件。一旦 vbit Agent 检测到相关信号，卡片将变为「已发现」并显示其要求。",
      scan_status_header: "扫描状态",
      scan_status_body:
        "vbit Agent 正在对本地历史进行一次扫描，之后卡片会自动出现。即使这需要几秒钟，也没有卡住。",
    ...
    share: {
      ...
      tweet_text: "刚刚解锁了 {tier_part}\"{name}\" — vbit Agent ⚡",
    },
  },
