<template>
  <div class="benchmark-dashboard p-6 max-w-6xl mx-auto">
    ...
    <div class="flex items-center gap-3 mb-6">
      ...
      <select v-model="llmTier" class="px-3 py-2 border rounded-lg bg-gray-800 text-gray-100 border-gray-600">
    ...
    <div v-if="runs.length > 0" class="mb-8">
      <h3 class="text-sm font-semibold text-gray-400 mb-2">通过率趋势</h3>
    ...
    <!-- 任务覆盖面 -->
    <div class="mb-8">
      <h3 class="text-sm font-semibold text-gray-400 mb-2">任务覆盖面（{{ tasks.length }} 个任务）</h3>
      <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
        <div
          v-for="t in tasks"
          :key="t.id"
          class="p-3 rounded-lg border border-gray-700 bg-gray-800/50"
        >
          <div class="flex items-center justify-between">
            <span class="text-sm font-medium">{{ t.title }}</span>
            <span
              class="text-xs px-1.5 py-0.5 rounded"
              :class="t.kind === 'pipeline' ? 'bg-purple-900/50 text-purple-300' : 'bg-blue-900/50 text-blue-300'"
            >
              {{ t.kind }}
            </span>
          </div>
    ...
    <div v-if="runs.length > 0">
      <h3 class="text-sm font-semibold text-gray-400 mb-2">历史运行</h3>
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            >
              <td class="py-2 px-3 text-gray-300">{{ formatTime(run.timestamp) }}</td>
              <td class="py-2 px-3">
                <span :class="run.mode === 'dry' ? 'text-gray-400' : 'text-emerald-400'">{{ run.mode }}</span>
              </td>
              <td class="py-2 px-3 text-gray-400">{{ run.llm_tier }}</td>
              <td class="py-2 px-3">
              ...
              <td class="py-2 px-3 text-gray-400">{{ run.summary?.wiring_rate ? Math.round(run.summary.wiring_rate * 100) + '%' : '-' }}</td>
              <td class="py-2 px-3 text-gray-400">{{ run.summary?.total_wall_time_s ? run.summary.total_wall_time_s.toFixed(1) + 's' : '-' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <!-- 空态 -->
    <div v-if="!running && runs.length === 0" class="text-center py-12 text-gray-500">
...
async function loadRuns() {
  try {
    const r = await fetch('/api/v1/benchmark/runs?limit=50')
    if (r.ok) {
...
async function loadTasks() {
  try {
    const r = await fetch('/api/v1/benchmark/tasks')
    if (r.ok) {
      const body = await r.json()
