...
def register_credential_file(
    ...
    *relative_path* is relative to ``VERMES_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside VERMES_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    VERMES_home = _resolve_vermes_home()

    # Reject absolute paths — they bypass the VERMES_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to VERMES_HOME)",
            relative_path,
        )
        return False

    host_path = VERMES_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape VERMES_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, VERMES_home)
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="Vermes-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching vermes_constants.get_vermes_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_vermes_dir()`` for backward compatibility with old directory layouts.
    """
    from vermes_constants import get_vermes_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_vermes_dir(new_subpath, old_name)
        if host_dir.is_dir():
