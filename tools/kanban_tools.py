...
running under the dispatcher (env var ``VERMES_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``Vermes chat`` session still sees **zero**
...
1. **Backend portability.** A worker whose terminal tool points at Docker
   ...
   inside the container, where ``Vermes`` isn't installed and the DB
...
(``Vermes dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``VERMES_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``Vermes chat`` without the kanban toolset see zero
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("VERMES_KANBAN_TASK")
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``VERMES_KANBAN_TASK`` set
    ...
    ``VERMES_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``VERMES_KANBAN_DB`` → ``VERMES_KANBAN_BOARD`` env → current symlink
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set VERMES_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # VERMES_KANBAN_CLAIM_LOCK in the worker env at spawn time
            # (see _default_spawn in kanban_db.py); falling back to the
            # default _claimer_id() covers locally-driven workers that
            # never went through the dispatcher path.
            claim_lock = os.environ.get("VERMES_KANBAN_CLAIM_LOCK")
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``Vermes-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("VERMES_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("VERMES_TENANT")
    ...
    try:
        ...
        try:
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("VERMES_PROFILE") or "worker",
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to VERMES_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: VERMES_KANBAN_DB env → "
    "VERMES_KANBAN_BOARD env → the 'current' symlink under the kanban "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to VERMES_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
