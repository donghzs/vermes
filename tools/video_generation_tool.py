        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `vermes tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    ...
    msg = (
        "No video generation backend is configured. Run `vermes tools` → "
...
# `vermes tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`vermes tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `vermes tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
