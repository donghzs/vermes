...
Lets the LLM write a Python script that calls Vermes tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `VERMES_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `VERMES_tools.py` with file-based RPC stubs
...
# available on every platform Vermes itself runs on.
...
# ---------------------------------------------------------------------------
# VERMES_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "search_files": (
        "search_files",
        'pattern: str, target: str = "content", path: str = ".", file_glob: str = None, limit: int = 50, offset: int = 0, output_mode: str = "content", context: int = 0, respect_gitignore: bool = False',
        '"""Search file contents (target="content") or find files by name (target="files"). Returns dict with "matches". Defaults respect_gitignore=False so files you just wrote in this sandbox are always findable."""',
        '{"pattern": pattern, "target": target, "path": path, "file_glob": file_glob, "limit": limit, "offset": offset, "output_mode": output_mode, "context": context, "respect_gitignore": respect_gitignore}',
    ),
    "patch": (
        "patch",
...
def generate_vermes_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the VERMES_tools.py stub module.
...
"""Auto-generated Vermes tools RPC stubs."""
...
def _connect():
    ...
    VERMES_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["VERMES_RPC_SOCKET"]
...
"""Auto-generated Vermes tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("VERMES_RPC_DIR") or os.path.join(tempfile.gettempdir(), "VERMES_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated VERMES_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/VERMES_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_vermes_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/VERMES_tools.py", tools_src)
        ...
        env_prefix = (
            f"VERMES_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("VERMES_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with VERMES_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="VERMES_sandbox_")
    ...
    # selector from VERMES_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"VERMES_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated VERMES_tools module.
        ...
        tools_src = generate_vermes_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "VERMES_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  VERMES_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["VERMES_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the Vermes-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from VERMES_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _vermes_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _vermes_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # VERMES_TIMEZONE is an internal Vermes setting and must not leak
        # into child processes.
        _tz_name = os.getenv("VERMES_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("VERMES_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {VERMES_HOME}/home/ when that directory exists.
        from vermes_constants import get_subprocess_home
        _profile_home = get_subprocess_home()
        if _profile_home:
            child_env["HOME"] = _profile_home
            from vermes_constants import get_real_home
            child_env["VERMES_REAL_HOME"] = get_real_home()
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.vermes/.env')).
...
def _load_config() -> dict:
    ...
    ``Vermes --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (Vermes-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``vermes tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and Vermes-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.vermes/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Vermes tools programmatically. "
        ...
        f"Available via `from VERMES_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into VERMES_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from VERMES_tools import {import_str}` "
