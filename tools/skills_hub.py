#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Vermes Skills Hub.
...
Used by vermes_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            VERMES_meta = metadata.get("Vermes", {})
            if isinstance(VERMES_meta, dict):
                tags = VERMES_meta.get("tags", [])
        if not tags:
...
# ---------------------------------------------------------------------------
# Skillhub (skillhub.cn) source adapter
# ---------------------------------------------------------------------------

class SkillhubSource(SkillSource):
    """Discover and install skills from the Skillhub store (skillhub.cn).

    Uses the Skillhub search API and COS-hosted zip archives.
    Provides Chinese-first skill discovery with bilingual metadata.
    """
    ...
    HOMEPAGE_URL = "https://api.skillhub.cn/{slug}"

    def source_id(self) -> str:
        return "skillhub"

    def search(self, query: str, limit: int = 10) -> List[SkillMeta]:
        """Search Skillhub store with Chinese + English support."""
        cache_key = f"skillhub_search_{hashlib.md5(f'{query}|{limit}'.encode()).hexdigest()}"
        cached = _read_index_cache(cache_key)
        if cached is not None:
            return [SkillMeta(**item) for item in cached][:limit]
        ...
        try:
            resp = _guarded_http_get(
                self.SEARCH_URL + f"?q={quote(query)}&limit={limit}",
                timeout=15,
            )
                ...
                for item in data.get("results", [])[:limit]:
                    ...
                    results.append(SkillMeta(
                        ...
                        extra={
                            ...
                            "homepage": self.HOMEPAGE_URL.format(slug=slug),
                        },
                    ))
        except Exception as e:
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        """Fetch metadata for a single skill from index."""
        try:
            resp = _guarded_http_get(self.INDEX_URL, timeout=15)
            if resp and resp.status_code == 200:
                data = resp.json()
                for item in data.get("skills", []):
                    if item.get("slug", "").lower() == identifier.lower():
                        slug = item["slug"]
                        desc = item.get("description_zh", "") or item.get("description", "")
                        return SkillMeta(
            ...
            logger.debug("Skillhub inspect failed: %s", e)
        return None

    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        """Download a skill zip from Skillhub COS."""
        slug = identifier.strip().strip("/")

        # Try primary download URL first, fall back to COS
        for url in [
            self.PRIMARY_DOWNLOAD_URL.format(slug=slug),
            self.DOWNLOAD_URL_TEMPLATE.format(slug=slug),
        ]:
            try:
                resp = _guarded_http_get(url, timeout=60)
                if resp and resp.status_code == 200 and len(resp.content) > 100:
                    return self._parse_zip(slug, resp.content)
            except Exception as e:
                logger.debug("Skillhub download from %s failed: %s", url, e)
                continue

        return None

    def _parse_zip(self, slug: str, zip_data: bytes) -> Optional[SkillBundle]:
        """Parse a zip archive into a SkillBundle."""
        import zipfile
        import io
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(zip_data)) as zf:
                for info in zf.infolist():
                    if info.is_dir():
                        continue
                    # Normalize: strip top-level directory if present
                    parts = info.filename.split("/")
                    if len(parts) > 1 and parts[0].endswith("-" + slug.split("-")[0]):
            ...
            logger.debug("Skillhub zip parse failed: %s", e)
            return None
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Vermes) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.vermes/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                VERMES_meta = meta_block.get("Vermes", {})
                if isinstance(VERMES_meta, dict):
                    tags = VERMES_meta.get("tags", [])
...
def _load_vermes_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for VERMES_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if VERMES_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - VERMES_INDEX_CACHE_FILE.stat().st_mtime
            if age < VERMES_INDEX_TTL:
                return json.loads(VERMES_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(VERMES_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Vermes index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Vermes index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        VERMES_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        VERMES_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class VermesIndexSource(SkillSource):
    """Skill source backed by the centralized Vermes Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_vermes_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "Vermes-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "Vermes-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        VermesIndexSource(auth=auth), # Centralized index (search + resolved install paths)
        SkillsShSource(auth=auth),
        WellKnownSkillSource(),
        UrlSource(),                  # Direct HTTP(S) URL to a SKILL.md file
        GitHubSource(auth=auth, extra_taps=extra_taps),
        ClawHubSource(),
        SkillhubSource(),               # skillhub.cn: 中文技能商店
