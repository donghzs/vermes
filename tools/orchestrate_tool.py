...
当用户（在单聊或移动渠道）下达一个需要多角色协作的任务时，用本工具把它交给秘书分身组队执行。适合：需要不同专长（研究/编码/法律/写作/分析等）分工协作、或需要拆解-执行-审计-汇总流水线的任务。简单的单步任务不要用本工具，自己直接做即可。
...
delegate 模式会阻塞直到流水线跑完（拆解→执行→审计→汇总→交付），耗时较长（数十秒到数分钟），适合真需要多 agent 协作的场景。""",
    "parameters": {
        "type": "object",
        "properties": {
            "action": {
