...
Backend is selected during ``vermes tools`` setup (web.backend in config.yaml).
When available, Vermes can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.vermes/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``vermes tools``).
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "donghzs.com" or host.endswith(".donghzs.com")
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `vermes tools` to set one up."
                ),
            }
        else:
            logger.info(
...
WEB_SEARCH_SCHEMA = {
    "name": "web_search",
    "description": "Search the web for information. Returns up to 5 results by default with titles, URLs, and descriptions. The query is passed through to the configured backend, so operators such as site:domain, filetype:pdf, intitle:word, -term, and \"exact phrase\" may work when the backend supports them. For academic research, add site:arxiv.org or site:semanticscholar.org to find papers, or use web_extract on PDF URLs to get full text.",
    "parameters": {
        "type": "object",
        "properties": {
            "query": {
