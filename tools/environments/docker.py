...
def _load_vermes_env_vars() -> dict[str, str]:
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``VERMES_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("VERMES_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using VERMES_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'Vermes'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        container_name = f"Vermes-{uuid.uuid4().hex[:8]}"
        run_cmd = [
