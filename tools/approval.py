...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("VERMES_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("VERMES_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("VERMES_SESSION_PLATFORM", "") or ""


def _is_cron_session() -> bool:
    """True when the current task/thread is a cron job (task-local first).

    Delegates to ``gateway.session_context.is_cron_session`` so a cron job
    running inside the gateway process is detected via its contextvar rather
    than the (historically process-global, never-cleared)
    ``VERMES_CRON_SESSION`` env var. The env fallback is preserved for the
    standalone ``vermes cron`` daemon and test/CLI processes.
    """
    try:
        ...
        return env_var_enabled("VERMES_CRON_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set VERMES_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind VERMES_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds VERMES_SESSION_PLATFORM via
    ...
    if _is_cron_session():
        return False
    if env_var_enabled("VERMES_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $VERMES_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_vermes_ENV_PATH = (
    r'(?:~\/\.vermes/|'
    r'(?:\$home|\$\{home\})/\.vermes/|'
    r'(?:\$VERMES_home|\$\{VERMES_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bvermes\s+gateway\s+(stop|restart)\b', "stop/restart vermes gateway (kills running agents)"),
    (r'\bvermes\s+update\b', "vermes update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart Vermes-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart Vermes-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(Vermes|gateway|cli\.py)\b', "kill Vermes/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill Vermes` but not
    # `kill -9 $(pgrep -f Vermes)` because the substitution is opaque
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["VERMES_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "VERMES_SPINNER_PAUSE" in os.environ:
            del os.environ["VERMES_SPINNER_PAUSE"]
        logger.info("")
        sys.stdout.flush()
...
def _smart_approve(command: str, description: str) -> str:
    ...
    try:
        ...
        user_prompt = (
            ...
            '`python -c "logger.info(\'hello\')"` is flagged as "script execution '
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("VERMES_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("VERMES_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}
