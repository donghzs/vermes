...
-----------------------------------------------------------------------------
    ...
    ~/.vermes/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/Vermes/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_vermes_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/Vermes"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("VERMES_CHECKPOINT_TIMEOUT", "30"))))
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``Vermes checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `Vermes checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "Vermes@local"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``VERMES_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at VERMES_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "VERMES_WORKDIR").write_text(
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "VERMES_WORKDIR"
            if wd_marker.exists():
                try:
...
# ---------------------------------------------------------------------------
# Public helpers for `Vermes checkpoints` CLI
# ---------------------------------------------------------------------------
