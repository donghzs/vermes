...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    VERMES_node_bin = str(VERMES_home / "node" / "bin")
    VERMES_node_root = str(VERMES_home / "node")
    VERMES_nm_bin = str(VERMES_home / "node_modules" / ".bin")
    return [VERMES_node_bin, VERMES_node_root, VERMES_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-VERMES_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed Vermes processes — this way every clean Vermes exit sweeps
    ...
    # Sweep orphans from other crashed Vermes processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live Vermes processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current Vermes PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live Vermes process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning Vermes process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced Vermes PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent Vermes instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-VERMES_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live Vermes process.
            continue
...
def _find_agent_browser() -> str:
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                VERMES_nm = str(get_vermes_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=VERMES_nm)
            if not recheck:
                VERMES_node_bin = str(get_vermes_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=VERMES_node_bin)
            if not recheck:
                VERMES_node_root = str(get_vermes_home() / "node")
                recheck = shutil.which("agent-browser", path=VERMES_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this Vermes PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = VERMES_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    screenshots_dir = get_vermes_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    logger.info("       docker pull ghcr.io/donghzs/vermes:latest")
                else:
