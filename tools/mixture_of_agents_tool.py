...
async def mixture_of_agents_tool(
    ...
    try:
        ...
        if not get_api_key("openrouter", env_var="OPENROUTER_API_KEY"):
...
if __name__ == "__main__":
    ...
    """
    logger.info("🤖 Mixture-of-Agents Tool Module")
    logger.info("=" * 50)
    ...
    if not api_available:
        logger.info("❌ OPENROUTER_API_KEY environment variable not set")
        logger.info("Please set your API key: export OPENROUTER_API_KEY='your-key-here'")
        logger.info("Get API key at: https://openrouter.ai/")
        sys.exit(1)
    else:
        logger.info("✅ OpenRouter API key found")
    
    logger.info("🛠️  MoA tools ready for use!")
    
    # Show current configuration
    config = get_moa_configuration()
    logger.info("\n⚙️  Current Configuration:")
    logger.info(f"  🤖 Reference models ({len(config['reference_models'])}): {', '.join(config['reference_models'])}")
    logger.info(f"  🧠 Aggregator model: {config['aggregator_model']}")
    logger.info(f"  🌡️  Reference temperature: {config['reference_temperature']}")
    logger.info(f"  🌡️  Aggregator temperature: {config['aggregator_temperature']}")
    logger.info(f"  🛡️  Failure tolerance: {config['failure_tolerance']}")
    logger.info(f"  📊 Minimum successful models: {config['min_successful_references']}")
    
    # Show debug mode status
    if _debug.active:
        logger.info(f"\n🐛 Debug mode ENABLED - Session ID: {_debug.session_id}")
        logger.info(f"   Debug logs will be saved to: ./logs/moa_tools_debug_{_debug.session_id}.json")
    else:
        logger.info("\n🐛 Debug mode disabled (set MOA_TOOLS_DEBUG=true to enable)")
    
    logger.info("\nBasic usage:")
    logger.info("  from mixture_of_agents_tool import mixture_of_agents_tool")
    logger.info("  import asyncio")
    logger.info("")
    logger.info("  async def main():")
    logger.info("      result = await mixture_of_agents_tool(")
    logger.info("          user_prompt='Solve this complex mathematical proof...'")
    logger.info("      )")
    logger.info("      logger.info(result)")
    logger.info("  asyncio.run(main())")
    
    logger.info("\nBest use cases:")
    logger.info("  - Complex mathematical proofs and calculations")
    logger.info("  - Advanced coding problems and algorithm design")
    logger.info("  - Multi-step analytical reasoning tasks")
    logger.info("  - Problems requiring diverse domain expertise")
    logger.info("  - Tasks where single models show limitations")
    
    logger.info("\nPerformance characteristics:")
    logger.info("  - Higher latency due to multiple model calls")
    logger.info("  - Significantly improved quality for complex tasks")
    logger.info("  - Parallel processing for efficiency")
    logger.info(f"  - Optimized temperatures: {REFERENCE_TEMPERATURE} for reference models, {AGGREGATOR_TEMPERATURE} for aggregation")
    logger.info("  - Token-efficient: only returns final aggregated response")
    logger.info("  - Resilient: continues with partial model failures")
    logger.info("  - Configurable: easy to modify models and settings at top of file")
    logger.info("  - State-of-the-art results on challenging benchmarks")
    
    logger.info("\nDebug mode:")
    logger.info("  # Enable debug logging")
    logger.info("  export MOA_TOOLS_DEBUG=true")
    logger.info("  # Debug logs capture all MoA processing steps and metrics")
    logger.info("  # Logs saved to: ./logs/moa_tools_debug_UUID.json")
