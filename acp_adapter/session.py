"""ACP session manager — maps ACP sessions to Vermes AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.vermes/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Vermes itself is running in WSL.

    Windows ACP clients can launch ``vermes acp`` inside WSL while still sending
    ...
    """
    from vermes_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Vermes from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["vermes-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Vermes agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Vermes AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Vermes runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.vermes/state.db``) is lazily created.
        """
