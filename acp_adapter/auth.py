"""ACP auth helpers — detect and advertise Vermes authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "vermes-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Vermes runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Vermes can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Vermes.
    ...
    not have Vermes provider credentials configured yet, so Vermes always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Vermes using the currently configured "
