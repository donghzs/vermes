...
VERSION_FILE = REPO_ROOT / "vermes_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+Vermes-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@donghzs.com": "shannonsands",
    ...
    "Vermes@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "REDACTED_USER@localhost.localdomain": "holynn-q",
    "holynn@placeholder.local": "holynn-q",
    "agent@Vermes.local": "jacdevos",
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (VERMES_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (VERMES_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: Vermes-eval community link)
    ...
    "vermesagent26@gmail.com": "vermesagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+vermesagent26@users.noreply.github.com": "vermesagent26",
    ...
    "clement@donghzs.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@Vermes": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.Vermes@agentmail.to": "oseftg",
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level VERMES_HOME state files)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"Vermes-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "Vermes@donghzs.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/donghzs/vermes",
    ...
    lines.append(f"# Vermes v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Vermes. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Vermes Release Tool")
    ...
    if args.publish:
        ...
        gh_cmd = [
            ...
            "--title", f"Vermes v{new_version} ({calver_date})",
