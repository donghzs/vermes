#!/usr/bin/env python3
"""
SQLite State Store for Vermes Agent.
...
# vermes_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``vermes_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see vermes_cli/kanban_db.py) would
...
class SessionDB:
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``vermes_state._set_last_init_error(None)`` explicitly.
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    ...
                    "disabled. This usually means Vermes is running on an "
                    ...
                    "https://donghzs.github.io/vermes (underlying error: %s)",
                    self.db_path,
                    fts_exc,
                )
    ...
    def _insert_message_rows(
        ...
        ``(inserted_count, tool_call_count)``.
        """
        now_ts = time.time()
        inserted = 0
        tool_calls_total = 0
    ...
    def archive_and_compact(
        ...
        - The live-context load filters ``active = 1`` by default, so the model
        ...
        - The archived pre-compaction turns stay on disk (active=0) and stay
          ...
          hidden. They remain in the FTS index and are recoverable via
    ...
    def get_messages(
        ...
        ``include_inactive=True`` to load soft-archived rows (e.g. for
        audit / debug views of compacted history).
        """
    ...
    def message_count(self, session_id: str = None) -> int:
        ...
        with self._lock:
            if session_id:
                cursor = self._conn.execute(
                    "SELECT COUNT(*) FROM messages WHERE session_id = ? AND active = 1", (session_id,)
                )
            else:
                cursor = self._conn.execute("SELECT COUNT(*) FROM messages WHERE active = 1")
            return cursor.fetchone()[0]
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Vermes, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Vermes session.

        A Vermes session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Vermes session is already bound to any Telegram DM topic.
