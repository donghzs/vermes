...
# 5. Symlinks the 'vermes' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Vermes Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/vermes-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink vermes into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up vermes command..."

VERMES_BIN="$SCRIPT_DIR/venv/bin/vermes"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$VERMES_BIN" "$COMMAND_LINK_DIR/vermes"
echo -e "${GREEN}✓${NC} Symlinked vermes → $COMMAND_LINK_DISPLAY_DIR/vermes"
...
# ============================================================================
# Seed bundled skills into ~/.vermes/skills/
# ============================================================================

VERMES_SKILLS_DIR="${VERMES_HOME:-$HOME/.vermes}/skills"
mkdir -p "$VERMES_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.vermes/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$VERMES_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  vermes status        # Check configuration"
if is_termux; then
    echo "  vermes gateway       # Run gateway in foreground"
else
    echo "  vermes gateway install # Install gateway service (messaging + cron)"
fi
echo "  vermes cron list     # View scheduled jobs"
echo "  vermes doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m vermes_cli.main setup
fi
