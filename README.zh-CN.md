<p align="center">
  <img src="assets/banner.png" alt="Vermes Agent" width="100%">
</p>
...
```bash
vermes              # 交互式 CLI — 开始对话
vermes model        # 选择 LLM 提供商和模型
vermes tools        # 配置启用的工具
vermes config set   # 设置单个配置项
vermes gateway      # 启动消息网关（Telegram、Discord 等）
vermes setup        # 运行完整设置向导（一次性配置所有内容）
vermes claw migrate # 从 OpenClaw 迁移（如果来自 OpenClaw）
...
Vermes 有两种入口：用 `vermes` 启动终端 UI，或运行网关从 Telegram、Discord、Slack、WhatsApp、Signal 或 Email 与之对话。进入对话后，许多斜杠命令在两种界面中通用。

| 操作 | CLI | 消息平台 |
|------|-----|----------|
| 开始对话 | `vermes` | 运行 `vermes gateway setup` + `vermes gateway start`，然后给机器人发消息 |
...
如果你来自 OpenClaw，Vermes 可以自动导入你的设置、记忆、技能和 API 密钥。

**首次安装时：** 安装向导（`vermes setup`）会自动检测 `~/.openclaw` 并在配置开始前提供迁移选项。

**安装后任意时间：**

```bash
vermes claw migrate              # 交互式迁移（完整预设）
vermes claw migrate --dry-run    # 预览将要迁移的内容
vermes claw migrate --preset user-data   # 仅迁移用户数据，不含密钥
vermes claw migrate --overwrite  # 覆盖已有冲突
```

导入内容：
- **SOUL.md** — 人格文件
- **记忆** — MEMORY.md 和 USER.md 条目
- **技能** — 用户创建的技能 → `~/.vermes/skills/openclaw-imports/`
- **命令白名单** — 审批模式
- **消息设置** — 平台配置、允许用户、工作目录
...
使用 `vermes claw migrate --help` 查看所有选项，或使用 `openclaw-migration` 技能进行交互式代理引导迁移（含干运行预览）。
...
cd vermes-agent
./setup-vermes.sh     # 安装 uv、创建 venv、安装 .[all]、创建符号链接 ~/.local/bin/vermes
./vermes              # 自动检测 venv，无需先 source
```
...
- 💬 [Discord](https://discord.gg/donghzs)
- 📚 [技能中心](https://agentskills.io)
- 🐛 [问题反馈](https://github.com/donghzs/vermes/issues)
- 💡 [讨论区](https://github.com/donghzs/vermes/discussions)
- 🔌 [vermesClaw](https://github.com/AaronWong1999/vermesclaw) — 社区微信桥接：在同一微信账号上运行 Vermes Agent 和 OpenClaw。
