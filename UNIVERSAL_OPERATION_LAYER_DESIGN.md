...
6. **L2 启动注册（bootstrap，2026-08-21 完成）** ✅：`vermes_cli/adapters/bootstrap.py` 新增 `discover_l2_adapters()`，在 `model_tools` 模块级（内置工具注册后、plugin discovery 后）扫描 PATH 上已安装的 `cli-anything-*`，对每个复用 `SoftwareAdapter.discover_tools()+register()` 全链路注册进运行态工具表。失败容错：单适配器 discover/register 失败仅记 warn、不阻塞其他；`--help` 内省加 15s 超时防损坏 CLI 挂死启动。扫描结果以**绝对路径**驱动 discover（避免「扫到 A 目录、`shutil.which` 按全局 PATH 跑 B 目录同名文件」的不一致）。已知 software→domain 映射（freecad/blender→3d），未知 fallback 到 software 名。真实 PATH 实测：发现 `cli-anything-freecad` 并注册 **273 工具**（与 §13.9 数字吻合）；隔离 tmp 端到端探针 `discover→parse→register→dispatch` 全通。测试 5 个（`tests/adapters/test_l2_bootstrap.py`）。
7. **默认 llm_chooser（2026-08-21 完成，用户定调「真实能力非测试桩」）** ✅：`vermes_cli/adapters/llm_chooser.py` 新增 `default_llm_chooser()`，复用 `agent/auxiliary_client.call_llm`（走 "auto" 检测链 = agent 统一配置的那个 LLM），**零独立配置**——小白开箱即用，不要求为 `select_tool` 单独配 API。模型无关：用「文本输出工具名」而非原生 `tool_choice`（DeepSeek/GPT/Gemini 通吃，不替用户假设模型能力）。未配置/失败/输出未命中候选集 → 返回 `None` → `select_tool` 降级启发式 argmax（兜底非主路径）。候选名校验按名字长度降序子串匹配（防编造 + 短名前缀误配）。`select_tool` 同步改语义：`llm_chooser` 返回 `None` = LLM 不可用 → 降级启发式，而非误判为选错（`NEEDS_CLARIFY`）。**开放性**：`llm_chooser` 参数已预留，用户可注入自己的函数（按域选模型/垂直域专用 prompt）覆盖默认——框架不预设域、不预设模型能力，域选择是用户生态自发生长。测试 6 个（`tests/adapters/test_llm_chooser.py`）+ select_tool 降级 1 用例。

---

> 文档版本：v1.10（战略基线 · 评审修订 · §13 spike · §13.8 B 桶关闭 · §13.9 Blender 双域实证 · §14 发现层+信任闸门 · §15 L2a/L2b 已落地 · §15.5 L2 ctx 接线 + L2 启动注册 + 默认 llm_chooser 完成）
...
> 实现落点（v1.10 更新）：`vermes_cli/adapters/discovery.py`(L2a，select_tool 支持 llm_chooser 返 None 降级) / `discovery_registry.py`(进程内索引) / `trust_gate.py`(L2b) / `software_adapter.py` 扩展(两层发现+闸门+索引+handler 契约修正+--help 超时) / `bootstrap.py`(启动注册) / `llm_chooser.py`(默认 llm_chooser，复用 agent LLM) / `tests/adapters/test_l2_discovery_trust.py`(22 passed) + `test_l2_bootstrap.py`(5 passed) + `test_llm_chooser.py`(6 passed) / `model_tools` 注入 ctx + 启动接 bootstrap / Blender spike 实跑 50 工具（§13.9）。B 桶 6/6 用户机器验收关闭（§13.8）。
> 下一步：第三域（office/ide）spike 扩展积木池广度 + 把 L2a（route_toolset+select_tool）接进 agent 工具选择流程（select_tool 生产调用点，待架构决策）。`ctx` 接线（§15.5 第 5 条）+ L2 启动注册（§15.5 第 6 条）+ 默认 llm_chooser（§15.5 第 7 条）已完成。
