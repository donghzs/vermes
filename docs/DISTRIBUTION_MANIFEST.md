...
| L-010 | —（**Vermes 自有缺陷**，非上游取长） | `gateway/session_context.py`, `agent/agent_init.py`, `agent/conversation_compression.py`, `acp_adapter/server.py`, `tools/kanban_tools.py` | 待做（真 bug，**首选**）：`set_current_session_id` 全仓 6 处调用 0 处定义 → writer 走 contextvar 静默回落 os.environ；ACP 进程级 save/restore 并发串味；reader 直读 os.environ 未走 `get_session_env()`。修法三步：① `session_context` 补 `set_current_session_id()`（写 `_SESSION_ID` ContextVar）② `kanban_tools.py:125/688` reader 改 `get_session_env("VERMES_SESSION_ID")` ③ `acp_adapter/server.py:1454` 改调 setter 弃用进程级 save/restore（与 L-007 同模式）。见 T13 | — | — | — | ⏳ 待做 |
