...
| L-020 | `2dfb795cb78f`+`1e2cb5797362`+`6332216384b7` | `tools/approval.py` | 重写（后续 6/7/8，家族合并）：未送达/未回答 = `cancelled` + 真实原因，不再写 `denied by user`；真 deny 才归用户 | `tests/tools/test_approval_cancelled_attribution.py` 4 passed | ~0.5h | 8 | ✅ 已合入（本切片） |
