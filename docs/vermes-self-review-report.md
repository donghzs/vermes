...
> **审查范围**: 前端 UI/UX、后端架构、Hermes 引擎、内置技能生态、非技术用户体验
...
| 配置目录 | 2 个 | ~/.vermes/（Vermes 分发配置）+ ~/.hermes/（Hermes 引擎配置） |
