...
| ⑥a | **Tier2** | **Bot Mode Phase 2a**（国产渠道群聊：飞书/钉钉/企微/微信） | v3.0.0 | ①③ | `gateway/platforms/weixin.py:362`（已提 `room_id`/`is_group`） | ~3–5d |
| ⑥b | **Tier2** | **Bot Mode Phase 2b**（海外渠道：Discord/Telegram/Slack） | v3.0.0+ | ①③⑥a | 各渠道 `@mention` 解析自写（SDK 原生优先） | ~3–5d |
...
| ⑭ | **原生愿景** | **Bot 实验室（发现→接入→编排→监控）+ Agent 一等公民** 🆕 | v2.5.0–v3.0.0 | ①③ | `vermes_cli/blueprints/capabilities.py` + `agent/capability_registry.py`（468 行）+ `vermes_cli/capabilities/registry.py` BrickRegistry + `vermes_cli/adapters/*` | **~430–570 行**（agent 类型 + `_discover_agents()` 80–120 / `LocalAgentScanner` ~150 / 前端 200–300） |
