...
> **版本状态**：**vFINAL.7（2026-09-06「⑭ 造神 / 神魔堂整轮 + ⑪ 状态纠偏」版）**。本轮变更：9-05 晚→9-06 新增 9 commit（`+1464/-256`）——⑭ 造神（per-agent 专属 API key + 原生 agent CRUD 端点 `b5cdccec06`）、神魔堂微信式群聊模型（建群/拉人/群公告/群任务/踢人/9 专家预设 `0572856075`、融合群聊+智能体市场 `00ea357bed`）、造神三连（厂商模型下拉 `745b59c6ed`、技能集勾选+按角色裁剪工具 `f8eae8eb4b`、按角色技能推荐 `b0a1b10f7e`）、群聊专家记忆按角色 scope 隔离 `438606cade`，附 frontend toast 运行时崩修复 `dc7e28f474` + web_dist 构建产物 `e3f7343582`。**已审计通过**：未重复造轮子（原生 agent 走 `agent_profiles.api_key` 列，外部 ACP agent 仍走 `~/.vermes/agent_auth.json` 凭据库，`vermes_state.py:27` 注释显式引用 `credentials.py`）、`api_key` 不回传前端（`GET /api/agents/native` 只返 `has_api_key` 布尔，chat.py L4292/L4327/L4567）、无 DB 泄露（`git ls-files` 无 `.db`，DB 在用户目录不进 git）、零回归实跑（botmode+a2a **86 passed** / hermes_state **204 passed + 9 failed 与基线一致**）。**审计问题（待补）**：后端新增 750 行仅 55 行测试改动（造神 CRUD / 技能裁剪 / 记忆 scope / 群聊踢人·群任务**无直接测试**，commit 称"验证通过"疑为手动验证未落成测试）；前端未经运行时验证（toast 导入错误能被提交）。⚠️ **⑪ 状态纠偏（2026-09-06 实证）**：源码仓 `channel_push|channelPush|push_channel` **零命中**，前端 `botRoom.js` 仅 `loadRooms/loadTimeline/loadMembers` 主动拉取、**无 SSE/WS/轮询** → **⑪ 未实现**；而 §1 标其为「**③ Bot Mode 群聊前的 P0 先决条件**（~150 行 SSE）」，现 ③ 群聊已落地而 ⑪ 未做 → **顺序倒置**，群聊当前靠手动刷新同步（详见 §13 vFINAL.7）。上一版 **vFINAL.6（2026-09-05「⑭ B 部分 · ACP Registry dump + 文档权威裁决」版）**：⚠️ 历史漏升已纠正：vFINAL.4 曾漏升（仍写 vFINAL.3）；本副本 vFINAL.6 内容一度误标 vFINAL.5，2026-09-05 审计后已对齐。相对 vFINAL 的全部变更见 §13。已落定：3 轮审阅补丁（A2A 行数上调、Phase2 拆 2a/2b、Plugin SDK↔MCP 对调、Cron 200–250 行、`_session_key_for_room` 澄清、锚点 2 处纠偏、⑪⑫⑬ 纳入）+ 异构融合平台升级（① transport adapter registry + 外部 agent 当 bot；③ agent_profiles 异构三列；#1 再纠偏）+ **⑭ Bot 实验室与 Agent 一等公民** + **⑮ 文档记忆层** + **通篇审计修复 14 项一致性问题（见 §9 #11 与 §13）** + **vFINAL.3 实证深化**：§11 重写为「错题本 / 笔记本 / 插件层」三发现（`anti_patterns` 僵尸表坐实、P3 学习管线出口坐实推翻"无出口"旧结论 §9 #12、`project_handoff` 通用笔记本仅单域发射、`continuity_facade` 7 源门面），⑮ 落地形态改**三条腿**（~170–260 行），§11.4 确立「错题本 + 笔记本 + 索引本」三件套方法论。
...
| ④ | **Tier2** | **Cron monitor-mode**（记忆 + hash 短路 + notepad） ✅ **2026-09-06 实证：核心逻辑已落地**（commit `794a774616`「feat(cron): ④ monitor-mode — hash 短路 + 记忆加载（零回归）」）——`cron/scheduler.py:1743` `skip_memory=not bool(job.get("monitor_mode"))` 已条件化、`_resolve_monitor_target()` L100 / `_get_monitor_hash()` L155 / `_store_monitor_hash()` L168 三 helper 在盘、`vermes_state.py` 两张表（`cron_monitor_state` L592 / `cron_notepad` L598）+ 索引 L1117 + 方法 L1832 齐备、`cron/jobs.py:528-529` `create_job` 参数链路 + L779-784 更新归一化已通。⚠️ **缺口二**：① **零测试覆盖**——grep tests/ 中 `monitor_mode\|monitor_target\|_resolve_monitor_target\|cron_notepad` **全零命中**；② **前端/API 未暴露 `monitor_mode` 开关**——grep `frontend/src` 零命中（spec §5 口径中 API/UI 那 ~50–80 行未做），即**用户目前无法在界面上开启 monitor-mode**，只能改库/调 API。⚠️ **历史误判更正**：2026-09-05 汇报曾称本项"未实现"，根因是 grep 只搜 `agent/` + `vermes_cli/` **漏了 `cron/` 目录**，属「否定性结论搜索范围必须覆盖整个模块目录」纪律第 9 次违反，非代码缺失 | v2.5.0 并行先落 | — | 见 `Vermes-cron-monitor-mode-spec.md`（锚点已漂移：spec 记 `scheduler.py:1622`，**实跑现为 `:1743`**——文档契约③ 锚点每次引用重新实跑） | 核心 **~110 行** ✅ 已做；API/UI 暴露 **~50–80 行** ❌ 未做（合计仍 ~200–250） |
...
| ⑪ | **Tier 1.5 / P0 先决** | **桌面-渠道实时同步（channel_push）** ⚠️ **2026-09-06 实证：❌ 未实现** —— 源码仓 `channel_push\|channelPush\|push_channel` **零命中**；前端 `botRoom.js` 仅 `loadRooms/loadTimeline/loadMembers` 主动拉取、**无 SSE / WS / 轮询** → 群聊当前**靠手动刷新**同步。⚠️ **顺序倒置**：本项 §1/§5 标为「**③ Bot Mode 群聊前的 P0 先决条件**」，但 ③ 群聊（`0572856075` 微信式群聊模型）已落地而本项未做，形成「先决条件未做、依赖它的功能已上线」的倒挂。补做口径仍为 ~150 行 SSE（事件总线底座 `tui_gateway/event_publisher.py` 思路可复用） | v2.5.0 | — | `tui_gateway/event_publisher.py`（事件总线思路可复用） | ~150 行（**未做**） |
| ⑭ | **原生愿景** | **Bot 实验室（发现→接入→编排→监控）+ Agent 一等公民** 🆕 | v2.5.0–v3.0.0 | ①③ | `vermes_cli/blueprints/capabilities.py` + `agent/capability_registry.py`（468 行）+ `vermes_cli/capabilities/registry.py` BrickRegistry + `vermes_cli/adapters/*` | **~430–570 行**（agent 类型 + `_discover_agents()` 80–120 / `LocalAgentScanner` ~150 / 前端 200–300）<br>🆕 **vFINAL.5 已落地「请神收尾」T0–T5**：ACP 客户端泛型化（`AcpAgentTransportBase` + `vermes_cli/a2a/recipes/` 食谱目录 + `POST /api/agents/register-profile` 登堂端点 + 前端「登堂」按钮），**任意 ACP 兼容 agent 现可登堂**；实测 65 绿。详见 §13 vFINAL.5 与 `outputs/vermes-14-acp-generic-client-sprint_20260905.md`<br>🆕 **vFINAL.7 已落地「造神 / 神魔堂」整轮 + hardening 三件套**（9 commit `+1464/-256`，2026-09-05 晚→09-06）：`b5cdccec06` **per-agent 专属 API key**（`agent_profiles.api_key` 列，`__KEEP__` 哨兵保持原 key）+ 原生 agent CRUD 端点（`GET/POST /api/agents/native`，**api_key 不回传只返 `has_api_key` 布尔**）；`0572856075`/`00ea357bed` **微信式联系人+群聊模型**（建群自由命名/手动拉人/群公告/群任务/踢人/9 专家预设，融合群聊+智能体市场为单一入口）；`745b59c6ed`/`f8eae8eb4b`/`b0a1b10f7e` **造神三连**（厂商模型下拉勾选、技能集勾选+运行时按角色裁剪工具、按角色语义推荐 SKILL 文档）；`438606cade` 群聊专家记忆按角色 scope 隔离（boost 非硬隔离，共享底座保留）。**hardening（`a61f353869` + `12bcfd4a70`）**：授权 Key 持久化（`~/.vermes/agent_auth.json`，0600 + 原子写；**P0 键冲突已修**——凭据库 key 用 `recipe.name` 而非 `fallback_settings`，后者是 settings.json 路径字面量致多 recipe 互相覆盖）+ 健康检查升级为**真 ACP `initialize` 握手**（spawn + 协议协商 + 进程清理，默认 30s，失败降级仍注册）+ **dispatch 接线**（新增 `agent/a2a/transports_acp.py`，`a2a_agents` 表加 `recipe` 列三处一致改）→ **「注册成功 ≠ 能对话」的断层已补**。**职责划分**：原生 agent（transport=native）走 `agent_profiles.api_key` 列，外部 ACP agent 仍走 `~/.vermes/agent_auth.json` 凭据库（`vermes_state.py:27` 注释显式引用），**未重复造轮子**。⚠️ **审计问题**：后端新增 750 行仅 55 行测试改动（造神 CRUD / 技能裁剪 / 记忆 scope / 群聊踢人·群任务**无直接测试**，commit 称"验证通过"疑为手动验证未落成测试）；前端未经运行时验证（`dc7e28f474` 所修 toast 导入错误本应在提交前暴露，**build 绿 ≠ 前端正确**再命中）。**剩余待办**：binary recipe 需先下载解包（16 条 cursor/kimi/junie 等，见 §13）；⑨ 鉴权统一收紧留隐私硬化一并做。详见 §13 vFINAL.7 |
