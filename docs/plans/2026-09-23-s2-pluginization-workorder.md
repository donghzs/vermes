...
| S2.3 | 迁剩余 14 键；`editing_guardrails` 补 YAML | 每个键单独一次比对 | ✅ 本切片（4 键 YAML/常量漂移已钉扎，见 S2.3 报告） |
| S2.4 | 最后才删 `_PROCESSOR_FALLBACK` 硬编码回退 | 全场景绿 + cache 哨兵绿 | ✅ `feat/s24-retire-fallback`（4 键回写 + map 退役 + computer_use 惰性保留；见 `reports/qclaw/s24-retire-fallback_20260923.md`） |
