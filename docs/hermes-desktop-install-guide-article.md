...
> 2026 年 5 月 29 日，Nous Research 正式发布 Hermes Agent v0.15.0——这是首个打包桌面安装程序的版本。本文基于官方文档与实测，完整梳理 Hermes Desktop 的安装流程，并带出一个你可能更需要的选择。
...
Hermes Agent 是由 Nous Research（知名 AI 研究团队，以 Hermes 系列开源模型闻名）构建的一款**自主 AI Agent**。它不是 IDE 里的代码补全插件，也不是 ChatGPT 那样的聊天窗口——它是一个可以独立运行、能在服务器上持续工作的 Agent。
...
## 二、Hermes Desktop 安装指南

v0.15.0 是首发桌面版本的里程碑。此前 Hermes 只有 CLI 和 TUI 两种交互方式，现在 macOS 和 Windows 用户可以通过原生桌面应用使用。
...
1. 打开 [hermes-agent.nousresearch.com/desktop](https://hermes-agent.nousresearch.com/desktop)
...
5. 安装完成后打开终端，运行 `hermes setup --portal` 通过 Nous Portal OAuth 一键配置模型和工具
...
这个安装器会自动处理：uv、Python 3.11、Node.js 22、ripgrep、ffmpeg、虚拟环境创建、`hermes` 命令注册。

安装后运行 `source ~/.bashrc`，然后：

```bash
hermes setup           # 选择模型和工具
hermes                 # 开始聊天
```
...
安装器会下载 PortableGit（自包含 Git 发行版，约 50 MB），完全不依赖系统 Git。数据目录在 `%LOCALAPPDATA%\hermes`。
...
Vermes 当前版本 v2.0.7，基于 Hermes Agent 引擎深度定制。完全开源，MIT 许可证。

---

## 四、客观对比：Hermes vs Vermes

先声明：Vermes 基于 Hermes，两者是**继承关系**而非竞争关系。以下对比是为了帮你根据自身需求做选择。

### 4.1 什么时候选 Hermes

- **你需要在服务器上部署 Agent**：Hermes 的沙箱执行、Gateway 多平台、Cron 定时任务、Kanban 工作流，构成了一个完整的服务端 Agent 基础设施。
- **你需要 Telegram/Discord/Slack 集成**：Hermes 的 Gateway 是目前最完善的多平台 Agent 网关，没有之一。
- **你需要 MCP 集成**：Hermes 原生支持 MCP（Model Context Protocol），可以接入 MCP 生态系统。
- **你是海外用户或服务器在海外**：网络不是问题，CLI 安装器 2 分钟搞定。
- **你的团队需要共享 Agent 服务**：Hermes 的系统服务模式、多用户共用安装更适合团队场景。
...
Hermes v0.15.0 桌面版的发布是一个重要的里程碑——它标志着 AI Agent 从「只有开发者能用的命令行工具」走向了「普通用户也能打开安装的桌面应用」。Nous Research 做了一件了不起的事。

Vermes 站在巨人的肩膀上，把 Hermes 的能力带到中文用户的桌面上。你可以把它理解为「Hermes 的汉化开箱版」——这没什么可遮掩的，做得好用比做得大更重要。

如果你一直在关注 AI Agent 但被安装门槛劝退，现在有两个选择了：想要最完整的能力，装 Hermes；想要最简单的启动，试试 Vermes。

无论选哪个，你得到的都是一个能自我进化的 AI Agent——这才是最令人兴奋的部分。

---

*本文所有信息基于 2026 年 6 月 3 日的最新版本（Hermes v0.15.2 / Vermes v0.2.0.7）。功能差异可能随版本更新而变化。*
