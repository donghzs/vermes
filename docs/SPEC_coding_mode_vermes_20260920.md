...
> 本规格按「coding 姿态 + cwd/平台判定」起草，已被「全世界皆 code + 记忆底座」终局取代。  
> **真源**：`reports/qclaw/p2-coding-context-qclaw-终局立场_20260920.md`  
> 结论摘要：P2 591 行**永久不做**；A′=安全补丁；M7 技能降级触发建议改为 **prompt/token 长度**（非 coding 场景），待董董拍板。  
> 本文件仅作历史参考。
