"""
Session-scoped context variables for the Vermes gateway.

Replaces the previous ``os.environ``-based session state
(``VERMES_SESSION_PLATFORM``, ``VERMES_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["VERMES_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("VERMES_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("VERMES_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("VERMES_SESSION_PLATFORM", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("VERMES_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("VERMES_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("VERMES_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("VERMES_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("VERMES_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("VERMES_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("VERMES_SESSION_ID", default=_UNSET)
...
_VAR_MAP = {
    ...
    "VERMES_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "VERMES_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "VERMES_SESSION_USER_ID": _SESSION_USER_ID,
    "VERMES_SESSION_USER_NAME": _SESSION_USER_NAME,
    "VERMES_SESSION_KEY": _SESSION_KEY,
    "VERMES_SESSION_ID": _SESSION_ID,
    "VERMES_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``VERMES_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("VERMES_SESSION_*", default)``.
