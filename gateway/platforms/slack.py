...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_vermes_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /Vermes prefix). A single regex
            ...
            # manifest (see `Vermes slack manifest`). In Socket Mode, Slack
            ...
            from vermes_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/Vermes$")

            @self._app.command(_slash_pattern)
            async def handle_vermes_command(ack, command):
            ...
            for _action_id in (
                "VERMES_approve_once",
                "VERMES_approve_session",
                "VERMES_approve_always",
                "VERMES_deny",
            ):
            ...
            for _action_id in (
                "VERMES_confirm_once",
                "VERMES_confirm_always",
                "VERMES_confirm_cancel",
            ):
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                from vermes_cli.commands import is_gateway_known_command
                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@Vermes`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "VERMES_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "VERMES_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "VERMES_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "VERMES_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "VERMES_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "VERMES_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "VERMES_confirm_once": "once",
            "VERMES_confirm_always": "always",
            "VERMES_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "VERMES_approve_once": "once",
            "VERMES_approve_session": "session",
            "VERMES_approve_always": "always",
            "VERMES_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/Vermes <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/Vermes
        ...
        if slash_name in {"vermes", "Vermes", ""}:
            # Legacy /Vermes <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from vermes_cli.commands import slack_subcommand_map
            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/Vermes   ``).
        ...
        # "Running /cmd…" ack shown by handle_vermes_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/Vermes <question>" must produce public replies so
