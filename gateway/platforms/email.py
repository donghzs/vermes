"""
Email platform adapter for the Vermes gateway.

Allows users to interact with Vermes by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "Vermes-agent" "version" "{_vermes_version}" '
            '"vendor" "donghzs" '
            '"support-email" "noreply@donghzs.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        logger.info(f"[Email] Connected as {self._address}")
        return True
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Vermes")
        ...
        msg_id = f"<Vermes-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
