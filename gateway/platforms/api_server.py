...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Vermes-Session-Id header; opt-in long-term memory scoping via X-Vermes-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Vermes-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists vermes-agent as an available model
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to vermes-agent
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_vermes_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        try:
        ...
        # gracefully on NFS/SMB/FUSE-mounted VERMES_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # vermes_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same Vermes session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through vermes-agent's AIAgent.
    """
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Vermes-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Vermes-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Vermes-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Vermes-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Vermes-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``Vermes sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the Vermes-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Vermes-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "Vermes-agent"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return vermes-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Vermes version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Vermes AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Vermes-Session-Id",
                "session_key_header": "X-Vermes-Session-Key",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Vermes-Session-Key.  This
        # is independent of X-Vermes-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Vermes-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Vermes-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Vermes-Session-Id rejected: "
            ...
            # conversation map to the same Vermes session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``Vermes.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Vermes-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Vermes-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["Vermes"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-Vermes-Completed"] = "false"
            response_headers["X-Vermes-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Vermes-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Vermes-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Vermes-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Vermes-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Vermes-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: Vermes.tool.progress`` SSE event so
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Vermes-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Vermes-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    _UPDATE_ALLOWED_FIELDS = {"name", "schedule", "prompt", "deliver", "skills", "skill", "repeat", "enabled", "monitor_mode", "monitor_target"}
    ...
    async def _handle_create_job(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            monitor_target = body.get("monitor_target")
            ...
            if monitor_mode:
                kwargs["monitor_mode"] = bool(monitor_mode)
            if monitor_target:
                kwargs["monitor_target"] = monitor_target
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            # (e.g. X-Vermes-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Vermes-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "Vermes.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # the June 2026 Vermes-0day hardening (an 8-char key dispatching
            # terminal-capable agent work on a public bind is brute-forceable).
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from vermes_cli.auth import has_usable_secret
            ...
            # that is the exact surface the Vermes-0day campaign abused to write
            # ~/.vermes/config.yaml and plant persistence. Sandboxing (Docker /
            ...
            if is_network_accessible(self._host):
                try:
                    from vermes_cli.config import load_config as _load_cfg
                    _backend = (
