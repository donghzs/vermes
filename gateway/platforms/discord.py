...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.vermes/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, behavior: Optional[DiscordBehaviorConfig] = None):
        ...
        self._text_batch_delay_seconds = float(os.getenv("VERMES_DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("VERMES_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_vermes_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Vermes manages."""
    ...
    async def delete_message(self, chat_id: str, message_id: str) -> bool:
        ...
        try:
            ...
            logger.warning("Discord: failed to delete message %s in %s", message_id, chat_id)
            return False
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Vermes session")
        ...
        @tree.command(name="status", description="Show Vermes session status")
        ...
        @tree.command(name="stop", description="Stop the running Vermes agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.vermes/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Vermes to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Vermes gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Vermes session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Vermes in the thread",
        ...
        # vermes_cli/commands.py automatically appear as Discord slash
        ...
        try:
            from vermes_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from vermes_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Vermes skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from vermes_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Vermes session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Vermes: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Vermes"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Vermes: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Vermes handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``vermes update --gateway``
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``vermes update`` prompts.
