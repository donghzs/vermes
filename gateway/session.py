...
# ``sessions_dir / f"{session_id}.json"`` in vermes_state, request-dump
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `vermes tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `vermes tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_vermes_home()}/cron/output/)"
    )
...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        try:
            ...
            logger.info(f"[gateway] Warning: SQLite session store unavailable, falling back to JSONL: {e}")
    ...
    def _ensure_loaded_locked(self) -> None:
        ...
        if sessions_file.exists():
            try:
                ...
                logger.info(f"[gateway] Warning: Failed to load sessions: {e}")
    ...
    def get_or_create_session(
        ...
        if self._db and db_create_kwargs:
            try:
                ...
                logger.info(f"[gateway] Warning: Failed to create SQLite session: {e}")
