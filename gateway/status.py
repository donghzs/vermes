...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting VERMES_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("VERMES_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "Vermes" / _LOCKS_DIRNAME
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different VERMES_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``Vermes.service`` + ``Vermes-
# gateway.service``).
#
