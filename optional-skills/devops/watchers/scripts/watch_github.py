...
Usage (via cron with --no-agent):
    ...
    Vermes cron create Vermes-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$VERMES_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name Vermes-issues --repo donghzs/vermes --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.vermes/.env to avoid the 60 req/hr
