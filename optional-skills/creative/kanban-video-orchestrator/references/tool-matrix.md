...
Maps each role archetype to the Vermes skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public vermes-agent
...
## Vermes skills relevant to video production

### Visual / rendering skills (`vermes-agent/skills/creative/`)
...
### Design / pre-production skills (`vermes-agent/skills/creative/`)
...
### Audio / media skills (`vermes-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`vermes-agent/skills/devops/`)
...
These are **not** Vermes skills but external CLIs / APIs that profiles invoke.
...
## Built-in Vermes tools for media review

These are native Vermes tools — not invoked via terminal but through their own
...
key is present in `~/.vermes/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Vermes profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.vermes/.env` → environment variable.
...
When a new Vermes-public video skill ships:
