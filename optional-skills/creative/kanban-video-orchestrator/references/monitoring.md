...
vermes kanban watch --tenant <project-slug>

# Snapshot of the board
vermes kanban list --tenant <project-slug>
vermes kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
vermes kanban stats --tenant <project-slug>

# Visual dashboard (browser)
vermes dashboard

# Inspect a specific task (includes comments + events)
vermes kanban show <task-id>

# Follow a single task's event stream
vermes kanban tail <task-id>
```

Verify available subcommands with `vermes kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `vermes kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `vermes kanban block <id>` then `vermes kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `vermes kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `vermes kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `vermes profile list` (profile exists?), `vermes status` (gateway/dispatcher up?), and verify tenant. |
...
vermes kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
vermes kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(vermes kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
vermes kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
vermes kanban block <task-id>
vermes kanban archive <task-id>

# Diagnose what happened
vermes kanban show <task-id>      # task body, comments, recent events
vermes kanban tail <task-id>      # follow the live event stream
vermes kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `vermes kanban create` for the director
...
while true; do
    clear
    vermes kanban list --tenant <slug>
    echo "---"
    vermes kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `vermes kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `vermes profile list` and start
