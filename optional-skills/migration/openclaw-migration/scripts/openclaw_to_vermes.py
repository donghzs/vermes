...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'Vermes cron'.",
    },
    "hooks-config": {
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-Vermes/apply.ts "blocked by earlier
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-Vermes/plan.ts warnings[].
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
        ...
        for oc_key, VERMES_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and VERMES_key not in secret_additions:
                secret_additions[VERMES_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, VERMES_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and VERMES_key not in secret_additions:
                        secret_additions[VERMES_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        VERMES_config = load_yaml_file(destination)
        current_model = VERMES_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = VERMES_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                VERMES_config["model"] = {"default": model_str}
            dump_yaml_file(destination, VERMES_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        VERMES_config = load_yaml_file(destination)
        existing_tts = VERMES_config.get("tts", {})
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        VERMES_cfg_path = self.target_root / "config.yaml"
        VERMES_cfg = load_yaml_file(VERMES_cfg_path)
        existing_mcp = VERMES_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            VERMES_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                VERMES_srv["command"] = srv["command"]
                if srv.get("args"):
                    VERMES_srv["args"] = srv["args"]
                if srv.get("env"):
                    VERMES_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    VERMES_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                VERMES_srv["url"] = srv["url"]
                if srv.get("headers"):
                    VERMES_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    VERMES_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                VERMES_srv["enabled"] = False
            if srv.get("timeout"):
                VERMES_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                VERMES_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                VERMES_srv["tools"] = {}
                if tools_cfg.get("include"):
                    VERMES_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    VERMES_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(VERMES_cfg_path)
            VERMES_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(VERMES_cfg_path, VERMES_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'Vermes cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = VERMES_cfg.get("agent") or {}
        ...
        if compaction:
            compression = VERMES_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("VERMES_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        VERMES_cfg_path = self.target_root / "config.yaml"
        VERMES_cfg = load_yaml_file(VERMES_cfg_path)
        sr = VERMES_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        VERMES_cfg_path = self.target_root / "config.yaml"
        VERMES_cfg = load_yaml_file(VERMES_cfg_path)
        custom_providers = VERMES_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(VERMES_cfg_path)
            VERMES_cfg["custom_providers"] = custom_providers
            dump_yaml_file(VERMES_cfg_path, VERMES_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            VERMES_cfg_path = self.target_root / "config.yaml"
            VERMES_cfg = load_yaml_file(VERMES_cfg_path)
            discord_vermes = VERMES_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_vermes["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_vermes["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                VERMES_cfg["discord"] = discord_vermes
                dump_yaml_file(VERMES_cfg_path, VERMES_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        VERMES_cfg_path = self.target_root / "config.yaml"
        VERMES_cfg = load_yaml_file(VERMES_cfg_path)
        browser_vermes = VERMES_cfg.get("browser") or {}
        changed = False

        # Map fields that have Vermes equivalents
        if browser.get("cdpUrl"):
            browser_vermes["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_vermes["headless"] = browser["headless"]
            changed = True

        if changed:
            VERMES_cfg["browser"] = browser_vermes
            if self.execute:
                self.maybe_backup(VERMES_cfg_path)
                dump_yaml_file(VERMES_cfg_path, VERMES_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            VERMES_mode = mode_map.get(mode, "manual")
            VERMES_cfg.setdefault("approvals", {})["mode"] = VERMES_mode
            if self.execute:
                self.maybe_backup(VERMES_cfg_path)
                dump_yaml_file(VERMES_cfg_path, VERMES_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{VERMES_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        ...
        notes.extend([
            ...
            "",
            "**Strongly recommended:** Run `Vermes claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `Vermes claw cleanup` to fix it.",
            "",
            "## Vermes-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `Vermes claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `vermes setup` to configure any remaining settings",
            "- Run `Vermes mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `Vermes cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `Vermes cron` to recreate scheduled tasks (see archived cron-store)")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", default=os.environ.get("VERMES_HOME") or str(Path.home() / ".vermes"), help="Vermes home directory")
    parser.add_argument(
        "--workspace-target",
...
def main() -> int:
    ...
    if args.execute:
        ...
        logger.info("    2. Run: Vermes mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            logger.info("    3. Recreate cron jobs: Vermes cron")
