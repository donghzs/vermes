...
- **Note:** The previous Open Refine reconciliation endpoint at
  ...
  the ZIP locally (default `~/.cache/vermes-osint/icij/`, refreshes after
...
First call downloads the ~70 MB ZIP under `~/.cache/vermes-osint/icij/`
(or `$VERMES_OSINT_CACHE/icij/`). Subsequent calls reuse the cache for 30 days.
