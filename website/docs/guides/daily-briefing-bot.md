...
2. **Vermes spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.vermes/cron/output/` and you can read them anytime.
:::
...
Vermes will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Vermes Agent
```
...
Before creating cron jobs, ensure Vermes has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Vermes what you want:
...
Vermes will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Vermes to delegate each topic to a sub-agent:
...
Vermes will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
