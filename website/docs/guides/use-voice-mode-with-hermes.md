---
sidebar_position: 8
title: "Use Voice Mode with Vermes"
description: "A practical guide to setting up and using Vermes voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Vermes
...
- you want Vermes sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Vermes.
...
## Step 1: make sure normal Vermes works first
...
Vermes supports both local and cloud speech stacks.
...
### If you use `vermes setup`

If you choose NeuTTS in the setup wizard, Vermes checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Vermes transcribes and responds
...
- asking Vermes to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Vermes loop.

## Tuning CLI behavior

### Silence threshold

If Vermes starts/stops too aggressively, tune:
...
Vermes stays a normal chat bot, but can speak replies.
...
- you want Vermes to function like a portable research or ops assistant
...
Vermes joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Vermes detects speech boundaries
- transcripts are posted in the associated text channel
- Vermes responds in text and audio
...
1. get text Vermes working
2. install `vermes-agent[voice]`
