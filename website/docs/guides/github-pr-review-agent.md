...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Vermes Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Vermes instantly when PRs are opened or updated.
:::
...
- **Vermes Agent installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  vermes gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.vermes/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Vermes can access GitHub. Start a chat:
...
```
Run: gh pr list --repo donghzs/vermes --state open --limit 3
```
...
Still in the chat, ask Vermes to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo donghzs/vermes
```
...
A skill gives Vermes consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.vermes/skills/code-review
```

Create `~/.vermes/skills/code-review/SKILL.md`:
...
Verify it loaded — start `vermes` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Vermes your team's standards:
...
2. Teach Vermes your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
vermes gateway status    # Is the gateway running?
vermes cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/docs/guides/build-a-vermes-plugin)** — wrap the review logic into a shareable plugin
