...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Vermes Agent — no API key required"
---

# MiniMax OAuth

Vermes Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Vermes automatically refreshes your session.
...
# → Vermes opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.vermes/auth.json` and are refreshed automatically before each session.
...
```bash
vermes auth add minimax-cn --type oauth   # if OAuth is supported on your CN account
# or simpler:
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.vermes/.env
```
...
```bash
vermes auth add minimax-oauth --no-browser
```

Vermes will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Vermes implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. Vermes generates a PKCE verifier / challenge pair and a random state value.
...
4. Vermes polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.vermes/auth.json` under the `minimax-oauth` key.
...
```bash
vermes config set model MiniMax-M2.7
vermes config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.vermes/config.yaml` will contain entries similar to:
...
```bash
vermes --provider minimax-oauth    # canonical
vermes --provider minimax-portal   # alias
vermes --provider minimax-global   # alias
vermes --provider minimax_oauth    # alias (underscore form)
```
...
```bash
VERMES_INFERENCE_PROVIDER=minimax-oauth vermes
```
...
Vermes refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Vermes marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Vermes marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `vermes auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Vermes raises a timeout error.

**Fix:** re-run `vermes auth add minimax-oauth` (or `vermes model`). The flow starts fresh.

### State mismatch (possible CSRF)

Vermes detected that the `state` value returned by the authorization server does not match what it sent.
...
If `vermes` cannot open a browser window, use `--no-browser`:

```bash
vermes auth add minimax-oauth --no-browser
```

Vermes prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `vermes model` and select MiniMax (OAuth), or run `vermes auth add minimax-oauth`.
...
```bash
vermes auth remove minimax-oauth
```
...
- [vermes doctor](../reference/cli-commands.md)
