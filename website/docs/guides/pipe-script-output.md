...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `vermes send`."
---

# Pipe Script Output to Messaging Platforms

`vermes send` is a small, scriptable CLI that pushes a message to any
messaging platform Vermes is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | vermes send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `vermes
...
vermes send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
vermes send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
vermes send --list

# Filter by platform
vermes send --list telegram
```
...
Any platform Vermes ships adapters for works as a target:
...
`vermes send` resolves the message body in this order:

1. **Positional argument** — `vermes send --to telegram "hi"`
2. **`--file PATH`** — `vermes send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | vermes send --to telegram`

When stdin is a TTY (no pipe), Vermes does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  vermes send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `vermes send` reuses your Vermes config, the same script works on
any host where Vermes is installed — no need to export bot tokens into
...
`vermes send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  vermes send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | vermes send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.vermes/bin/vermes send \
...
```bash
./train.py --epochs 200 && \
  vermes send --to telegram "training done" || \
  vermes send --to telegram "training failed (exit $?)"
```
...
vermes send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(vermes send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `vermes send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `vermes send` calls
the platform's REST endpoint directly using credentials from
`~/.vermes/.env` and `~/.vermes/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `vermes gateway start` and retry.
...
# Machine-readable
vermes send --list --json
```

The listing is built from `~/.vermes/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`vermes gateway
...
| Approach | Multi-platform | Reuses Vermes creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `vermes send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`vermes send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `vermes send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `vermes send` shares with cron delivery.
