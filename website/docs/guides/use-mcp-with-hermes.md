---
sidebar_position: 6
title: "Use MCP with Vermes"
description: "A practical guide to connecting MCP servers to Vermes Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Vermes

This guide shows how to actually use MCP with Vermes Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Vermes tool
- you want Vermes to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Vermes to internal APIs, databases, or company systems without modifying Vermes core

Do not use MCP when:
- a built-in Vermes tool already solves the job well
...
- Vermes remains the agent
- MCP servers contribute tools
- Vermes discovers those tools at startup or reload time
...
If you installed Vermes with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Vermes banner/status should show MCP integration when configured
- ask Vermes what tools it has available
...
## WSL2: bridge Vermes in WSL to Windows Chrome

This is the practical setup when:

- Vermes runs inside WSL2
...
In this setup, Vermes does **not** connect to Chrome directly. Instead:

- Vermes runs in WSL
- Vermes starts a local stdio MCP server
...
```text
Vermes (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Vermes stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Vermes core browser transport
...
```bash
vermes mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
vermes mcp test chrome-devtools-win
```

Then start a fresh Vermes session or run:
...
Once loaded, Vermes can use the MCP-prefixed browser tools directly. For example:
...
If Vermes runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Vermes from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Vermes from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Vermes:
...
2. Vermes-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Vermes will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Vermes to reason over a bounded workspace.
...
Now Vermes can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Vermes core.
...
Because Vermes now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
