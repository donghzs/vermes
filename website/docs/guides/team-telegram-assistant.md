...
This tutorial walks you through setting up a Telegram bot powered by Vermes Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Vermes Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.vermes/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Vermes itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Vermes Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_vermes_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Vermes Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Vermes Gateway...
...
# View live logs
journalctl --user -u vermes-gateway -f
...
vermes gateway start
vermes gateway stop
tail -f ~/.vermes/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `vermes gateway install` to update the plist.
:::
...
```bash
vermes gateway stop && vermes gateway start
```
...
vermes pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.vermes/.env`:
...
Control how much detail the bot shows when using tools. In `~/.vermes/config.yaml`:
...
Customize how the bot communicates by editing `~/.vermes/SOUL.md`:

For a full guide, see [Use SOUL.md with Vermes](/docs/guides/use-soul-with-vermes).
...
```bash
# From the CLI
vermes cron list          # View all scheduled jobs
vermes cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u vermes-gateway` (Linux) or `~/.vermes/logs/gateway.log` (macOS) |
| Cron job output | `~/.vermes/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.vermes/cron/jobs.json` |
| Pairing data | `~/.vermes/pairing/` |
| Session history | `~/.vermes/sessions/` |
