---
sidebar_position: 7
title: "Use SOUL.md with Vermes"
description: "How to use SOUL.md to shape Vermes Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Vermes

`SOUL.md` is the **primary identity** for your Vermes instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Vermes to feel like the same assistant every time you talk to it — or if you want to replace the Vermes persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Vermes should be
- what Vermes should avoid stylistically
- how Vermes should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Vermes is and how Vermes speaks
...
Vermes now uses only the global SOUL file for the current instance:

```text
~/.vermes/SOUL.md
```

If you run Vermes with a custom home directory, it becomes:
...
Vermes automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Vermes does not overwrite it
- if the file exists but is empty, Vermes adds nothing from it to the prompt

## How Vermes uses it

When Vermes starts a session, it reads `SOUL.md` from `VERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Vermes feels.
...
Vermes already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Vermes should sound.

# Avoid
What Vermes should not do.

# Defaults
How Vermes should behave when ambiguity appears.
```
...
Then restart Vermes or start a new session.
...
### I edited SOUL.md but Vermes still sounds the same

Check:
- you edited `~/.vermes/SOUL.md` or `$VERMES_HOME/SOUL.md`
...
### Vermes is ignoring parts of my SOUL.md
