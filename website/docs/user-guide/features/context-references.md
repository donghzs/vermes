...
Type `@` followed by a reference to inject content directly into your message. Vermes expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Vermes env: `$VERMES_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$VERMES_HOME/skills/.hub/`
