...
If the file is empty, whitespace-only, or cannot be read, Vermes falls back to a built-in default identity ("You are Vermes Agent, an intelligent AI assistant created by Vermes..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
