...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Vermes's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Vermes instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
vermes memory setup    # select "honcho" from the provider list
```
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.vermes/.env
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$VERMES_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `vermes` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Vermes syncs them back at session init.
...
The `vermes honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `vermes memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
vermes honcho status          # Connection status, config, and key settings
vermes honcho setup           # Redirects to `vermes memory setup`
vermes honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
vermes honcho peer            # Show or update peer names + dialectic reasoning level
vermes honcho mode            # Show or set recall mode (hybrid/context/tools)
vermes honcho tokens          # Show or set token budget for context and dialectic
vermes honcho identity        # Seed or show the AI peer's Honcho identity
vermes honcho sync            # Sync Honcho config to all existing profiles
vermes honcho peers           # Show peer identities across all profiles
vermes honcho sessions        # List known Honcho session mappings
vermes honcho map             # Map current directory to a Honcho session name
vermes honcho enable          # Enable Honcho for the active profile
vermes honcho disable         # Disable Honcho for the active profile
vermes honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `vermes honcho`

If you previously used the standalone `vermes honcho setup`:
...
No re-login or re-setup needed. Run `vermes memory setup` and select "honcho" — the wizard detects your existing config.
