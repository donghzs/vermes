...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Vermes to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.vermes/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
vermes chat --toolsets skills -q "What skills do you have?"
vermes chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.vermes/cache/chart-q4-2025.png
...
When a missing value is encountered, Vermes asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `vermes setup` or `~/.vermes/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `vermes config migrate` prompts for unconfigured settings, and `vermes config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.vermes/skills/                  # Single source of truth
...
If you maintain skills outside of Vermes — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Vermes to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.vermes/config.yaml`:
...
- **Read-only**: External dirs are only scanned for skill discovery. When the agent creates or edits a skill, it always writes to `~/.vermes/skills/`.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Vermes ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.vermes/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.vermes/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `vermes bundles list`.
...
vermes bundles create research

# Overwrite an existing bundle
vermes bundles create backend-dev --skill ... --force

# Delete a bundle
vermes bundles delete backend-dev

# Re-scan ~/.vermes/skill-bundles/ and report changes
vermes bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.vermes/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.vermes/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
vermes skills browse                              # Browse all hub skills (official first)
vermes skills browse --source official            # Browse only official optional skills
vermes skills search kubernetes                   # Search all sources
vermes skills search react --source skills-sh     # Search the skills.sh directory
vermes skills search https://mintlify.com/docs --source well-known
vermes skills inspect openai/skills/k8s           # Preview before installing
vermes skills install openai/skills/k8s           # Install with security scan
vermes skills install official/security/1password
vermes skills install skills-sh/vercel-labs/json-render/json-render-react --force
vermes skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
vermes skills list --source hub                   # List hub-installed skills
vermes skills check                               # Check installed hub skills for upstream updates
vermes skills update                              # Reinstall hub skills with upstream changes when needed
vermes skills audit                               # Re-scan all hub skills for security
vermes skills uninstall k8s                       # Remove a hub skill
vermes skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
vermes skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
vermes skills publish skills/my-skill --to github --repo owner/repo
vermes skills snapshot export setup.json          # Export skill config
vermes skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Vermes. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `vermes skills search <query> --source skills-sh`. Vermes resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Vermes currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Vermes repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Vermes can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
vermes skills search https://mintlify.com/docs --source well-known
vermes skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
vermes skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Vermes can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
vermes skills install openai/skills/k8s
vermes skills tap add myorg/skills-repo
```
...
- Vermes source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Vermes supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Vermes source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Vermes can search and convert agent entries from LobeHub's public catalog into installable Vermes skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Vermes source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Vermes integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Vermes' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Vermes source id: `browse-sh`
- Trust level: `community`

```bash
vermes skills search airbnb --source browse-sh
vermes skills inspect browse-sh/airbnb.com/search-listings-ddgioa
vermes skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Vermes fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- Vermes source id: `url`
...
```bash
vermes skills install https://sharethis.chat/SKILL.md
vermes skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
vermes skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `vermes skills update` re-fetches from the same URL automatically when you want to refresh.
...
`vermes skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Vermes | Always trusted |
...
```bash
vermes skills check          # Report which installed hub skills changed upstream
vermes skills update         # Reinstall only the skills with updates available
vermes skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Vermes users add with `vermes skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.vermes.tags`, `version`, `author`, `platforms`, `metadata.vermes.config`).
...
Vermes discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/vermes-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Vermes user can subscribe and install:

```bash
vermes skills tap add my-org/vermes-skills
vermes skills search deploy
vermes skills install my-org/vermes-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.vermes/.hub/taps.json`:
...
The `vermes skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `vermes skills tap list` shows the effective path per tap.
...
```bash
vermes skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.vermes/.hub/taps.json` (created on demand).
...
vermes skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `VERMES_HOME`, so `vermes -p coder skills reset <name>` only affects that profile.
:::
