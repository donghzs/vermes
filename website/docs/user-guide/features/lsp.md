...
Vermes runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Vermes
...
1. Vermes captures a baseline of current diagnostics for the file.
...
…). Vermes auto-detects the binary on PATH or in
`<VERMES_HOME>/lsp/bin/`.
...
`node_modules` tree — Vermes installs both packages together when you
run `vermes lsp install typescript` or auto-install fires on first
use.

## CLI

```
vermes lsp status          # service state + per-server install status
vermes lsp list            # registry, optionally --installed-only
vermes lsp install <id>    # eagerly install one server
vermes lsp install-all     # try every server with a known recipe
vermes lsp restart         # tear down running clients
vermes lsp which <id>      # print resolved binary path
```

`vermes lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <VERMES_HOME>/lsp/bin
...
When `install_strategy: auto`, Vermes installs binaries into
`<VERMES_HOME>/lsp/bin/`. NPM packages land in
`<VERMES_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Vermes-owned and is
...
Servers are kept alive for the life of the Vermes process. There's
...
**`vermes lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<VERMES_HOME>/lsp/bin/`. Run
`vermes lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `vermes lsp status`**
...
When `vermes lsp status` shows a `Backend warnings` section, install
...
`~/.vermes/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.vermes/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `vermes lsp restart` to clear the set;
