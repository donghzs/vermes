...
Connect Vermes to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Vermes supports two WeCom integration modes:
...
3. Vermes decrypts the message, queues it for the agent
...
(Use `vermes gateway start` only after `vermes gateway install` has registered the systemd/launchd service.)
