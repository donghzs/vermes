...
description: "Chat with Vermes from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Vermes from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/docs/user-guide/features/voice-mode) and [Use Voice Mode with Vermes](/docs/guides/use-voice-mode-with-vermes).
...
```mermaid
flowchart TB
    subgraph Gateway["Vermes Gateway"]
...
```bash
vermes gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
vermes gateway              # Run in foreground
vermes gateway setup        # Configure messaging platforms interactively
vermes gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo vermes gateway install --system   # Linux only: install a boot-time system service
vermes gateway start        # Start the default service
vermes gateway stop         # Stop the default service
vermes gateway status       # Check default service status
vermes gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Vermes Agent to the latest version |
...
Configure per-platform overrides in `~/.vermes/gateway.json`:
...
vermes pairing approve telegram XKGH5N7P

# Other pairing commands:
vermes pairing list          # View pending + approved users
vermes pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Vermes appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.vermes/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.vermes/config.yaml`:
...
```bash
VERMES_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
vermes gateway install               # Install as user service
vermes gateway start                 # Start the service
vermes gateway stop                  # Stop the service
vermes gateway status                # Check status
journalctl --user -u vermes-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo vermes gateway install --system
sudo vermes gateway start --system
sudo vermes gateway status --system
journalctl -u vermes-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Vermes will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Vermes installations on the same machine (with different `VERMES_HOME` directories), each gets its own systemd service name. The default `~/.vermes` uses `vermes-gateway`; other installations use `vermes-gateway-<hash>`. The `vermes gateway` commands automatically target the correct service for your current `VERMES_HOME`.
:::

### macOS (launchd)

```bash
vermes gateway install               # Install as launchd agent
vermes gateway start                 # Start the service
vermes gateway stop                  # Stop the service
vermes gateway status                # Check status
tail -f ~/.vermes/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.vermes.gateway.plist`. It includes three environment variables:
...
- **VERMES_HOME** — scopes the gateway to your Vermes installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `vermes gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `VERMES_HOME` directory gets its own launchd label. The default `~/.vermes` uses `ai.vermes.gateway`; other installations use `ai.vermes.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `vermes-cli` | Full access |
| Telegram | `vermes-telegram` | Full tools including terminal |
| Discord | `vermes-discord` | Full tools including terminal |
| WhatsApp | `vermes-whatsapp` | Full tools including terminal |
| Slack | `vermes-slack` | Full tools including terminal |
| Google Chat | `vermes-google_chat` | Full tools including terminal |
| Signal | `vermes-signal` | Full tools including terminal |
| SMS | `vermes-sms` | Full tools including terminal |
| Email | `vermes-email` | Full tools including terminal |
| Home Assistant | `vermes-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `vermes-mattermost` | Full tools including terminal |
| Matrix | `vermes-matrix` | Full tools including terminal |
| DingTalk | `vermes-dingtalk` | Full tools including terminal |
| Feishu/Lark | `vermes-feishu` | Full tools including terminal |
| WeCom | `vermes-wecom` | Full tools including terminal |
| WeCom Callback | `vermes-wecom-callback` | Full tools including terminal |
| Weixin | `vermes-weixin` | Full tools including terminal |
| BlueBubbles | `vermes-bluebubbles` | Full tools including terminal |
| QQBot | `vermes-qqbot` | Full tools including terminal |
| Yuanbao | `vermes-yuanbao` | Full tools including terminal |
| Microsoft Teams | `vermes-teams` | Full tools including terminal |
| API Server | `vermes-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `vermes-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.vermes/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
