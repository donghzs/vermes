...
Vermes Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.vermes/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.vermes/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
Vermes stores session history so it can resume conversations, but it does not
...
Vermes explicitly injects for that turn.
...
For example, if a user sends an image and asks Vermes to make a meme from it,
Vermes may inspect that image once with vision and run an image-processing
...
`vermes sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`vermes` or `vermes chat`) |
...
vermes -c "my project"   # → resumes "my project #3"
```
...
vermes --resume 20250305_091523_a1b2c3d4
vermes -r 20250305_091523_a1b2c3d4

# Resume by title
vermes --resume "refactoring auth"

# Or with the chat subcommand
vermes chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `vermes sessions list`.

### Conversation Recap on Resume

When you resume a session, Vermes displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Vermes session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.vermes/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `vermes -r "<title>"` from the shell) and pick up where the platform left off.
...
Vermes automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `vermes sessions list` or `vermes sessions browse`.
...
```bash
vermes sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Vermes creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`vermes -c "my project"`), it automatically picks the most recent session in the lineage.
...
Vermes provides a full set of session management commands via `vermes sessions`:
...
# Filter by platform
vermes sessions list --source telegram

# Show more sessions
vermes sessions list --limit 50
```
...
vermes sessions export backup.jsonl

# Export sessions from a specific platform
vermes sessions export telegram-history.jsonl --source telegram

# Export a single session
vermes sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
vermes sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
vermes sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
vermes sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
vermes sessions prune

# Custom age threshold
vermes sessions prune --older-than 30

# Only prune sessions from a specific platform
vermes sessions prune --source telegram --older-than 60

# Skip confirmation
vermes sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`vermes insights`](/docs/reference/cli-commands#vermes-insights).
...
When Vermes cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Vermes uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Vermes in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.vermes/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.vermes/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.vermes/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Vermes process in the same `VERMES_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.vermes/config.yaml`:
...
vermes sessions delete <session_id>
