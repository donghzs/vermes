...
title: "Git Worktrees"
description: "Run multiple Vermes agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Vermes Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Vermes so each session has a clean, isolated working directory.

## Why Use Worktrees with Vermes?

Vermes treats the **current working directory** as the project root:

- CLI: the directory where you run `vermes` or `vermes chat`
...
git worktree add ../repo-feature feature/vermes-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/vermes-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Vermes there:

```bash
cd ../repo-feature

# Start Vermes in the worktree
vermes
```
...
git worktree add ../repo-experiment-a feature/vermes-a
git worktree add ../repo-experiment-b feature/vermes-b
```
...
- Works on its own branch (`feature/vermes-a` vs `feature/vermes-b`).
...
- Vermes checkpoint data under `~/.vermes/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Vermes experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/vermes-checkpoints-docs`, `feature/vermes-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Vermes from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `vermes -w` (Automatic Worktree Mode)

Vermes has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `vermes/vermes-<hash>`).
...
```bash
vermes -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `vermes -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Vermes session its own clean checkout.
