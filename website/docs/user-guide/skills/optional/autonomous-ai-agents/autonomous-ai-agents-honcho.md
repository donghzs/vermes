...
description: "Configure and use Honcho memory with Vermes -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Vermes -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `vermes skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Vermes

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Vermes profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/vermes#running-honcho-locally-with-vermes

### Verify

```bash
vermes honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Vermes profile's AI peer
...
Honcho models conversations as interactions between **peers**. Vermes creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Vermes instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Vermes run |
| `global` | Single session across all directories |

Manual override: `vermes honcho map my-project-name`
...
Each Vermes profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block vermes.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `vermes.coder` host block in `honcho.json`
...
```bash
vermes honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Vermes when Honcho memory is active.
...
Config file: `$VERMES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `vermes honcho setup`. Ensure `memory.provider: honcho` is in `~/.vermes/config.yaml`.

### Memory not persisting across sessions
Check `vermes honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `vermes profile create <name> --clone`. For existing profiles: `vermes honcho sync`.
...
| Command | Description |
|---------|-------------|
| `vermes honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `vermes honcho status` | Show resolved config, connection test, peer info for active profile |
| `vermes honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `vermes honcho disable` | Disable Honcho for the active profile |
| `vermes honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `vermes honcho peers` | Show peer identities across all profiles |
| `vermes honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `vermes honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `vermes honcho sessions` | List known directory-to-session-name mappings |
| `vermes honcho map <name>` | Map current working directory to a Honcho session name |
| `vermes honcho identity` | Seed AI peer identity or show both peer representations |
| `vermes honcho sync` | Create host blocks for all Vermes profiles that don't have one yet |
| `vermes honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Vermes + Honcho |
| `vermes memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `vermes memory status` | Show active memory provider and config |
| `vermes memory off` | Disable external memory provider |
