...
| | |
|---|---|
| Source | Optional — install with `vermes skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Vermes Agent |
| License | MIT |
...
import upstream classes into Vermes itself.
...
```bash
mkdir -p ~/.vermes/cache/darwinian-evolver && cd ~/.vermes/cache/darwinian-evolver
...
```bash
cd ~/.vermes/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.vermes/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.vermes/skills/research/darwinian-evolver
DE_DIR=~/.vermes/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Vermes core.
   Custom driver scripts under `~/.vermes/skills/...` are user-side and fine.
