---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Vermes Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Vermes Agent"
---
...
Migrate a user's OpenClaw customization footprint into Vermes Agent. Imports Vermes-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `vermes skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Vermes Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Vermes`, `Memory`, `Persona`, `Import` |
| Related skills | [`vermes-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-vermes-agent) |
...
# OpenClaw -> Vermes Migration

Use this skill when a user wants to move their OpenClaw setup into Vermes Agent with minimal manual cleanup.
...
```bash
vermes claw migrate              # Full interactive migration
vermes claw migrate --dry-run    # Preview what would be migrated
vermes claw migrate --preset user-data   # Migrate without secrets
vermes claw migrate --overwrite  # Overwrite existing conflicts
vermes claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `vermes setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_vermes.py` to:

- import `SOUL.md` into the Vermes home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Vermes memory entries
- merge OpenClaw command approval patterns into Vermes `command_allowlist`
- migrate Vermes-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.vermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Vermes workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.vermes/tts/`
- archive non-secret docs that do not have a direct Vermes destination
...
- `scripts/openclaw_to_vermes.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.vermes/skills/migration/openclaw-migration/scripts/openclaw_to_vermes.py`

Do not guess a shorter path like `~/.vermes/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.vermes/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Vermes-compatible secrets, currently:
...
Vermes CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Vermes. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Vermes workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Vermes skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.vermes/skills/migration/openclaw-migration/scripts/openclaw_to_vermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.vermes/skills/migration/openclaw-migration/scripts/openclaw_to_vermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.vermes/skills/migration/openclaw-migration/scripts/openclaw_to_vermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.vermes/skills/migration/openclaw-migration/scripts/openclaw_to_vermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.vermes/skills/migration/openclaw-migration/scripts/openclaw_to_vermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.vermes/skills/migration/openclaw-migration/scripts/openclaw_to_vermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Vermes unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Vermes targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Vermes destination. Unsupported auth blobs must still be reported as skipped.
...
- Vermes persona state imported
- Vermes memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.vermes/skills/openclaw-imports/`
