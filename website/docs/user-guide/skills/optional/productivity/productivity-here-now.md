...
| | |
|---|---|
| Source | Optional — install with `vermes skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${VERMES_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${VERMES_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${VERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client vermes
```
...
```bash
PUBLISH="${VERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client vermes
```
...
```bash
DRIVE="${VERMES_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `vermes`)    |
