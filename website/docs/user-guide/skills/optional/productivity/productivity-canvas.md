...
| | |
|---|---|
| Source | Optional — install with `vermes skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Vermes Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.vermes/.env`:
...
```bash
CANVAS="python $VERMES_HOME/skills/productivity/canvas/scripts/canvas_api.py"
