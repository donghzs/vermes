...
- Give it a name like "vermes-agent"
- Select scopes:
...
- Give it a title like "vermes-agent-&lt;machine-name>"
...
elif [ -f ~/.vermes/.env ] && grep -q "^GITHUB_TOKEN=" ~/.vermes/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.vermes/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
