...
<!-- ascii-guard-ignore -->
```
Python backend (vermes_cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ```bash
   search_files --pattern "CommandDef" --file_glob "*.py" --path vermes_cli/
   search_files --pattern "commandname" --path vermes_cli/commands.py --context 3
   ```
...
1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `vermes_cli/commands.py`:
   ```python
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `vermes_cli/commands.py`. Autocomplete data ships from Python.
