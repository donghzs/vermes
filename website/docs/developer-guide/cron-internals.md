...
description: "How Vermes stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `vermes_cli/cron.py` | CLI `vermes cron` subcommands |
...
Jobs are stored in `~/.vermes/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `vermes cron` commands are run or during active CLI sessions.
...
```python
# ~/.vermes/scripts/check_competitors.py
...
2. **Environment variable** — `VERMES_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.vermes/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `vermes cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `vermes cron` CLI provides direct job management:

```bash
vermes cron list                    # Show all jobs
vermes cron create                  # Interactive job creation (alias: add)
vermes cron edit <job_id>           # Edit job configuration
vermes cron pause <job_id>          # Pause a running job
vermes cron resume <job_id>         # Resume a paused job
vermes cron run <job_id>            # Trigger immediate execution
vermes cron remove <job_id>         # Delete a job
```
