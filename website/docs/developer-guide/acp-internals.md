...
The ACP adapter wraps Vermes' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
vermes acp / vermes-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.vermes/.env
  -> configure stderr logging
  -> construct vermesACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'vermes-agent[acp]==<version>' vermes-acp`, pointed at the `vermes-agent` PyPI release.
...
- `allow_once` -> Vermes `once`
- `allow_always` -> Vermes `always`
- reject options -> Vermes `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Vermes tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["vermes-acp"])
...
Instead it reuses Vermes' runtime resolver:

- `acp_adapter/auth.py`
- `vermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Vermes provider/credentials. It also always advertises a terminal setup auth method (`vermes-setup`, args `--setup`) so first-run registry clients can open Vermes' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.vermes/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `vermes-acp` toolset definition
- `vermes_cli/main.py` — `vermes acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `vermes-acp` script
