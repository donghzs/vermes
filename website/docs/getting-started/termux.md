...
description: "Run Vermes Agent directly on an Android phone with Termux"
---

# Vermes on Android with Termux

This is the tested path for running Vermes Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Vermes from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Vermes now ships a Termux-aware installer path:
...
- links `vermes` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `vermes` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/vermes" "$PREFIX/bin/vermes"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `vermes` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.vermes/.env`.
...
### `vermes doctor` says ripgrep or Node is missing
