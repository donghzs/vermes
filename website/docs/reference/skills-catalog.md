...
description: "Catalog of bundled skills that ship with Vermes Agent"
---

# Bundled Skills Catalog

:::note Honest inventory (E-P0-4)
In this repo, top-level `skills/` is a **thin seed** (SOUL + structure; almost no `SKILL.md` files). The large skill library lives under **`optional-skills/`** (~80+) and is installed on demand into `~/.vermes/skills/` via Skillhub / `vermes skills install official/...` / agent self-evolution. Pages below are generated skill docs — treat them as the **installable catalog**, not as “already enabled out of the box.”
:::

Vermes ships installable skill definitions that can be copied into `~/.vermes/skills/` on install or on demand. Each skill below links to a dedicated page with its full definition, setup, and usage.

`vermes update` may sync seed skills when present, and the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.vermes/skills/` tree, install or restore it (e.g. `vermes skills install official/<name>` or `vermes skills reset <name> --restore`).
...
| [`vermes-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-vermes-agent) | Configure, extend, or contribute to Vermes Agent. | `autonomous-ai-agents/vermes-agent` |
...
| [`kanban-worker`](/docs/user-guide/skills/bundled/devops/devops-kanban-worker) | Pitfalls, examples, and edge cases for Vermes Kanban workers. The lifecycle itself is auto-injected into every worker's system prompt as KANBAN_GUIDANCE (from agent/prompt_builder.py); this skill is what you load when you want deeper det... | `devops/kanban-worker` |
...
| [`teams-meeting-pipeline`](/docs/user-guide/skills/bundled/productivity/productivity-teams-meeting-pipeline) | Operate the Teams meeting summary pipeline via Vermes CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions. | `productivity/teams-meeting-pipeline` |
...
| Skill | Description | Path |
|-------|-------------|------|
| [`debugging-vermes-tui-commands`](/docs/user-guide/skills/bundled/software-development/software-development-debugging-vermes-tui-commands) | Debug Vermes TUI slash commands: Python, gateway, Ink UI. | `software-development/debugging-vermes-tui-commands` |
| [`vermes-agent-skill-authoring`](/docs/user-guide/skills/bundled/software-development/software-development-vermes-agent-skill-authoring) | Author in-repo SKILL.md: frontmatter, validator, structure. | `software-development/vermes-agent-skill-authoring` |
...
| [`plan`](/docs/user-guide/skills/bundled/software-development/software-development-plan) | Plan mode: write markdown plan to .vermes/plans/, no exec. | `software-development/plan` |
