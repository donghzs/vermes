...
def render_skill_page(
    ...
    VERMES_meta = _meta.get("Vermes") or _meta.get("vermes") or _meta.get("hermes") or {}
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `vermes skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Vermes ships with a large built-in skill library copied into `~/.vermes/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Vermes also syncs bundled skills on `vermes update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.vermes/skills/` tree, it is still shipped with Vermes; restore it with `vermes skills reset <name> --restore`.",
        "",
