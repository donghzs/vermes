...
normal tests assume away:
  ...
  - Filesystem: VERMES_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own VERMES_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"VERMES_atyp_{name}_")
            os.environ["VERMES_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("vermes_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from vermes_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "VERMES_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
    ...
    for bad in bad_metas:
        r = subprocess.run(
            [sys.executable, "-m", "vermes_cli.main", "kanban",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from vermes_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["VERMES_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE VERMES_home: {resolved}")
...
@scenario("VERMES_home_with_spaces")
def _(home, kb):
    """VERMES_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="Vermes with spaces ")
    os.environ["VERMES_HOME"] = weird
    ...
    try:
        ...
        print(f"  VERMES_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("VERMES_home_with_unicode")
def _(home, kb):
    """VERMES_HOME with non-ASCII chars."""
    # Pre-create directly since tempfile doesn't love unicode prefixes
    weird = f"/tmp/VERMES_héllo_émöji_{os.getpid()}"
    ...
    try:
        ...
        print(f"  VERMES_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("VERMES_home_via_symlink")
def _(home, kb):
    """VERMES_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    uses Path.resolve() — two different symlink names pointing at the
    same dir should NOT double-init."""
    real = tempfile.mkdtemp(prefix="VERMES_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
    os.symlink(real, link1)
    os.symlink(real, link2)
    try:
        os.environ["VERMES_HOME"] = link1
        ...
        conn2.close()
        print("  symlinks to same VERMES_HOME share DB correctly")
    finally:
...
def _idempotency_race_worker(VERMES_home: str, key: str, result_file: str,
...
def _(home, kb):
    ...
    try:
        from vermes_cli import web_server as ws  # noqa
    except Exception:
        pass
