"""
Tests for mcp_serve — Vermes MCP server.
...
def _isolate_vermes_home(tmp_path, monkeypatch):
    """Redirect VERMES_HOME to a temp directory."""
    monkeypatch.setenv("VERMES_HOME", str(tmp_path))
    try:
        import vermes_constants
        monkeypatch.setattr(vermes_constants, "get_vermes_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking vermes_state schema."""
