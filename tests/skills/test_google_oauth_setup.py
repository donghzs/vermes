...
class TestvermesConstantsFallback:
    """Tests for _vermes_home.py fallback when vermes_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_vermes_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _vermes_home.py with vermes_constants blocked."""
        monkeypatch.setitem(sys.modules, "vermes_constants", None)
        spec = importlib.util.spec_from_file_location("_vermes_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_vermes_home_env_var(self, monkeypatch, tmp_path):
        """When vermes_constants is missing, VERMES_HOME comes from env var."""
        monkeypatch.setenv("VERMES_HOME", str(tmp_path / "custom-Vermes"))
    ...
    def test_fallback_ignores_empty_vermes_home(self, monkeypatch):
        """Empty/whitespace VERMES_HOME is treated as unset."""
        monkeypatch.setenv("VERMES_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_vermes_home() == Path.home() / ".vermes"

    def test_fallback_display_vermes_home_shortens_path(self, monkeypatch):
    ...
    def test_fallback_display_vermes_home_profile_path(self, monkeypatch):
        ...
        monkeypatch.setenv("VERMES_HOME", str(Path.home() / ".vermes/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_vermes_home() == "~/.vermes/profiles/coder"

    def test_fallback_display_vermes_home_custom_path(self, monkeypatch):
        """Fallback display_vermes_home() returns full path for non-home locations."""
        monkeypatch.setenv("VERMES_HOME", "/opt/Vermes-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_vermes_home() == "/opt/Vermes-custom"

    def test_delegates_to_vermes_constants_when_available(self):
        """When vermes_constants IS importable, _vermes_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_vermes_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import vermes_constants
        assert module.get_vermes_home is vermes_constants.get_vermes_home
        assert module.display_vermes_home is vermes_constants.display_vermes_home
