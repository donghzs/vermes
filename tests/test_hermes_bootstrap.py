...
(Vermes, vermes-agent, Vermes-acp, gateway, batch_runner, cli.py).  It
...
def _fresh_import():
    """Return a freshly-imported vermes_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("vermes_bootstrap", None)
    import vermes_bootstrap  # noqa: WPS433
    return vermes_bootstrap
...
class TestEntryPointsImportBootstrap:
    ...
    ENTRY_POINTS = [
        "vermes_cli/main.py",   # Vermes CLI (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import vermes_bootstrap' and that
        ...
        ``vermes_bootstrap`` as a top-level module) leaves Vermes
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # Vermes start even when vermes_bootstrap hasn't been
        ...
        assert first_import_name == "vermes_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'vermes_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import vermes_bootstrap' line to be the first import."
        )
