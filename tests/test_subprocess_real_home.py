"""Test VERMES_REAL_HOME is set in subprocess environments.

Covers: https://github.com/donghzs/vermes-agent/issues/25114

When profile isolation activates (VERMES_HOME/home/ exists), child
processes receive HOME={VERMES_HOME}/home/ for tool config isolation.
This test verifies that VERMES_REAL_HOME is also set, pointing to the
actual user home so scripts can locate ~/.vermes/ correctly.
"""

from __future__ import annotations

import os
from pathlib import Path
from unittest import mock

import pytest


# ---------------------------------------------------------------------------
# get_real_home unit tests
# ---------------------------------------------------------------------------

class TestGetRealHome:
    """Verify get_real_home() returns the actual user home."""

    def test_returns_home_env(self):
        """When HOME is set, get_real_home returns it."""
        from vermes_constants import get_real_home
        with mock.patch.dict(os.environ, {"HOME": "/home/testuser"}, clear=False):
            assert get_real_home() == "/home/testuser"

    def test_prefers_vermes_real_home(self):
        """VERMES_REAL_HOME takes priority over HOME."""
        from vermes_constants import get_real_home
        with mock.patch.dict(os.environ, {
            "VERMES_REAL_HOME": "/home/real",
            "HOME": "/home/fake",
        }, clear=False):
            assert get_real_home() == "/home/real"

    def test_fallback_expanduser(self):
        """When HOME is empty, falls back to expanduser."""
        from vermes_constants import get_real_home
        with mock.patch.dict(os.environ, {"HOME": ""}, clear=False):
            result = get_real_home()
            assert result  # not empty
            assert result != ""

    def test_fallback_tmp(self):
        """Last resort is /tmp."""
        from vermes_constants import get_real_home
        with mock.patch.dict(os.environ, {}, clear=True):
            # Remove HOME and VERMES_REAL_HOME
            env = {k: v for k, v in os.environ.items()
                   if k not in ("HOME", "VERMES_REAL_HOME")}
            with mock.patch.dict(os.environ, env, clear=True):
                with mock.patch("os.path.expanduser", return_value="~"):
                    result = get_real_home()
                    assert result == "/tmp"


# ---------------------------------------------------------------------------
# Subprocess env injection tests
# ---------------------------------------------------------------------------

class TestSubprocessEnvRealHome:
    """Verify VERMES_REAL_HOME is injected into subprocess environments."""

    def test_code_execution_sets_real_home(self, tmp_path):
        """execute_code child_env includes VERMES_REAL_HOME."""
        # Simulate profile isolation: VERMES_HOME/home/ exists
        profile_home = tmp_path / "profiles" / "worker"
        home_dir = profile_home / "home"
        home_dir.mkdir(parents=True)

        with mock.patch.dict(os.environ, {
            "HOME": "/home/testuser",
            "VERMES_HOME": str(profile_home),
        }, clear=False):
            from vermes_constants import get_subprocess_home, get_real_home
            
            profile_home_val = get_subprocess_home()
            assert profile_home_val == str(home_dir)
            
            real_home = get_real_home()
            assert real_home == "/home/testuser"
            assert real_home != profile_home_val

    def test_local_env_sets_real_home(self, tmp_path):
        """Local environment subprocesses get VERMES_REAL_HOME."""
        profile_home = tmp_path / "profiles" / "worker"
        home_dir = profile_home / "home"
        home_dir.mkdir(parents=True)

        with mock.patch.dict(os.environ, {
            "HOME": "/home/testuser",
            "VERMES_HOME": str(profile_home),
        }, clear=False):
            # Import and check the _make_run_env function
            import importlib
            import tools.environments.local as local_mod
            importlib.reload(local_mod)
            
            # The function should add VERMES_REAL_HOME when profile home is active
            from vermes_constants import get_real_home
            assert get_real_home() == "/home/testuser"

    def test_no_real_home_when_not_isolated(self):
        """When profile isolation is off, VERMES_REAL_HOME is not needed."""
        with mock.patch.dict(os.environ, {
            "HOME": "/home/testuser",
            "VERMES_HOME": "/home/testuser/.vermes",
        }, clear=False):
            from vermes_constants import get_subprocess_home
            result = get_subprocess_home()
            assert result is None  # No profile home dir


# ---------------------------------------------------------------------------
# Integration: verify the pattern works end-to-end
# ---------------------------------------------------------------------------

class TestRealHomeIntegration:
    """End-to-end verification that subprocesses can find ~/.vermes/."""

    def test_subprocess_can_find_vermes_dir(self, tmp_path):
        """A subprocess with overridden HOME can still find .vermes/ via VERMES_REAL_HOME."""
        real_home = tmp_path / "real_home"
        real_home.mkdir()
        (real_home / ".vermes").mkdir()

        profile_home = tmp_path / "profile_home"
        profile_home.mkdir()

        with mock.patch.dict(os.environ, {
            "HOME": str(profile_home),  # Simulated profile override
            "VERMES_REAL_HOME": str(real_home),
        }, clear=False):
            # Script logic: find .vermes/ using VERMES_REAL_HOME fallback
            VERMES_base = Path(os.environ.get("VERMES_REAL_HOME", os.environ.get("HOME", ""))) / ".vermes"
            assert VERMES_base.exists()
            assert str(VERMES_base).startswith(str(real_home))
