...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("Vermes/Vermes-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned Vermes/* and pr-* branches."""

    def test_prunes_orphaned_vermes_branch(self, git_repo):
        """Vermes/Vermes-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "Vermes/Vermes-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "Vermes/Vermes-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "Vermes/Vermes-deadbeef" in result.stdout
        ...
        assert "Vermes/Vermes-deadbeef" in orphaned
        ...
        assert "Vermes/Vermes-deadbeef" not in result.stdout
