...
class TestToolsetConsistency:
    ...
    def test_vermes_platforms_share_core_tools(self):
        """All Vermes-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on Vermes-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["Vermes-cli", "Vermes-telegram", "Vermes-discord", "Vermes-whatsapp", "Vermes-slack", "Vermes-signal", "Vermes-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_vermes_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("Vermes-whatsapp")

    def test_vermes_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("Vermes-api-server")
