...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    vermes_state = ModuleType("vermes_state")
    vermes_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "vermes_state", vermes_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Vermes"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Vermes"
