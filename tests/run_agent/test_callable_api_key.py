...
Vermes preserves the callable end-to-end so the SDK refreshes tokens
...
class TestCliEnsureRuntimeCredentialsCallable:
    ...
    ``VermesCLI`` instance) — the predicate change is the load-bearing
...
class TestInlinedDisplayMasks:
    ...
    def test_cli_show_config_handles_callable(self):
        """``cli.VermesCLI.show_config`` previously did
        ...
        assert "is_token_provider(self.api_key)" in src, (
            "cli.VermesCLI.show_config must guard self.api_key via "
            "is_token_provider so callable Entra ID providers don't "
            "crash /config."
        )
        assert '"Microsoft Entra ID"' in src, (
            "cli.VermesCLI.show_config must print the static "
