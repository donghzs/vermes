...
class TestVerifierEnabled:
    def test_default_is_enabled(self, monkeypatch):
        monkeypatch.delenv("VERMES_FILE_MUTATION_VERIFIER", raising=False)
        agent = _bare_agent()
    ...
    def test_env_disables(self, monkeypatch, value):
        monkeypatch.setenv("VERMES_FILE_MUTATION_VERIFIER", value)
        agent = _bare_agent()
        assert agent._file_mutation_verifier_enabled() is False

    def test_env_enables_over_config(self, monkeypatch):
        monkeypatch.setenv("VERMES_FILE_MUTATION_VERIFIER", "1")
        import vermes_cli.config as _cfg_mod
        monkeypatch.setattr(
