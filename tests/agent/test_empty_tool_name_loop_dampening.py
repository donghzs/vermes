...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated VERMES_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="VERMES_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".vermes"))
    prev_home = os.environ.get("VERMES_HOME")
    os.environ["VERMES_HOME"] = os.path.join(test_home, ".vermes")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("VERMES_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["VERMES_HOME"] = prev_home
