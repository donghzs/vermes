...
""",
        ...
        from vermes_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir()
        (VERMES_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
        from vermes_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from vermes_cli.plugins import PluginContext, PluginManifest, PluginManager
