...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from vermes_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "vermes_state", mod("vermes_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "vermes_cli.config",
        mod("vermes_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "vermes_cli.models",
        mod("vermes_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "vermes_cli.runtime_provider",
        mod(
            "vermes_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        ...
        mod("vermes_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("vermes_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "vermes_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
