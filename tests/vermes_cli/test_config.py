"""Tests for vermes_cli configuration management."""
...
class TestGetvermesHome:
    def test_default_path(self):
        # Fork branding: canonical home is ~/.vermes (VERMES_HOME overrides,
        # VERMES_HOME retained for backward compatibility).
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VERMES_HOME", None)
            os.environ.pop("VERMES_HOME", None)
            home = get_vermes_home()
            assert home == Path.home() / ".vermes"

    def test_env_override(self):
        with patch.dict(os.environ, {"VERMES_HOME": "/custom/path"}):
            home = get_vermes_home()
            assert home == Path("/custom/path")

    def test_vermes_home_env_override(self):
        with patch.dict(os.environ, {"VERMES_HOME": "/vermes/custom"}):
            home = get_vermes_home()
    ...
    def test_legacy_home_auto_migration(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        (legacy / "auth.json").write_text('{"k": 1}')
        sub = legacy / "sub"
        sub.mkdir()
        ...
        assert (home / "sub" / "x.txt").exists()
        # legacy 原文件保留 (拷贝非移动)
        assert (legacy / "auth.json").exists()
        # 幂等: 再次调用不报错, 已存在文件不被覆盖
        (home / "auth.json").write_text('{"k": 999}')
        get_vermes_home()
        assert (home / "auth.json").read_text() == '{"k": 999}'

    def test_existing_vermes_home_skips_migration(self, tmp_path, monkeypatch):
        """两个目录都已存在时跳过迁移, 直接用现有 ~/.vermes。"""
        (tmp_path / ".vermes").mkdir()
        (tmp_path / ".vermes" / "secret.txt").write_text("do-not-touch")
        ...
        assert home == tmp_path / ".vermes"
        # legacy 不被触碰
        assert (tmp_path / ".vermes" / "secret.txt").read_text() == "do-not-touch"


class TestEnsurevermesHome:
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``Vermes logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from vermes_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"VERMES_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="vermes_cli.config"):
                config = load_config()
            ...
            # stands out at Vermes startup before logging is configured)
            captured = capsys.readouterr()
            assert "vermes config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"VERMES_HOME": str(tmp_path)}):
            ...
            assert "vermes config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"VERMES_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"VERMES_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"VERMES_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"VERMES_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"VERMES_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from vermes_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
