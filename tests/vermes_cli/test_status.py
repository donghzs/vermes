...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    ...
    assert "Start with:   vermes gateway" in output
...
def test_show_status_reports_nous_auth_error(monkeypatch, capsys, tmp_path):
    ...
    output = capsys.readouterr().out
    assert "Nous Portal   ✗ not logged in (run: vermes auth add nous --type oauth)" in output
...
class TestShowStatusXaiOAuth:
    """xAI OAuth row in vermes status."""
    ...
    def test_not_logged_in_shows_login_command(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: vermes auth add xai-oauth)" in out
