...
The terminal `vermes model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import vermes_cli.providers as providers_mod
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "VERMES_OVERLAYS", {})
...
def test_switch_model_accepts_explicit_named_custom_provider(monkeypatch):
    ...
    monkeypatch.setattr("vermes_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("vermes_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("vermes_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: Vermes's own writer stores configured models as a dict
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("vermes_cli.models.fetch_lmstudio_models", _fake_fetch)
...
def test_custom_providers_uses_live_models_for_multi_model_endpoint(monkeypatch):
    ...
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("vermes_cli.providers.VERMES_OVERLAYS", {})
