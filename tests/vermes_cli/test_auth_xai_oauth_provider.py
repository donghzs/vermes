...
def test_xai_oauth_authorize_url_includes_referrer_vermes_agent():
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    VERMES_home = tmp_path / "Vermes"
    _setup_vermes_auth(VERMES_home, refresh_token="")
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    VERMES_home = tmp_path / "Vermes"
    fresh = _jwt_with_exp(int(time.time()) + 3600)
    _setup_vermes_auth(VERMES_home, access_token=fresh)
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
    monkeypatch.delenv("VERMES_XAI_BASE_URL", raising=False)
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_vermes_auth(VERMES_home, access_token=fresh)
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
    monkeypatch.setenv("VERMES_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
def _seed_xai_oauth_state(
    VERMES_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    VERMES_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    VERMES_home = tmp_path / "Vermes"
    _seed_xai_oauth_state(VERMES_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
    ...
    raw = json.loads((VERMES_home / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    VERMES_home = tmp_path / "Vermes"
    _seed_xai_oauth_state(VERMES_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    ...
    VERMES_home = tmp_path / "Vermes"
    fresh = _jwt_with_exp(int(time.time()) + 3600)
    _setup_vermes_auth(VERMES_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    ...
    # nothing to import.
    raw_after = json.loads((VERMES_home / "auth.json").read_text())
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_vermes_auth(VERMES_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    VERMES_home = tmp_path / "Vermes"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_vermes_auth(VERMES_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    ...
    _setup_vermes_auth(VERMES_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
    ...
    (VERMES_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_vermes_auth(VERMES_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    _setup_vermes_auth(VERMES_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    _setup_vermes_auth(VERMES_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    ``Vermes`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from agent.credential_pool import load_pool

    VERMES_home = tmp_path / "Vermes"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_vermes_auth(VERMES_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
