...
def test_ensure_dependency_skips_when_present():
    ...
    with patch("vermes_cli.dep_ensure.shutil") as mock_shutil:
...
def test_ensure_dependency_returns_false_when_missing_noninteractive():
    ...
    with patch("vermes_cli.dep_ensure.shutil") as mock_shutil:
        ...
        with patch("vermes_cli.dep_ensure._find_install_script", return_value=(None, None)):
            result = ensure_dependency("node", interactive=False)
...
def test_find_install_script_from_checkout(tmp_path):
    ...
    from vermes_cli.dep_ensure import _find_install_script
    scripts_dir = tmp_path / "scripts"
    scripts_dir.mkdir()
    (scripts_dir / "install.sh").write_text("#!/bin/bash", encoding="utf-8")
    with patch("vermes_cli.dep_ensure._IS_WINDOWS", False):
        path, shell = _find_install_script(package_dir=tmp_path / "vermes_cli", repo_root=tmp_path)
...
def test_find_install_script_from_wheel(tmp_path):
    ...
    from vermes_cli.dep_ensure import _find_install_script
    bundled = tmp_path / "vermes_cli" / "scripts"
...
def test_find_install_script_prefers_ps1_on_windows(tmp_path):
    ...
    scripts_dir = tmp_path / "vermes_cli" / "scripts"
    ...
    with patch("vermes_cli.dep_ensure._IS_WINDOWS", True):
        path, shell = _find_install_script(package_dir=tmp_path / "vermes_cli")
...
def test_find_install_script_falls_back_to_repo_root(tmp_path):
    ...
    from vermes_cli.dep_ensure import _find_install_script
    with patch("vermes_cli.dep_ensure._IS_WINDOWS", False):
        path, shell = _find_install_script(package_dir=tmp_path / "vermes_cli", repo_root=repo_root)
...
def test_has_system_browser_checks_windows_names():
    from vermes_cli.dep_ensure import _has_system_browser
    with patch("vermes_cli.dep_ensure._IS_WINDOWS", True), \
