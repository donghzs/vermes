"""Unit tests for vermes_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf Vermes-ok"])
        try:
            output = _read_until(bridge, b"Vermes-ok")
            assert b"Vermes-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$VERMES_PTY_TEST\""],
            env={**os.environ, "VERMES_PTY_TEST": "pty-env-works"},
        )
        try:
