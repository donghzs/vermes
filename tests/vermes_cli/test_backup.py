...
def _make_vermes_tree(root: Path) -> None:
    ...
    (root / "vermes_state.db").write_bytes(b"fake-state")
...
class TestBackup:
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        ...
        zips = list(tmp_path.glob("vermes-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only vermes_state.db (old wrong name) is rejected."""
        from vermes_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["vermes_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        from vermes_cli.backup import run_backup, run_import
...
class TestBackupEdgeCases:
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("vermes-backup-*.zip"))
        assert len(zips) == 1
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from vermes_cli.backup import _safe_copy_db
...
class TestRunPreUpdateBackup:
    ...
    def test_config_enabled_creates_backup(self, VERMES_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("vermes_cli.config"):
