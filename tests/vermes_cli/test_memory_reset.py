"""Tests for the `Vermes memory reset` CLI command.
...
- Profile-scoped reset (uses VERMES_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake VERMES_HOME with memory files."""
    VERMES_home = tmp_path / ".vermes"
    memories = VERMES_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nvermes repo is at ~/.vermes/vermes-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `Vermes memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        VERMES_home = tmp_path / ".vermes"
        (VERMES_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's VERMES_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("VERMES_HOME", str(VERMES_home))

        # The memories dir won't exist; get_vermes_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
