...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from vermes_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from vermes_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("vermes_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("vermes_cli.main._require_tty"):
            from vermes_cli.fallback_cmd import cmd_fallback_add
...
class TestClearCommand:
    def test_clear_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from vermes_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "vermes_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
