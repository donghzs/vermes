"""Tests for the Kanban DB layer (vermes_cli.kanban_db)."""
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see vermes_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("vermes_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
...
def test_resolve_vermes_argv_module_actually_runs():
    ...
    sufficient — if `vermes_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m vermes_cli.main --version`
