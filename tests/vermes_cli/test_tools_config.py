"""Tests for vermes_cli.tools_config platform tool persistence."""
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `vermes tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "vermes_cli.tools_config._xai_credentials_present", lambda: True
    )
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "vermes_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `vermes tools`,
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("vermes_cli.tools_config.save_config"):
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    monkeypatch.setattr("vermes_cli.tools_config.managed_nous_tools_enabled", lambda: True)
    config = {"model": {"provider": "nous"}}

    monkeypatch.setattr(
        "vermes_cli.nous_subscription.get_nous_auth_status",
...
def test_visible_providers_hide_nous_subscription_when_feature_flag_is_off(monkeypatch):
    monkeypatch.setattr("vermes_cli.tools_config.managed_nous_tools_enabled", lambda: False)
    config = {"model": {"provider": "nous"}}
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("vermes_cli.tools_config._run_post_setup", lambda key: None)
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "vermes_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("vermes_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "vermes_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config: configured.append(ts_key),
    )
    monkeypatch.setattr("vermes_cli.tools_config.save_config", lambda config: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        "vermes_cli.tools_config._prompt_toolset_checklist",
    ...
    monkeypatch.setattr(
        "vermes_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "vermes_cli.nous_subscription.get_nous_auth_status",
    ...
    configured = []
    monkeypatch.setattr(
        "vermes_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from vermes_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from vermes_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from vermes_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from vermes_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from vermes_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("vermes_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("vermes_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("vermes_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`vermes tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'vermes-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `vermes tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from vermes_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `vermes tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from vermes_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `vermes tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from vermes_cli.tools_config import _get_effective_configurable_toolsets
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("vermes_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("vermes_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("vermes_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("vermes_cli.tools_config.save_env_value", lambda k, v: None)
