...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (VERMES_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "Vermes" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_vermes_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:VERMES_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:VERMES_pkce"
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "vermes_cli.auth._nous_device_code_login",
        lambda **kwargs: {
            "portal_base_url": "https://portal.example.com",
            "inference_base_url": "https://inference.example.com/v1",
            "client_id": "Vermes-cli",
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "vermes_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "vermes_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from vermes_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`Vermes logout --provider openai-codex` must still clear model.provider.
    ...
    (VERMES_home / "config.yaml").write_text(
        "model:\n"
    ...
    config_text = (VERMES_home / "config.yaml").read_text()
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `Vermes logout` should target configured Codex if auth has no active provider."""
    VERMES_home = tmp_path / "Vermes"
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (VERMES_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((VERMES_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from vermes_cli.auth import _reset_config_provider
    ...
    with patch("vermes_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from vermes_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "vermes_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("vermes_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("vermes_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("vermes_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "vermes_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (VERMES_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from vermes_cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((VERMES_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from vermes_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from vermes_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    ...
    payload = json.loads((VERMES_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vermes_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((VERMES_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    ...
    (VERMES_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (VERMES_home / ".env").read_text().strip() == ""
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from vermes_cli.auth import is_source_suppressed
    from vermes_cli.auth_commands import auth_add_command
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import vermes_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_vermes_pkce_suppression(tmp_path, monkeypatch):
    """anthropic VERMES_pkce must not re-seed from ~/.vermes/.anthropic_oauth.json when suppressed."""
    VERMES_home = tmp_path / "Vermes"
    VERMES_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("VERMES_HOME", str(VERMES_home))

    import yaml
    (VERMES_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (VERMES_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["VERMES_pkce"]},
    }))

    # Stub the readers so only VERMES_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_vermes_oauth_credentials", lambda: {
    ...
    # VERMES_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "VERMES_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (VERMES_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
