...
VERMES_HOME scoping, and tick() serialization for profile jobs.
"""
...
def isolated_cron_profile_home(tmp_path, monkeypatch):
    """Create an isolated Vermes root with a named profile and temp cron store."""
    root = tmp_path / "Vermes-root"
...
class TestCronjobToolProfile:
    ...
    def test_schema_advertises_profile(self):
        ...
        assert "Vermes profile" in desc_lower
        assert "context-local" in desc_lower
        assert "subprocess" in desc_lower
        assert "temporarily sets VERMES_home" not in desc_lower
...
class TestRunJobProfileContext:
    ...
    def _install_agent_stubs(monkeypatch, observed: dict):
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["env_home_during_init"] = os.environ.get("VERMES_HOME")
                observed["profile_env_only_during_init"] = os.environ.get(
                    "VERMES_PROFILE_TEST_ONLY"
                )
                observed["profile_env_shared_during_init"] = os.environ.get(
                    "VERMES_PROFILE_TEST_SHARED"
                )
                observed["VERMES_home_during_init"] = str(get_vermes_home())
                observed["scheduler_home_during_init"] = str(sched._get_vermes_home())
                observed["skip_context_files"] = kwargs.get("skip_context_files")

            def run_conversation(self, *_a, **_kw):
                from vermes_constants import get_vermes_home

                observed["env_home_during_run"] = os.environ.get("VERMES_HOME")
                ...
                observed["VERMES_home_during_run"] = str(get_vermes_home())
                observed["scheduler_home_during_run"] = str(sched._get_vermes_home())
        ...
        monkeypatch.setattr(sched, "_vermes_home", None)
    ...
    def test_run_job_sets_and_restores_profile_home(
        ...
        assert observed["VERMES_home_during_init"] == str(profile_home.resolve())
        assert observed["VERMES_home_during_run"] == str(profile_home.resolve())
        ...
        assert os.environ["VERMES_HOME"] == str(root)
        assert sched._get_vermes_home() == root
    ...
    def test_profile_dotenv_environment_is_restored(
        ...
        observed: dict = {}
        self._install_agent_stubs(monkeypatch, observed)
        monkeypatch.setenv("VERMES_PROFILE_TEST_SHARED", "outer")
        monkeypatch.delenv("VERMES_PROFILE_TEST_ONLY", raising=False)

        def fake_load_dotenv(path, *_a, **_kw):
            observed.setdefault("dotenv_paths", []).append(str(path))
            os.environ["VERMES_PROFILE_TEST_SHARED"] = "profile-value"
            os.environ["VERMES_PROFILE_TEST_ONLY"] = "profile-only"
            os.environ["VERMES_CRON_TIMEOUT"] = "123"
            return True
        ...
        assert os.environ["VERMES_PROFILE_TEST_SHARED"] == "outer"
        assert "VERMES_PROFILE_TEST_ONLY" not in os.environ
        assert os.environ["VERMES_CRON_TIMEOUT"] == "0"
        assert os.environ["VERMES_HOME"] == str(root)
        assert sched._get_vermes_home() == root
    ...
    def test_no_agent_profile_uses_profile_scripts_dir_and_restores_env(
        ...
        (scripts_dir / "print_home.py").write_text(
            "import os\nprint(os.environ.get('VERMES_HOME', ''))\n",
            encoding="utf-8",
        )
        monkeypatch.setattr(sched, "_vermes_home", None)
    ...
    def test_run_job_without_profile_leaves_vermes_home_untouched(
        self, isolated_cron_profile_home, monkeypatch
    ):
        import cron.scheduler as sched
        ...
        assert observed["VERMES_home_during_init"] == str(root)
        assert os.environ["VERMES_HOME"] == str(root)
