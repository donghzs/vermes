...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("vermes_cli.config.get_config_path", return_value=config_path), \
             patch("vermes_cli.config.ensure_vermes_home"):
    ...
    def test_ensure_vermes_home_sets_0700(self):
        home = Path(self.tmpdir) / ".vermes"
        with patch("vermes_cli.config.get_vermes_home", return_value=home):
