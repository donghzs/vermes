...
def _clean_state():
    ...
    saved = {}
    for k in ("VERMES_INTERACTIVE", "VERMES_GATEWAY_SESSION", "VERMES_EXEC_ASK", "VERMES_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["VERMES_INTERACTIVE"] = "1"
        session_key = os.getenv("VERMES_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No VERMES_INTERACTIVE or VERMES_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
