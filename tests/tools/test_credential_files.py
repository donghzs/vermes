...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        VERMES_home.mkdir()
        (VERMES_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"VERMES_HOME": str(VERMES_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(VERMES_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.vermes/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir()
        (VERMES_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir()
        (VERMES_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir()
        (VERMES_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.vermes/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        VERMES_home = tmp_path / ".vermes"
        (VERMES_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"VERMES_HOME": str(VERMES_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.vermes")

        assert mounts[0]["container_path"] == "/home/user/.vermes/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.vermes/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.vermes/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape VERMES_HOME."""
        monkeypatch.setenv("VERMES_HOME", str(tmp_path / ".vermes"))
        (tmp_path / ".vermes").mkdir()

        # Create a sensitive file one level above VERMES_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside VERMES_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside VERMES_HOME must still be registered."""
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir()
        monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
        (VERMES_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_vermes_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of VERMES_HOME must be allowed."""
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir()
        subdir = VERMES_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside VERMES_HOME pointing outside must be rejected."""
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir()
        monkeypatch.setenv("VERMES_HOME", str(VERMES_home))

        # Create a sensitive file outside VERMES_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside VERMES_home pointing outside
        symlink = VERMES_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes VERMES_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, VERMES_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape VERMES_HOME."""
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir()
        monkeypatch.setenv("VERMES_HOME", str(VERMES_home))
        ...
        sensitive.write_text("{}")
        self._write_config(VERMES_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(VERMES_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside VERMES_HOME via config must still mount."""
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir()
        monkeypatch.setenv("VERMES_HOME", str(VERMES_home))

        (VERMES_home / "oauth.json").write_text("{}")
        self._write_config(VERMES_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir()
        (VERMES_home / "cache" / "documents").mkdir(parents=True)
        (VERMES_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("VERMES_HOME", str(VERMES_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.vermes/cache/documents" in paths
        assert "/root/.vermes/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.vermes/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir()
        # Use legacy dir name — get_vermes_dir prefers old if it exists
        (VERMES_home / "document_cache").mkdir()
        (VERMES_home / "image_cache").mkdir()
        monkeypatch.setenv("VERMES_HOME", str(VERMES_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(VERMES_home / "document_cache") in host_paths
        assert str(VERMES_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.vermes/cache/documents" in container_paths
        assert "/root/.vermes/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        VERMES_home = tmp_path / ".vermes"
        doc_dir = VERMES_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        VERMES_home = tmp_path / ".vermes"
        ss_dir = VERMES_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.vermes/cache/screenshots/session_abc/screen1.png"
