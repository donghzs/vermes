"""Tests for ACP Registry metadata shipped with Vermes."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "vermes-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Vermes Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/donghzs/vermes-agent"
    assert data["website"].startswith("https://vermes-agent.donghzs.com/")
    assert data["authors"] == ["Vermes"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"vermes-agent[acp]=={data['version']}"
    assert uvx["args"] == ["vermes-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"vermes-agent[acp]=={_pyproject_version()}"
    )
