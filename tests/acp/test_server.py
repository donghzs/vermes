...
from acp_adapter.server import VermesACPAgent, VERMES_VERSION
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.version == VERMES_VERSION
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("vermes_cli.config.load_config", lambda: {
...
class TestPrompt:
    ...
    @pytest.mark.asyncio
    async def test_prompt_propagates_vermes_session_id_env(self, agent, monkeypatch):
        """ACP must propagate the originating session id to the agent loop
        via ``VERMES_SESSION_ID`` so tools that want to stamp side-effects
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("VERMES_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "VERMES_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("VERMES_SESSION_ID") is None, (
            "VERMES_SESSION_ID must be restored after the agent call so "
            "a re-used executor thread doesn't leak the id into the next "
            "session's tools"
        )

    @pytest.mark.asyncio
    async def test_prompt_restores_prior_vermes_session_id(self, agent, monkeypatch):
        """If the env already had VERMES_SESSION_ID set (e.g. nested
        agent loops), the prior value must be restored after the inner
        prompt completes — not popped, not left at the inner id."""
        monkeypatch.setenv("VERMES_SESSION_ID", "outer-sess")
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("VERMES_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("VERMES_SESSION_ID") == "outer-sess"
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert VERMES_VERSION in result
