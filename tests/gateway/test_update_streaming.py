...
def _make_runner(VERMES_home=None):
...
class TestGatewayPrompt:
    ...
    def test_writes_prompt_file_and_reads_response(self, tmp_path):
        ...
        def write_response():
            time.sleep(0.3)
            (VERMES_home / ".update_response").write_text("y")

        thread = threading.Thread(target=write_response)
        thread.start()

        with patch.dict(os.environ, {"VERMES_HOME": str(VERMES_home)}):
            from vermes_cli.main import _gateway_prompt
            result = _gateway_prompt("Restore? [Y/n]", "y", timeout=5.0)

        thread.join()
        assert result == "y"
        # Both files should be cleaned up
        assert not (VERMES_home / ".update_prompt.json").exists()
        assert not (VERMES_home / ".update_response").exists()
    ...
    def test_prompt_file_content(self, tmp_path):
        ...
        def capture_and_respond():
            ...
            prompt_path = VERMES_home / ".update_prompt.json"
            for _ in range(20):
                if prompt_path.exists():
                    prompt_data = json.loads(prompt_path.read_text())
                    (VERMES_home / ".update_response").write_text("n")
                    return
                time.sleep(0.1)
    ...
    def test_empty_response_returns_default(self, tmp_path):
        ...
        VERMES_home = tmp_path / ".vermes"
        VERMES_home.mkdir()
        (VERMES_home / ".update_response").write_text("")
...
class TestRestoreStashWithInputFn:
    ...
    def test_uses_input_fn_when_provided(self, tmp_path):
        ...
        from vermes_cli.main import _restore_stashed_changes
...
class TestUpdateCommandGatewayFlag:
    ...
    async def test_spawns_with_gateway_flag(self, tmp_path):
        ...
        with patch("gateway.run._vermes_home", VERMES_home), \
...
class TestWatchUpdateProgress:
    ...
    async def test_streams_output_to_adapter(self, tmp_path):
        ...
        (VERMES_home / ".update_pending.json").write_text(json.dumps(pending))
        # Write output
        (VERMES_home / ".update_output.txt").write_text("→ Fetching updates...\n", encoding="utf-8")
        ...
        async def write_exit_code():
            ...
            (VERMES_home / ".update_output.txt").write_text(
                "→ Fetching updates...\n✓ Code updated!\n"
            , encoding="utf-8")
            (VERMES_home / ".update_exit_code").write_text("0")
    ...
    async def test_detects_and_forwards_prompt(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (VERMES_home / ".update_pending.json").write_text(json.dumps(pending))
        (VERMES_home / ".update_output.txt").write_text("output\n")
        ...
        async def simulate_prompt_cycle():
            ...
            (VERMES_home / ".update_prompt.json").write_text(json.dumps(prompt))
            # Simulate user responding
            await asyncio.sleep(0.5)
            (VERMES_home / ".update_response").write_text("y")
            (VERMES_home / ".update_prompt.json").unlink(missing_ok=True)
            await asyncio.sleep(0.3)
            (VERMES_home / ".update_exit_code").write_text("0")
    ...
    async def test_prompt_forwarding_preserves_thread_metadata(self, tmp_path):
        ...
        (VERMES_home / ".update_pending.json").write_text(json.dumps(pending))
        (VERMES_home / ".update_output.txt").write_text("")
        (VERMES_home / ".update_prompt.json").write_text(json.dumps({
    ...
    async def test_cleans_up_on_completion(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        pending_path = VERMES_home / ".update_pending.json"
        output_path = VERMES_home / ".update_output.txt"
    ...
    async def test_failure_exit_code(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (VERMES_home / ".update_pending.json").write_text(json.dumps(pending))
        (VERMES_home / ".update_output.txt").write_text("error occurred\n")
        (VERMES_home / ".update_exit_code").write_text("1")
    ...
    async def test_falls_back_when_adapter_unavailable(self, tmp_path):
        ...
        (VERMES_home / ".update_pending.json").write_text(json.dumps(pending))
        (VERMES_home / ".update_output.txt").write_text("done\n")
        (VERMES_home / ".update_exit_code").write_text("0")
    ...
    async def test_prompt_is_recovered_after_watcher_restart(self, tmp_path):
        ...
        with patch("gateway.run._vermes_home", VERMES_home):
            ...
            assert (VERMES_home / ".update_prompt.json").exists()
...
class TestUpdatePromptInterception:
    ...
    async def test_intercepts_response_when_prompt_pending(self, tmp_path):
        ...
        (VERMES_home / ".update_prompt.json").write_text(json.dumps({"prompt": "test"}))
        ...
        response_path = VERMES_home / ".update_response"
