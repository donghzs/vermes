...
from vermes_cli.profiles import _get_default_vermes_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "Vermes"
        assert config.workspace_id == "Vermes"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("VERMES_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"Vermes": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"Vermes": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"Vermes": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveConfigPath:
    def test_prefers_vermes_home_when_exists(self, tmp_path):
        VERMES_home = tmp_path / "Vermes"
        VERMES_home.mkdir()
        local_cfg = VERMES_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        ...
        assert _get_default_vermes_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_vermes_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".vermes".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".vermes"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_default_returns_vermes(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("VERMES_HONCHO_HOST", None)
            os.environ.pop("VERMES_HOME", None)
            assert resolve_active_host() == "Vermes"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"VERMES_HONCHO_HOST": "Vermes.coder"}):
            assert resolve_active_host() == "Vermes.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VERMES_HONCHO_HOST", None)
            with patch("vermes_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "Vermes.coder"

    def test_default_profile_returns_vermes(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VERMES_HONCHO_HOST", None)
            with patch("vermes_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "Vermes"

    def test_custom_profile_returns_vermes(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VERMES_HONCHO_HOST", None)
            with patch("vermes_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "Vermes"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VERMES_HONCHO_HOST", None)
            # Temporarily remove vermes_cli.profiles to simulate import failure
            saved = sys.modules.get("vermes_cli.profiles")
            sys.modules["vermes_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "Vermes"
            finally:
                if saved is not None:
                    sys.modules["vermes_cli.profiles"] = saved
                else:
                    sys.modules.pop("vermes_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="Vermes.coder")
        assert config.host == "Vermes.coder"
        assert config.workspace_id == "Vermes"  # shared workspace
        assert config.ai_peer == "Vermes.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="Vermes")
        assert config.host == "Vermes"
        assert config.workspace_id == "Vermes"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "Vermes": {"aiPeer": "Vermes", "peerName": "alice"},
                "Vermes.coder": {
                    "aiPeer": "Vermes.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="Vermes.coder", config_path=config_file,
        )
        assert config.host == "Vermes.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "Vermes.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="Vermes.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "Vermes.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"Vermes": {"enabled": True, "aiPeer": "Vermes"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"Vermes": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_vermes_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="Vermes",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("vermes_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("vermes_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_vermes_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="Vermes",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("vermes_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
