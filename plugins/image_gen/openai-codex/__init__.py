...
Output is saved as PNG under ``$VERMES_HOME/cache/images/``.
"""
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `vermes auth codex` (or `vermes setup` → Codex) "
        ...
        if image_url or reference_image_urls:
            return error_response(
                error=(
                    ...
                    "supports edits (OpenAI, Agnes, xAI) via `vermes tools` "
                    "→ Image Generation."
                ),
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`vermes auth codex` (or `vermes setup` → Codex) to sign in."
                ),
