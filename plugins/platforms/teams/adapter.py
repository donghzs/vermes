"""
Microsoft Teams platform adapter for Vermes.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Vermes"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        VERMES_action = data.get("VERMES_action", "")
        session_key = data.get("session_key", "")

        if not VERMES_action or not session_key:
        ...
        choice = choice_map.get(VERMES_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="VERMES_approve",
                    data={**btn_data_base, "VERMES_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="VERMES_approve",
                    data={**btn_data_base, "VERMES_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="VERMES_approve",
                    data={**btn_data_base, "VERMES_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="VERMES_approve",
                    data={**btn_data_base, "VERMES_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Vermes\" --endpoint \"https://<tunnel>/api/messages\"")
    logger.info()
    ...
    logger.info()
    print_success("Teams configuration saved to ~/.vermes/.env")
