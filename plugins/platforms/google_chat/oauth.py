...
- Per-user tokens (keyed by sender email):
    ``${VERMES_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${VERMES_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${VERMES_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${VERMES_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${VERMES_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's VERMES_HOME helper so the token follows the user's
# profile (e.g. tests can override via VERMES_HOME=/tmp/...).
try:
    from vermes_constants import display_vermes_home, get_vermes_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where vermes_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _vermes_home.py shim).
...
def _vermes_home() -> Path:
    """Resolve VERMES_HOME at call time (NOT module import).

    Tests and ``VERMES_HOME=...`` env overrides need this to be late-
...
def _legacy_token_path() -> Path:
    return _vermes_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _vermes_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _vermes_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _vermes_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _vermes_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install Vermes-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        logger.info("  pip install 'Vermes-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into VERMES_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_vermes_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_vermes_home()}/google_chat_user_token.json"
    )
