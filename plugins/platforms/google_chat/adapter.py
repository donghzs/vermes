...
# ``model_tools`` import time, so ``vermes status``, ``Vermes chat``, etc.
...
# ``VERMES_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Vermes is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a Vermes MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${VERMES_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _vermes_home = _Path.home() / ".vermes"
        self._thread_count_store = _ThreadCountStore(
            _vermes_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Vermes is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("VERMES_HOME", str(_Path.home() / ".vermes"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${VERMES_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a Vermes MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Vermes is thinking…' marker message.
        ...
        "Vermes is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Vermes is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Vermes is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Vermes is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Vermes is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``vermes setup``.

    The setup wizard at ``vermes_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.vermes/.env`` so the next
    gateway restart picks them up.
    """
    from vermes_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. Vermes-chat-events) and a Pull subscription.")
    ...
    logger.info()
    print_success("Google Chat configuration saved to ~/.vermes/.env")
    print_info("Restart the gateway: vermes gateway restart")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'Vermes-agent[google_chat]'",
        ...
        # the "Vermes is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Vermes is thinking…' marker message "
