...
def cmd_enable(args) -> None:
    ...
    if block.get("enabled") is True:
        logger.info(f"  {label}Honcho is already enabled.\n")
        return
    ...
    _write_config(cfg)
    logger.info(f"  {label}Honcho enabled.")

    # Create peer eagerly
    if _ensure_peer_exists(host):
        logger.info(f"  {label}Peer '{block.get('aiPeer', host)}' ready.")
    else:
        logger.info(f"  {label}Peer creation deferred (no connection).")

    logger.info(f"  Saved to {_config_path()}\n")
...
def cmd_disable(args) -> None:
    ...
    if not block or block.get("enabled") is False:
        logger.info(f"  {label}Honcho is already disabled.\n")
        return

    block["enabled"] = False
    _write_config(cfg)
    logger.info(f"  {label}Honcho disabled.")
    logger.info(f"  Saved to {_config_path()}\n")
...
def cmd_sync(args) -> None:
    ...
    if created:
        ...
        logger.info("  All profiles already have Honcho config.")
    if skipped:
        logger.info(f"  {skipped} profile(s) already configured (skipped).")
    logger.info()
...
def _ensure_sdk_installed() -> bool:
    ...
    logger.info("  honcho-ai is not installed.")
    answer = _prompt("Install it now? (honcho-ai>=2.0.1)", default="y")
    if answer.lower() not in {"y", "yes"}:
        logger.info("  Skipping install. Run: pip install 'honcho-ai>=2.0.1'\n")
        return False
    ...
    if result.returncode == 0:
        ...
        logger.info("  Run manually: pip install 'honcho-ai>=2.0.1'\n")
        return False
...
def cmd_setup(args) -> None:
    ...
    logger.info("    cloud -- Honcho cloud (api.honcho.dev)")
    logger.info("    local -- self-hosted Honcho server")
    ...
    if is_local:
        ...
        if current_key:
            ...
            logger.info("  Local connections will skip auth automatically.")
        else:
    ...
    logger.info("    session -- batch write at session end only")
    ...
    logger.info("    context -- auto-injected context only, Honcho tools hidden")
    ...
    logger.info("    Depth Honcho uses when synthesizing user context on auto-injected calls.")
    logger.info("    minimal  -- quick factual lookups")
    logger.info("    low      -- straightforward questions (default)")
    logger.info("    medium   -- multi-aspect synthesis")
    logger.info("    high     -- complex behavioral patterns")
    ...
    logger.info("    per-repo      -- one session per git repository")
    ...
    logger.info("\n  Honcho is ready.")
    logger.info(f"  Session:   {hcfg.resolve_session_name()}")
    logger.info(f"  Workspace: {hcfg.workspace_id}")
    logger.info(f"  User:      {hcfg.peer_name}")
    logger.info(f"  AI peer:   {hcfg.ai_peer}")
    logger.info(f"  Observe:   {hcfg.observation_mode}")
    logger.info(f"  Frequency: {hcfg.write_frequency}")
    logger.info(f"  Recall:    {hcfg.recall_mode}")
    logger.info(f"  Sessions:  {hcfg.session_strategy}")
    logger.info("\n  Honcho tools available in chat:")
    logger.info("    honcho_context   -- session context: summary, representation, card, messages")
    logger.info("    honcho_search    -- semantic search over history")
    logger.info("    honcho_profile   -- peer card, key facts")
...
def cmd_status(args) -> None:
    ...
    logger.info(f"  Host:           {hcfg.host}")
    logger.info(f"  Enabled:        {hcfg.enabled}")
    ...
    logger.info(f"  Config:         {active_path}")
    if write_path != active_path:
        logger.info(f"  Write to:       {write_path}  (profile-local)")
    if active_path == global_path:
        logger.info(f"  Fallback:       (none — using global ~/.honcho/config.json)")
    elif global_path.exists():
        logger.info(f"  Fallback:       {global_path}  (exists, cross-app interop)")

    logger.info(f"  AI peer:        {hcfg.ai_peer}")
    logger.info(f"  User peer:      {hcfg.peer_name or 'not set'}")
    logger.info(f"  Session key:    {hcfg.resolve_session_name()}")
    logger.info(f"  Session strat:  {hcfg.session_strategy}")
    logger.info(f"  Recall mode:    {hcfg.recall_mode}")
    logger.info(f"  Context budget: {hcfg.context_tokens or '(uncapped)'} tokens")
    raw = getattr(hcfg, "raw", None) or {}
    dialectic_cadence = raw.get("dialecticCadence") or 1
    logger.info(f"  Dialectic cad:  every {dialectic_cadence} turn{'s' if dialectic_cadence != 1 else ''}")
    reasoning_cap = raw.get("reasoningLevelCap") or hcfg.reasoning_level_cap
    heuristic_on = "on" if hcfg.reasoning_heuristic else "off"
    logger.info(f"  Reasoning:      base={hcfg.dialectic_reasoning_level}, cap={reasoning_cap}, heuristic={heuristic_on}")
    logger.info(f"  Observation:    user(me={hcfg.user_observe_me},others={hcfg.user_observe_others}) ai(me={hcfg.ai_observe_me},others={hcfg.ai_observe_others})")
    logger.info(f"  Write freq:     {hcfg.write_frequency}")
    ...
    try:
        ...
        if card:
            ...
            for fact in card[:10]:
                logger.info(f"    - {fact}")
            if len(card) > 10:
                logger.info(f"    ... and {len(card) - 10} more")
...
def _cmd_status_all() -> None:
    ...
    logger.info(f"  {'Profile':<14} {'Host':<22} {'Enabled':<9} {'Recall':<9} {'Write'}")
    logger.info(f"  {'─' * 14} {'─' * 22} {'─' * 9} {'─' * 9} {'─' * 9}")
    ...
    for name, host, block in rows:
        ...
        logger.info(f"  {name + marker:<14} {host:<22} {enabled_str:<9} {recall:<9} {write}")
...
def cmd_peers(args) -> None:
    ...
    logger.info(f"  {'Profile':<14} {'User peer':<16} {'AI peer'}")
    logger.info(f"  {'─' * 14} {'─' * 16} {'─' * 18}")

    for name, host, block in rows:
        user = block.get("peerName") or cfg.get("peerName") or "(not set)"
        ai = block.get("aiPeer") or cfg.get("aiPeer") or host
        logger.info(f"  {name:<14} {user:<16} {ai}")
...
def cmd_sessions(args) -> None:
    ...
    for path, name in sorted(sessions.items()):
        ...
        logger.info(f"  {name:<30} {path}{marker}")
    logger.info()
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        logger.info(f"  Dialectic cap:        {max_chars} chars\n")
        return
    ...
    if user_name is not None:
        ...
        changed = True
        logger.info(f"  {label}User peer -> {user_name.strip()}")

    if ai_name is not None:
        cfg.setdefault("hosts", {}).setdefault(host, {})["aiPeer"] = ai_name.strip()
        changed = True
        logger.info(f"  {label}AI peer   -> {ai_name.strip()}")

    if reasoning is not None:
        if reasoning not in REASONING_LEVELS:
            logger.info(f"  Invalid reasoning level '{reasoning}'. Options: {', '.join(REASONING_LEVELS)}")
            return
        cfg.setdefault("hosts", {}).setdefault(host, {})["dialecticReasoningLevel"] = reasoning
        changed = True
        logger.info(f"  {label}Dialectic reasoning level -> {reasoning}")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg not in MODES:
        logger.info(f"  Invalid mode '{mode_arg}'. Options: {', '.join(MODES)}\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg not in STRATEGIES:
        logger.info(f"  Invalid strategy '{strat_arg}'. Options: {', '.join(STRATEGIES)}\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        logger.info("    Raw memory retrieval. Honcho returns stored facts/history about")
    ...
    if context is not None:
        ...
        logger.info(f"  {label}context tokens -> {context}")
        changed = True
    if dialectic is not None:
        cfg.setdefault("hosts", {}).setdefault(host, {})["dialecticMaxChars"] = dialectic
        logger.info(f"  {label}dialectic cap  -> {dialectic} chars")
        changed = True
...
def cmd_identity(args) -> None:
    ...
    try:
        ...
        logger.info(f"  Honcho connection failed: {e}\n")
        return
    ...
    if show:
        ...
        if user_card:
            ...
            logger.info("  No user peer card yet. Send a few messages to build one.")

        # ── AI peer ──────────────────────────────────────────────────────────
        ai_rep = mgr.get_ai_representation(session_key)
        logger.info(f"\nAI peer ({hcfg.ai_peer})\n" + "─" * 40)
        if ai_rep.get("representation"):
            logger.info(ai_rep["representation"])
    ...
    if not content:
        logger.info(f"  File is empty: {p}\n")
        return

    source = p.name
    ok = mgr.seed_ai_identity(session_key, content, source=source)
    if ok:
        logger.info(f"  Seeded AI peer identity from {p.name} into session '{session_key}'")
        logger.info(f"  Honcho will incorporate this into {hcfg.ai_peer}'s representation over time.\n")
    else:
...
def cmd_migrate(args) -> None:
    ...
    logger.info("  Honcho replaces that with a cloud-backed, LLM-observable memory layer:")
    logger.info("  context is retrieved semantically, injected automatically each turn,")
    logger.info("  and enriched by a dialectic reasoning layer that builds over time.")
    logger.info()

    # ── Step 1: Honcho account ────────────────────────────────────────────────
    logger.info("Step 1  Create a Honcho account")
    logger.info()
    if has_key:
    ...
    logger.info()
    logger.info("Step 2  Detected OpenClaw memory files")
    logger.info()
    if user_files or agent_files:
        if user_files:
            logger.info(f"  User memory ({len(user_files)} file(s)) — will go to Honcho user peer:")
            for f in user_files:
                logger.info(f"    {f}")
        if agent_files:
            logger.info(f"  Agent identity ({len(agent_files)} file(s)) — will go to Honcho AI peer:")
    ...
    logger.info()
    logger.info("Step 3  Migrate user memory files → Honcho user peer")
    logger.info()
    logger.info("  USER.md and MEMORY.md contain facts about you that the agent should")
    logger.info("  remember across sessions. Honcho will store these under your user peer")
    logger.info("  and inject relevant excerpts into the system prompt automatically.")
    logger.info()
    if user_files:
                ...
                try:
                    ...
                    for d in dirs_with_files:
                        if mgr.migrate_memory_files(session_key, d):
                            any_uploaded = True
                    if any_uploaded:
                        logger.info(f"  Uploaded user memory files from: {', '.join(dirs_with_files)}")
                    else:
                        logger.info("  Nothing uploaded (files may already be migrated or empty).")
                except Exception as e:
                    logger.info(f"  Failed: {e}")
        else:
    ...
    logger.info()
    logger.info("Step 4  Seed AI identity files → Honcho AI peer")
    logger.info()
    logger.info("  SOUL.md, IDENTITY.md, AGENTS.md, TOOLS.md, BOOTSTRAP.md define the")
    ...
    logger.info()
    if agent_files:
        logger.info(f"  Found: {', '.join(f.name for f in agent_files)}")
        logger.info()
        if has_key:
    ...
    # ── Step 5: What changes ──────────────────────────────────────────────────
    logger.info()
    logger.info("Step 5  What changes vs. OpenClaw native memory")
    logger.info()
    logger.info("  Storage")
    ...
    logger.info("    honcho_context   — session context: summary, representation, card, messages")
    logger.info("    honcho_search        — semantic search over stored context")
    logger.info("    honcho_profile       — fast peer card snapshot")
    logger.info("    honcho_reasoning     — ask Honcho a question, synthesized answer")
    logger.info("    honcho_conclude      — write a conclusion/fact back to memory")
    logger.info()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        logger.info(f"  Unknown honcho command: {sub}")
