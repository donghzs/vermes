...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/vermes>
...
```bash
vermes honcho setup    # full interactive wizard (cloud or local)
vermes memory setup    # generic picker, also works
```

Or manually:
```bash
vermes config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$VERMES_HOME/honcho.json` | Profile-local (isolated Vermes instances) |
| 2 | `~/.vermes/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Vermes profile: `vermes` (default) or `vermes.<profile>`.
...
| 4 | `per-session` strategy | Vermes session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`vermes-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`vermes`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-vermes-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening vermes in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Vermes session ID (timestamp + hex). Every `vermes` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Vermes profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`vermes`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Vermes profile: `vermes` (default) or `vermes.<profile>` (e.g. `vermes -p coder` → host key `vermes.coder`).
...
| `VERMES_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `vermes honcho setup` | Full interactive setup wizard |
| `vermes honcho status` | Show resolved config for active profile |
| `vermes honcho enable` / `disable` | Toggle Honcho for active profile |
| `vermes honcho mode <mode>` | Change recall or observation mode |
| `vermes honcho peer --user <name>` | Update user peer name |
| `vermes honcho peer --ai <name>` | Update AI peer name |
| `vermes honcho tokens --context <N>` | Set context token budget |
| `vermes honcho tokens --dialectic <N>` | Set dialectic max chars |
| `vermes honcho map <name>` | Map current directory to a session name |
| `vermes honcho sync` | Create host blocks for all Vermes profiles |
