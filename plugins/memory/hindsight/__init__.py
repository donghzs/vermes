...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: Vermes)
...
Or via $VERMES_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $VERMES_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_vermes_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "Vermes": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "Vermes"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    ...
    profile = config.get("profile", "Vermes")
    return str(profile or "Vermes")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    collapsed — e.g. ``Vermes-{user}`` with no user becomes ``Vermes``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Vermes and the User"
    ...
    def save_config(self, values, VERMES_home):
        """Write config to $VERMES_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(VERMES_home) / "hindsight"
    ...
    def post_setup(self, VERMES_home: str, config: dict) -> None:
        ...
        provider_config.setdefault("bank_id", "Vermes")
        ...
        self.save_config(provider_config, VERMES_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(VERMES_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(VERMES_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "Vermes"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: Vermes-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Vermes and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "Vermes"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "Vermes"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "Vermes", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "Vermes")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Vermes and the User")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See Vermes-agent#6672.
