...
Config file: `$VERMES_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `vermes` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `vermes-{identity}` → `vermes-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Vermes profile:

```json
{
  "container_tag": "vermes-{identity}"
}
```

For a profile named `coder`, this resolves to `vermes-coder`. The default profile resolves to `vermes-default`. Without `{identity}`, all profiles share the same container.
