...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with Vermes-agent)
2. User-installed providers: ``$VERMES_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$VERMES_HOME/plugins/`` or None if unavailable."""
    try:
        from vermes_constants import get_vermes_home
        d = get_vermes_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$VERMES_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_vermes_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_vermes_user_memory.{active_provider}.cli"
    try:
