"""disk_cleanup — ephemeral file cleanup for Vermes.
...
Rules:
  ...
  - empty dirs    → always delete (under VERMES_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly VERMES_HOME and /tmp/Vermes-*
...
try:
    ...
    def get_vermes_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("VERMES_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".vermes").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$VERMES_HOME/logs/``."""
    return get_vermes_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$VERMES_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under VERMES_HOME or ``/tmp/Vermes-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    VERMES_home = get_vermes_home()
    try:
        path.resolve().relative_to(VERMES_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/Vermes-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("Vermes-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside VERMES_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under VERMES_HOME (but leave VERMES_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "Vermes-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(VERMES_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == VERMES_home:
                continue
            try:
                rel_parts = dirpath.relative_to(VERMES_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    VERMES_home = get_vermes_home()
    try:
        rel = path.resolve().relative_to(VERMES_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "Vermes-agent",
        }:
            return None
        ...
        # Path isn't under VERMES_HOME (e.g. /tmp/Vermes-*) — fall through.
        pass
