...
Auto-tracks and cleans up ephemeral files created during Vermes Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$VERMES_HOME` and `/tmp/vermes-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `VERMES_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `VERMES_HOME` or `/tmp/vermes-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$VERMES_HOME/disk-cleanup/` is itself excluded
- `$VERMES_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
