...
session-scoped state file under ``$VERMES_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $VERMES_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_vermes_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this Vermes install, leave it first —
    ...
    env = os.environ.copy()
    env["VERMES_MEET_URL"] = url
    env["VERMES_MEET_OUT_DIR"] = str(out)
    env["VERMES_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["VERMES_MEET_HEADED"] = "1"
    if auth_state:
        env["VERMES_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["VERMES_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if VERMES_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["VERMES_MEET_MODE"] = mode
    if realtime_model:
        env["VERMES_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["VERMES_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["VERMES_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["VERMES_MEET_REALTIME_KEY"] = realtime_api_key
