...
Let the vermes agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where vermes runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `vermes meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `vermes meet` CLI |
...
| `cli.py` | `vermes meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$VERMES_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `vermes meet node {run,list,approve,remove,status,ping}` |
...
vermes meet install                                      # pip + Chromium
vermes meet setup                                        # preflight
vermes meet auth                                         # optional
vermes meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
vermes meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.vermes/.env
vermes meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
vermes meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
vermes meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.vermes/.env
vermes meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, vermes will **not** switch your system audio input automatically — the
...
vermes plugins enable google_meet
vermes meet node run --display-name my-mac --host 0.0.0.0 --port 18789
