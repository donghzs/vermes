# Vermes Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: vermes-achievements plugin
...
### `GET /api/plugins/vermes-achievements/achievements`
...
### `POST /api/plugins/vermes-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/vermes-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/vermes-achievements/overview`
...
- Persist plugin data under:
  - `~/.vermes/plugins/vermes-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.vermes/plugins/vermes-achievements/scan_checkpoint.json`
...
- `~/.vermes/plugins/vermes-achievements/`
