...
``vermes_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
the in-browser PTY bridge in ``vermes_cli/web_server.py``.

This means ``Vermes dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def _compute_task_diagnostics(
    ...
    Uses ``vermes_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from vermes_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from vermes_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``VERMES_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from vermes_cli.kanban import _check_dispatcher_presence
...
# spawn failures, stuck-blocked). See vermes_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def specify_task_endpoint(
    ...
    prev_env = os.environ.get("VERMES_KANBAN_BOARD")
    try:
        os.environ["VERMES_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        # Import lazily so a missing auxiliary client at import time
        # doesn't break plugin load.
        from vermes_cli import kanban_specify  # noqa: WPS433 (intentional)
        ...
        if prev_env is None:
            ...
            os.environ["VERMES_KANBAN_BOARD"] = prev_env
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            ...
            profile_dir = _Path(get_vermes_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``Vermes profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from vermes_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    try:
        os.environ["VERMES_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        from vermes_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in vermes_cli/web_server.py.
