...
TOOLSETS = {
    ...
    "Vermes-cron": {
        ...
        "description": "Default cron toolset - same core tools as Vermes-cli; gated by `vermes tools`",
        "tools": _vermes_CORE_TOOLS,
        "includes": []
    },
    ...
    "Vermes-webhook": {
        ...
        "tools": _vermes_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "vermes-gateway": {
        ...
        "tools": [],
        "includes": ["Vermes-telegram", "Vermes-discord", "Vermes-whatsapp", "Vermes-slack", "Vermes-signal", "Vermes-bluebubbles", "Vermes-homeassistant", "Vermes-email", "Vermes-sms", "Vermes-mattermost", "Vermes-matrix", "Vermes-dingtalk", "Vermes-feishu", "Vermes-wecom", "Vermes-wecom-callback", "Vermes-weixin", "Vermes-qqbot", "Vermes-webhook", "Vermes-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (Vermes-<name>).
        # Gives them _vermes_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("Vermes-"):
            platform_name = name[len("Vermes-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_vermes_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
