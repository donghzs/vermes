#!/usr/bin/env python3
"""
Vermes CLI - Interactive Terminal Interface

A beautiful command-line interface for the Vermes, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills Vermes-agent-dev,github-auth
...
# C1 god-file split: pure text/reasoning helpers live in cli_text_helpers
# (no CLI-internal state). Re-import so existing call sites are unchanged.
from cli_text_helpers import (
    _REASONING_TAGS,
...
os.environ["VERMES_QUIET"] = "1"  # Our own modules
...
try:
    from vermes_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
from vermes_cli.banner import _format_context_length, format_banner_version_label
...
# NOTE: ``_REASONING_TAGS`` / ``_strip_reasoning_tags`` /
# ``_assistant_content_as_text`` / ``_assistant_copy_text`` were extracted to
# ``cli_text_helpers.py`` (C1 god-file split). They are re-imported at the top
# of this module so all call sites below are unchanged. The reasoning-tag
# stripping logic is duplicated in ``agent.agent_runtime_helpers.strip_think_blocks``
# (used by run_agent.py) — prefer consolidating onto that shared helper rather
# than diverging.
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.vermes/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If VERMES_IGNORE_USER_CONFIG=1 is set (via ``Vermes chat --ignore-user-config``),
    the user config at ``~/.vermes/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({VERMES_HOME}/config.yaml)
    user_config_path = _vermes_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("VERMES_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Vermes, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Vermes - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and VermesCLI.__init__ checks "default" first.
            ...
            # location is model.provider (written by `vermes model`).
    ...
    # Normalize config key: the new config system (vermes_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && Vermes` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _vermes_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_vermes_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["VERMES_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.vermes/logs/.
...
try:
    from vermes_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``Vermes --help`` paths inside the chat command flow).  See
...
from vermes_cli.banner import build_welcome_banner
from vermes_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from vermes_cli.callbacks import prompt_for_secret
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        logger.info("\033[31m✗ --worktree requires being inside a git repository.\033[0m")
        logger.info("  cd into your project repo first, then run Vermes -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"Vermes-{short_id}"
    branch_name = f"Vermes/{wt_name}"
    ...
    try:
        ...
        if result.returncode != 0:
            logger.warning(f"\033[31m✗ Failed to create worktree: {result.stderr.strip()}\033[0m")
            return None
    except Exception as e:
        logger.info(f"\033[31m✗ Failed to create worktree: {e}\033[0m")
        return None
    ...
    logger.info(f"\033[32m✓ Worktree created:\033[0m {wt_path}")
...
def _cleanup_worktree(info: Dict[str, str] = None) -> None:
    ...
    if has_unpushed:
        ...
        logger.info(f"  To clean up manually: git worktree remove --force {wt_path}")
    ...
    _active_worktree = None
    logger.info(f"\033[32m✓ Worktree cleaned up: {wt_path}\033[0m")
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`vermes_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from vermes_constants import get_vermes_home as _get_vermes_home
        _vermes_home_maint = _get_vermes_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_vermes_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`vermes_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``Vermes/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("Vermes-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``Vermes/Vermes-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``Vermes -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("Vermes/Vermes-") or b.startswith("pr-"))
    ]
...
# D3a: pure render helpers extracted to cli_render_helpers.py
# (imported here so all existing call sites remain unchanged)
from cli_render_helpers import (  # noqa: F401
...
# D3c: command handlers extracted from this module to reduce file size.
from cli_command_handlers import (  # noqa: F401
    handle_cron_command,
...
#
# Detection priority:
#   1. VERMES_LIGHT / VERMES_TUI_LIGHT env (true/false) — explicit override
#   2. VERMES_TUI_THEME=light|dark — explicit theme
#   3. VERMES_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Vermes assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("VERMES_LIGHT", "VERMES_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("VERMES_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("VERMES_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from vermes_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_vermes_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._vermes_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
# _SkinAwareAnsi moved to cli_render_helpers.py (imported above)
...
def _cprint(text: str):
    ...
    Raw ANSI escapes written via logger.info() are swallowed by patch_stdout's
...
from vermes_constants import is_termux as _is_termux_environment
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``ChatConsole()``, which historically only implemented ``logger.info()``.
        ...
        ``VermesCLI._busy_command()``.
        """
        yield self

# ASCII Art - Vermes-AGENT logo (full width, single line - requires ~95 char terminal)
VERMES_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - Vermes Caduceus (compact, fits in left panel)
VERMES_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS Vermes - AI Agent Framework"
        tiny_line = "⚕ NOUS Vermes"
    else:
        agent_name = _skin.get_branding("agent_name", "Vermes") if _skin else "Vermes"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from vermes_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.vermes/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.vermes/config.yaml on first use)
...
class VermesCLI:
    """
    Interactive CLI for the Vermes.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from vermes_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("VERMES_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("VERMES_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("VERMES_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `Vermes chat --ignore-rules` in vermes_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("VERMES_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("VERMES_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Vermes processes for this VERMES_HOME.
        ...
        self._history_file = _vermes_home / ".VERMES_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from vermes_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            yolo_active = bool(os.getenv("VERMES_YOLO_MODE"))
            if width < 52:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Vermes'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from vermes_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from vermes_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from vermes_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from vermes_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Vermes")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - VermesCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                logger.info("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: vermes setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            logger.info("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: vermes setup")
            return False
        ...
        # `Vermes chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `vermes auth add openai-codex`
        # without `vermes model`), fall back to the provider's first catalog
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (Vermes sessions list).{_RST}")
                return False
        ...
        try:
            ...
            _effective_ephemeral = (
                (self.system_prompt + "\n\n" + _evo_block)
                if self.system_prompt and _evo_block
                else (_evo_block if _evo_block else self.system_prompt)
            )
            ...
            self.agent = AIAgent(
                ...
                ephemeral_system_prompt=_effective_ephemeral,
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``vermes doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``vermes doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   Vermes needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        # Warn if the configured model is a Nous Vermes LLM (not agentic)
        from vermes_cli.model_switch import is_nous_vermes_non_agentic
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Vermes: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Vermes: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.vermes/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from vermes_cli.clipboard import save_clipboard_image

        img_dir = get_vermes_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not hasattr(self, 'agent') or not self.agent:
            logger.info("  No active agent session.")
            return

        mgr = self.agent._checkpoint_mgr
        if not mgr.enabled:
            logger.info("  Checkpoints are not enabled.")
            logger.info("  Enable with: Vermes --checkpoints")
            logger.info("  Or in config.yaml: checkpoints: { enabled: true }")
            return
        ...
        if not args:
            ...
            logger.info(format_checkpoint_list(checkpoints, cwd))
            return

        # Handle /rollback diff <N>
        if args[0].lower() == "diff":
            if len(args) < 2:
                logger.info("  Usage: /rollback diff <N>")
                return
            checkpoints = mgr.list_checkpoints(cwd)
            if not checkpoints:
                logger.info(f"  No checkpoints found for {cwd}")
                return
            ...
            if result["success"]:
                ...
                if not stat and not diff:
                    logger.info("  No changes since this checkpoint.")
                else:
                    if stat:
                        logger.info(f"\n{stat}")
                    if diff:
        ...
        if result["success"]:
            if file_path:
                logger.info(f"  ✅ Restored {file_path} from checkpoint {result['restored_to']}: {result['reason']}")
            else:
                logger.info(f"  ✅ Restored to checkpoint {result['restored_to']}: {result['reason']}")
            logger.info("  A pre-rollback snapshot was saved automatically.")

            # Also undo the last conversation turn so the agent's context
            # matches the restored filesystem state
            if self.conversation_history:
                self.undo_last()
                logger.info("  Chat turn undone to match restored file state.")
        else:
            logger.info(f"  ❌ {result['error']}")
    ...
    def _resolve_checkpoint_ref(self, ref: str, checkpoints: list) -> str | None:
        ...
        try:
            ...
            if 0 <= idx < len(checkpoints):
                ...
                logger.info(f"  Invalid checkpoint number. Use 1-{len(checkpoints)}.")
                return None
        except ValueError:
            # Treat as a git hash
            return ref
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Vermes config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                logger.info("  No state snapshots yet.")
                logger.info("  Create one: /snapshot create [label]")
                return
            logger.info(f"  State snapshots ({display_vermes_home()}/state-snapshots/):\n")
            logger.info(f"  {'#':>3}  {'ID':<35} {'Files':>5} {'Size':>10} {'Label'}")
            logger.info(f"  {'─'*3}  {'─'*35} {'─'*5} {'─'*10} {'─'*20}")
            for i, s in enumerate(snaps, 1):
                ...
                logger.info(f"  {i:3}  {s['id']:<35} {s.get('file_count', 0):>5} {size_str:>10} {label}")
            ...
            if len(parts) < 3:
                logger.info("  Usage: /snapshot restore <snapshot-id>")
                # Show hint with most recent snapshot
                snaps = list_quick_snapshots(limit=1)
                if snaps:
                    logger.info(f"  Most recent: {snaps[0]['id']}")
                return
            snap_id = parts[2]
            ...
            try:
                ...
                if 1 <= idx <= len(snaps):
                    ...
                    logger.info(f"  Invalid snapshot number. Use 1-{len(snaps)}.")
                    return
            except ValueError:
                pass
            if restore_quick_snapshot(snap_id):
                logger.info(f"  Restored state from: {snap_id}")
                logger.info("  Restart recommended for state.db changes to take effect.")
            else:
                logger.info(f"  Snapshot not found: {snap_id}")
            ...
            if len(parts) > 2:
                try:
                    ...
                    logger.info("  Usage: /snapshot prune [keep-count]")
                    return
            deleted = prune_quick_snapshots(keep=keep)
            logger.info(f"  Pruned {deleted} old snapshot(s) (keeping {keep}).")

        else:
            logger.info(f"  Unknown subcommand: {subcmd}")
            logger.info("  Usage: /snapshot [list|create [label]|restore <id>|prune [N]]")
    ...
    def _handle_stop_command(self):
        ...
        if not running:
            logger.info("  No running background processes.")
            return

        logger.info(f"  Stopping {len(running)} background process(es)...")
        killed = process_registry.kill_all()
        logger.info(f"  ✅ Stopped {killed} process(es).")
    ...
    def _handle_paste_command(self):
        ...
        from vermes_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run Vermes chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'vermes setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from vermes_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from vermes_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Vermes!{_RST}")
    ...
    def show_tools(self):
        ...
        if not tools:
            logger.info("(;_;) No tools available")
            return
        ...
        pad = width - len(title)
        logger.info("+" + "-" * width + "+")
        logger.info("|" + " " * (pad // 2) + title + " " * (pad - pad // 2) + "|")
        logger.info("+" + "-" * width + "+")
        logger.info()
        ...
        for toolset in sorted(toolsets.keys()):
            logger.info(f"  [{toolset}]")
            for name, desc in toolsets[toolset]:
                logger.info(f"    * {name:<20} - {desc}")
            logger.info()
        
        logger.info(f"  Total: {len(tools)} tools  ヽ(^o^)ノ")
        logger.info()
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            logger.info() output through _cprint when inside the interactive TUI
            ...
            # Buffer reports isatty()=True so color() in vermes_cli/colors.py
        ...
        names = parts[2:]
        if not names:
            logger.info(f"(._.) Usage: /tools {subcommand} <name> [name ...]")
            logger.info(f"  Built-in toolset:  /tools {subcommand} web")
            logger.info(f"  MCP tool:          /tools {subcommand} github:create_issue")
            return
    ...
    def show_toolsets(self):
        ...
        for name in sorted(all_toolsets.keys()):
            ...
            if info:
                ...
                logger.info(f"  {marker} {name:<18} [{tool_count:>2} tools] - {desc}")
        
        logger.info()
        logger.info("  (*) = currently enabled")
        logger.info()
        logger.info("  Tip: Use 'all' or '*' to enable all toolsets")
        logger.info("  Example: python cli.py --toolsets web,terminal")
        logger.info()
    ...
    def show_config(self):
        ...
        logger.info(f"  Environment:  {terminal_env}")
        if terminal_env == "ssh":
        ...
        logger.info(f"  Working Dir:  {terminal_cwd}")
        logger.info(f"  Timeout:      {terminal_timeout}s")
        logger.info()
        logger.info("  -- Agent --")
        logger.info(f"  Max Turns:  {self.max_turns}")
        logger.info(f"  Toolsets:   {', '.join(self.enabled_toolsets) if self.enabled_toolsets else 'all'}")
        logger.info(f"  Verbose:    {self.verbose}")
        logger.info()
        logger.info("  -- Session --")
        logger.info(f"  Started:     {self.session_start.strftime('%Y-%m-%d %H:%M:%S')}")
        logger.info(f"  Config File: {config_path} {config_status}")
        logger.info()
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from vermes_cli.main import _relative_time

        logger.info()
        if reason == "history":
            logger.info("(._.) No messages in the current chat yet — here are recent sessions you can resume:")
        else:
            logger.info("  Recent sessions:")
        logger.info()
        logger.info(f"  {'Title':<32} {'Preview':<40} {'Last Active':<13} {'ID'}")
        logger.info(f"  {'─' * 32} {'─' * 40} {'─' * 13} {'─' * 24}")
        for session in sessions:
            title = (session.get("title") or "—")[:30]
            preview = (session.get("preview") or "")[:38]
            last_active = _relative_time(session.get("last_active"))
            logger.info(f"  {title:<32} {preview:<40} {last_active:<13} {session['id']}")
        logger.info()
        logger.info("  Use /resume <session id or title> to continue where you left off.")
        logger.info()
        return True
    ...
    def show_history(self):
        ...
        if not self.conversation_history:
            if not self._show_recent_sessions(reason="history"):
                logger.info("(._.) No conversation history yet.")
            return
        ...
        def flush_tool_summary():
            ...
            logger.info("\n  [Tools]")
            logger.info(f"    ({hidden_tool_messages} tool {noun} hidden)")
            hidden_tool_messages = 0

        logger.info()
        logger.info("+" + "-" * 50 + "+")
        logger.info("|" + " " * 12 + "(^_^) Conversation History" + " " * 11 + "|")
        logger.info("+" + "-" * 50 + "+")
    ...
    def new_session(self, silent=False, title=None):
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("VERMES_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
        ...
        if not silent:
            if title:
                logger.info(f"(^_^)v New session started: {title}")
            else:
                logger.info("(^_^)v New session started!")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `Vermes sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from vermes_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `Vermes sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.vermes/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``Vermes --resume <id>``
        regardless of whether the user ever runs ``/save``.
        """
        if not self.conversation_history:
            logger.info("(;_;) No conversation to save.")
            return

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        saved_dir = get_vermes_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            logger.info(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"VERMES_conversation_{timestamp}.json"
        ...
        try:
            ...
            logger.info(f"(^_^)v Conversation snapshot saved to: {path}")
            if self.session_id:
                logger.info(f"       Resume the live session with: Vermes --resume {self.session_id}")
        except Exception as e:
            logger.info(f"(x_x) Failed to save: {e}")
    ...
    def retry_last(self):
        ...
        """
        if not self.conversation_history:
            logger.info("(._.) No messages to retry.")
            return None
        ...
        if last_user_idx is None:
            logger.info("(._.) No user message found to retry.")
            return None
        ...
        logger.info(f"(^_^)b Retrying: \"{last_message[:60]}{'...' if len(last_message) > 60 else ''}\"")
        return last_message
    ...
    def undo_last(self):
        ...
        """
        if not self.conversation_history:
            logger.info("(._.) No messages to undo.")
            return
        ...
        if last_user_idx is None:
            logger.info("(._.) No user message found to undo.")
            return
        ...
        logger.info(f"(^_^)b Undid {removed_count} message(s). Removed: \"{removed_msg[:60]}{'...' if len(removed_msg) > 60 else ''}\"")
        remaining = len(self.conversation_history)
        logger.info(f"  {remaining} message(s) remaining in history.")
    
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        """Run curses_single_select via run_in_terminal so prompt_toolkit handles terminal ownership cleanly."""
        import threading
        from vermes_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `vermes model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from vermes_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from vermes_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from vermes_cli.model_switch import switch_model, parse_model_flags
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Vermes default (chat_completions)
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from vermes_cli.commands import resolve_command
        ...
        if len(parts) > 1:
            ...
            if personality_name in {"none", "default", "neutral"}:
                ...
                if save_config_value("agent.system_prompt", ""):
                    logger.info("(^_^)b Personality cleared (saved to config)")
                else:
                    logger.info("(^_^) Personality cleared (session only)")
                logger.info("  No personality overlay — using base agent behavior.")
            elif personality_name in self.personalities:
                self.system_prompt = self._resolve_personality_prompt(self.personalities[personality_name])
                self.agent = None  # Force re-init
                if save_config_value("agent.system_prompt", self.system_prompt):
                    logger.info(f"(^_^)b Personality set to '{personality_name}' (saved to config)")
                else:
                    logger.info(f"(^_^) Personality set to '{personality_name}' (session only)")
                logger.info(f"  \"{self.system_prompt[:60]}{'...' if len(self.system_prompt) > 60 else ''}\"")
            else:
                logger.info(f"(._.) Unknown personality: {personality_name}")
                logger.info(f"  Available: none, {', '.join(self.personalities.keys())}")
        else:
            # Show available personalities
            logger.info()
            logger.info("+" + "-" * 50 + "+")
            logger.info("|" + " " * 12 + "(^o^)/ Personalities" + " " * 15 + "|")
            logger.info("+" + "-" * 50 + "+")
            logger.info()
            logger.info(f"  {'none':<12} - (no personality overlay)")
            for name, prompt in self.personalities.items():
                if isinstance(prompt, dict):
                    preview = prompt.get("description") or prompt.get("system_prompt", "")[:50]
                else:
                    preview = str(prompt)[:50]
                logger.info(f"  {name:<12} - {preview}")
            logger.info()
            logger.info("  Usage: /personality <name>")
            logger.info()
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to vermes_cli.curator so the CLI and the `Vermes curator`
        ...
        try:
            from vermes_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from vermes_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to vermes_cli.skills_hub."""
        from vermes_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        logger.info()
        logger.info("+" + "-" * 60 + "+")
        logger.info("|" + " " * 15 + "(✿◠‿◠) Gateway Status" + " " * 17 + "|")
        logger.info("+" + "-" * 60 + "+")
        logger.info()
        ...
        try:
            ...
            logger.info("  Messaging Platform Configuration:")
            logger.info("  " + "-" * 55)
            ...
            for platform, (name, env_var) in platform_status.items():
                ...
                if pconfig and pconfig.enabled:
                    ...
                    logger.info(f"    ✓ {name:<12} Enabled{home_str}")
                else:
                    logger.info(f"    ○ {name:<12} Not configured ({env_var})")
            
            logger.info()
            logger.info("  Session Reset Policy:")
            logger.info("  " + "-" * 55)
            policy = config.default_reset_policy
            logger.info(f"    Mode: {policy.mode}")
            logger.info(f"    Daily reset at: {policy.at_hour}:00")
            logger.info(f"    Idle timeout: {policy.idle_minutes} minutes")
            
            logger.info()
            logger.info("  To start the gateway:")
            logger.info("    python cli.py --gateway")
            logger.info()
            logger.info(f"  Configuration file: {display_vermes_home()}/config.yaml")
            logger.info()
            
        except Exception as e:
            logger.info(f"  Error loading gateway config: {e}")
            logger.info()
            logger.info("  To configure the gateway:")
            logger.info("    1. Set environment variables:")
            logger.info("       TELEGRAM_BOT_TOKEN=your_token")
            logger.info("       DISCORD_BOT_TOKEN=your_token")
            logger.info(f"    2. Or configure settings in {display_vermes_home()}/config.yaml")
            logger.info()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in vermes_cli/commands.py instead of touching every dispatch site.
        from vermes_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                self.show_banner()
                logger.info("  ✨ (◕‿◕)✨ Fresh start! Screen cleared and conversation reset.\n")
                # Show a random tip on new session
                try:
                    from vermes_cli.tips import get_random_tip
                    _tip = get_random_tip()
            ...
            from vermes_cli.config import reload_env
            count = reload_env()
            logger.info(f"  Reloaded .env ({count} var(s) updated)")
            ...
            try:
                ...
                if not plugins:
                    ...
                    logger.info(f"Drop plugin directories into {display_vermes_home()}/plugins/ to get started.")
                else:
                    logger.info(f"Plugins ({len(plugins)}):")
                    for p in plugins:
                        ...
                        logger.info(f"  {status} {p['name']}{version}{detail}{error}")
            except Exception as e:
                logger.info(f"Plugin system error: {e}")
            ...
            # Check for plugin-registered slash commands
                ...
                from vermes_cli.plugins import (
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from vermes_cli.commands import COMMANDS
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``Vermes bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: Vermes bundles create "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Vermes keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from vermes_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
            logger.info("Skin engine not available.")
            return
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            logger.info("  Available skins:")
            for s in skins:
                marker = " ●" if s["name"] == current else "  "
                source = f" ({s['source']})" if s["source"] == "user" else ""
                logger.info(f"   {marker} {s['name']}{source} — {s['description']}")
            logger.info("\n  Usage: /skin <name>")
            logger.info(f"  Custom skins: drop a YAML file in {display_vermes_home()}/skins/\n")
            return

        new_skin = parts[1].strip().lower()
        available = {s["name"] for s in list_skins()}
        if new_skin not in available:
            logger.info(f"  Unknown skin: {new_skin}")
            logger.info(f"  Available: {', '.join(sorted(available))}")
            return
        ...
        if save_config_value("display.skin", new_skin):
            logger.info(f"  Skin set to: {new_skin} (saved)")
        else:
            logger.info(f"  Skin set to: {new_skin}")
        logger.info("  Note: banner colors will update on next session start.")
        if self._apply_tui_skin_style():
            logger.info("  Prompt + TUI colors updated.")
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from vermes_cli.config import load_config
        from vermes_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = is_truthy_value(os.environ.get("VERMES_YOLO_MODE"))
        if current:
            os.environ.pop("VERMES_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Vermes is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Vermes is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Vermes is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from vermes_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        if not self.conversation_history or len(self.conversation_history) < 4:
            logger.info("(._.) Not enough conversation to compress (need at least 4 messages).")
            return

        if not self.agent:
            logger.info("(._.) No active agent -- send a message first.")
            return

        if not self.agent.compression_enabled:
            logger.info("(._.) Compression is disabled in config.")
            return
        ...
        with self._busy_command("Compressing context..."):
            try:
                ...
                if focus_topic:
                    logger.info(f"🗜️  Compressing {original_count} messages (~{approx_tokens:,} tokens), "
                          f"focus: \"{focus_topic}\"...")
                else:
                    logger.info(f"🗜️  Compressing {original_count} messages (~{approx_tokens:,} tokens)...")
                ...
                logger.info(f"  {icon} {summary['headline']}")
                logger.info(f"     {summary['token_line']}")
                if summary["note"]:
                    logger.info(f"     {summary['note']}")

            except Exception as e:
                logger.info(f"  ❌ Compression failed: {e}")

    def _handle_debug_command(self):
        """Handle /debug — upload debug report + logs and print paste URLs."""
        from vermes_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Vermes to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``vermes update`` so the user sees update output directly and gets
        ...
        if is_managed():
            logger.info(f"  ✗ {format_managed_message('update Vermes')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Vermes"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Vermes",
            detail="This will exit the current session and run `vermes update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _show_usage(self):
        ...
        if calls == 0:
            logger.info("(._.) No API calls made yet in this session.")
            return
        ...
        if rl_state and rl_state.has_data:
            ...
            logger.info()
            logger.info(format_rate_limit_display(rl_state))
            logger.info()
        ...
        logger.info(f"  Input tokens:              {input_tokens:>10,}")
        logger.info(f"  Cache read tokens:         {cache_read_tokens:>10,}")
        logger.info(f"  Cache write tokens:        {cache_write_tokens:>10,}")
        logger.info(f"  Output tokens:             {output_tokens:>10,}")
        if reasoning_tokens:
            logger.info(f"  ↳ Reasoning (subset):      {reasoning_tokens:>10,}")
        logger.info(f"  Prompt tokens (total):     {prompt:>10,}")
        logger.info(f"  Completion tokens:         {completion:>10,}")
        logger.info(f"  Total tokens:              {total:>10,}")
        logger.info(f"  API calls:                 {calls:>10,}")
        logger.info(f"  Session duration:          {elapsed:>10}")
        logger.info(f"  Cost status:              {cost_result.status:>10}")
        logger.info(f"  Cost source:              {cost_result.source:>10}")
        if cost_result.amount_usd is not None:
            prefix = "~" if cost_result.status == "estimated" else ""
            logger.info(f"  Total cost:              {prefix}${float(cost_result.amount_usd):>10.4f}")
        elif cost_result.status == "included":
            logger.info(f"  Total cost:              {'included':>10}")
        else:
            logger.info(f"  Total cost:              {'n/a':>10}")
        logger.info(f"  {'─' * 40}")
        logger.info(f"  Current context:  {last_prompt:,} / {ctx_len:,} ({pct:.0f}%)")
        logger.info(f"  Messages:         {msg_count}")
        logger.info(f"  Compressions:     {compressions}")
        if cost_result.status == "unknown":
            logger.info(f"  Note:             Pricing unknown for {agent.model}")
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by vermes_logging not
        ...
        while i < len(parts):
            if parts[i] == "--days" and i + 1 < len(parts):
                try:
                    ...
                    logger.info(f"  Invalid --days value: {parts[i + 1]}")
                    return
                i += 2
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from vermes_cli.config import get_config_path as _get_config_path
        ...
        if _reload_thread.is_alive():
            logger.info("  ⚠️  MCP reload timed out (30s). Some servers may not have reconnected.")
    ...
    def _confirm_destructive_slash(self, command: str, detail: str) -> Optional[str]:
        ...
        if raw is None:
            logger.info(f"🟡 /{command} cancelled (no input).")
            return None
        choice = self._normalize_slash_confirm_choice(raw, choices)
        if choice is None:
            logger.info(f"🟡 Unrecognized choice '{raw}'. /{command} cancelled.")
            return None

        if choice == "cancel":
            logger.info(f"🟡 /{command} cancelled. Conversation unchanged.")
            return None

        if choice == "always":
            if save_config_value("approvals.destructive_slash_confirm", False):
                logger.info("🔒 Future /clear, /new, /reset, and /undo will run without confirmation.")
                logger.info("   Re-enable via `approvals.destructive_slash_confirm: true` in config.yaml.")
            else:
                logger.info("⚠️  Couldn't persist opt-out — proceeding once.")
    ...
    def _confirm_and_reload_mcp(self, cmd_original: str = "") -> None:
        ...
        if raw is None:
            logger.info("🟡 /reload-mcp cancelled (no input).")
            return
        choice = self._normalize_slash_confirm_choice(raw, choices)
        if choice is None:
            logger.info(f"🟡 Unrecognized choice '{raw}'. /reload-mcp cancelled.")
            return

        if choice == "cancel":
            logger.info("🟡 /reload-mcp cancelled. MCP tools unchanged.")
            return

        if choice == "always":
            if save_config_value("approvals.mcp_reload_confirm", False):
                logger.info("🔒 Future /reload-mcp calls will run without confirmation.")
                logger.info("   Re-enable via `approvals.mcp_reload_confirm: true` in config.yaml.")
            else:
                logger.info("⚠️  Couldn't persist opt-out — reloading once.")
    ...
    def _reload_mcp(self):
        ...
        try:
            ...
            if reconnected:
                logger.info(f"  ♻️  Reconnected: {', '.join(sorted(reconnected))}")
            if added:
            ...
            if not connected_servers:
                ...
                logger.info(f"  🔧 {len(new_tools)} tool(s) available from {len(connected_servers)} server(s)")
            ...
            logger.info(f"  ✅ Agent updated — {len(self.agent.tools if self.agent else [])} tool(s) available")

        except Exception as e:
            logger.info(f"  ❌ MCP reload failed: {e}")

    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.vermes/skills/ and queue a note for the
        next user turn.
        ...
        try:
            ...
            logger.info(f"  ❌ Skills reload failed: {e}")
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_vermes_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "VERMES_voice"), exist_ok=True)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _vermes_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            logger.info(f"  Vermes --resume {self.session_id}")
            if session_title:
                logger.info(f"  Vermes -c \"{session_title}\"")
            logger.info()
            logger.info(f"Session:        {self.session_id}")
            if session_title:
                logger.info(f"Title:          {session_title}")
            logger.info(f"Duration:       {duration_str}")
            logger.info(f"Messages:       {msg_count} ({user_msgs} user, {tool_calls} tool calls)")
        else:
            try:
                from vermes_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from vermes_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from vermes_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Vermes! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Vermes! Type your message or /help for commands."
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(VERMES_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Vermes migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from vermes_cli.config import get_config_path as _get_cfg_path_resid
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_vermes_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Vermes binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from vermes_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Vermes")
        ...
        # Dynamic prompt: shows Vermes symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Vermes needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Vermes needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Vermes needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = VermesCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_vermes_osd_patched", False):
                ...
                _pt_renderer._vermes_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``VERMES_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("VERMES_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the Vermes process when
        ...
        try:
            ...
            logger.info(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: vermes setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    Vermes CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        logger.info("Starting Vermes Gateway (messaging platforms)...")
    ...
    # Default to Vermes-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside VermesCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  VERMES_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Vermes" box, no tool-gen
            ...
            # Single-query mode (`Vermes chat -q "…"`): skip the welcome
