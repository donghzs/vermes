"""Centralized logging setup for Vermes Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.vermes/logs/`` (profile-aware via ``get_vermes_home()``).
...
from vermes_constants import get_config_path, get_vermes_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_vermes_session_injector", False):
    ...
    _session_record_factory._vermes_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``Vermes logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "VERMES_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Vermes logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    VERMES_home
        Override for the Vermes home directory.  Falls back to
        ``get_vermes_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_vermes_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._vermes_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the Vermes group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from vermes_cli.config import is_managed
