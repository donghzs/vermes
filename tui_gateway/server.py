...
load_vermes_dotenv(
    VERMES_home=_vermes_home, project_env=Path(__file__).parent.parent / ".env"
)
...
_CRASH_LOG = os.path.join(_vermes_home, "logs", "tui_gateway_crash.log")
...
_cfg_path = None
try:
    _slash_timeout = float(os.environ.get("VERMES_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("VERMES_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `Vermes` shows the prompt before constructing AIAgent; the TUI used
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        p = _vermes_home / "config.yaml"
        mtime = p.stat().st_mtime if p.exists() else None
        with _cfg_lock:
            if _cfg_cache is not None and _cfg_mtime == mtime and _cfg_path == p:
                return copy.deepcopy(_cfg_cache)
        data = load_roundtrip_yaml(p) if p.exists() else {}
        with _cfg_lock:
...
def _save_cfg(cfg: dict):
    ...
    path = _vermes_home / "config.yaml"
    atomic_roundtrip_yaml_dump(path, cfg)
    with _cfg_lock:
...
def resolve_skin() -> dict:
    try:
        from vermes_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("VERMES_MODEL", "")
        or os.environ.get("VERMES_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("VERMES_TUI_PROVIDER", "").strip()
    ...
    try:
        from vermes_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("VERMES_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("VERMES_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("VERMES_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                logger.info(
                    "[tui] VERMES_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from vermes_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            logger.info(
                f"[tui] ignoring unknown VERMES_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
            )
        if disabled:
            logger.info(
                "[tui] ignoring disabled MCP servers in VERMES_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid VERMES_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            logger.info(
                "[tui] no valid VERMES_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
            )
        return None
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from vermes_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from vermes_cli.config import get_compatible_custom_providers, load_config
    ...
    os.environ["VERMES_MODEL"] = result.new_model
    os.environ["VERMES_INFERENCE_MODEL"] = result.new_model
    ...
    # in config or env.
    #
    # VERMES_TUI_PROVIDER is the canonical "explicit-this-process" carrier
    ...
    if result.target_provider:
        os.environ["VERMES_INFERENCE_PROVIDER"] = result.target_provider
        os.environ["VERMES_TUI_PROVIDER"] = result.target_provider
    if persist_global:
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from vermes_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from vermes_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from vermes_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from vermes_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("VERMES_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("VERMES_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _make_agent(sid: str, key: str, session_id: str | None = None):
    ...
    # ── 跨会话涌现：注入进化上下文（提炼至 evolution_manager）──
    _evo_ephemeral = ""
    try:
        from agent.evolution_manager import build_evolution_prompt
        _evo_ephemeral = build_evolution_prompt() or ""
    except Exception:
        pass  # 进化上下文注入非阻塞
    ...
    return AIAgent(
        ...
        ephemeral_system_prompt=(
            (system_prompt + "\n\n" + _evo_ephemeral)
            if system_prompt and _evo_ephemeral
            else (_evo_ephemeral if _evo_ephemeral else system_prompt)
        ) or None,
        checkpoints_enabled=is_truthy_value(os.environ.get("VERMES_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("VERMES_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("VERMES_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("VERMES_IGNORE_RULES")),
        **_agent_cbs(sid),
    )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `VERMES_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    filename = os.path.abspath(
        f"VERMES_conversation_{_time.strftime('%Y%m%d_%H%M%S')}.json"
    )
    try:
...
#
# Layout:  $VERMES_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_vermes_home() / "spawn-trees"
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from vermes_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from vermes_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: vermes_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from vermes_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _vermes_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from vermes_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_vermes_home), "display": display_vermes_home()})
    if key == "full":
    ...
    if key == "mouse":
        ...
        return _ok(rid, {"value": "on" if on else "off"})
    if key == "mtime":
        cfg_path = _vermes_home / "config.yaml"
        try:
            return _ok(
...
def _(rid, params: dict) -> dict:
    try:
        from vermes_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if not user_confirm:
            try:
                from vermes_cli.config import load_config as _load_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from vermes_cli.commands import (
            COMMAND_REGISTRY,
            SUBCOMMANDS,
            _build_description,
        )
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `Vermes` is interactive — use `/Vermes chat -q …` or run `Vermes` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
    ...
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`Vermes sessions browse` is interactive — use /resume here, or run browse in another terminal"
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m vermes_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "vermes_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from vermes_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from vermes_cli.config import is_managed, save_env_value
        from vermes_cli.inventory import build_models_payload, load_picker_context
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from vermes_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from vermes_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("VERMES_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("VERMES_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["VERMES_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from vermes_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["VERMES_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from vermes_cli.voice import start_continuous
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from vermes_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from vermes_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("VERMES_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("VERMES_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_vermes_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "browse":
            from vermes_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from vermes_cli.skills_hub import inspect_skill
