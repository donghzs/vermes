...
from vermes_constants import get_config_path, get_skills_dir, is_termux
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``VERMES_PLATFORM`` or
            ``VERMES_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``Vermes`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    parsing a non-trivial config dominates ``Vermes`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against VERMES_HOME, not cwd
        if not p.is_absolute():
            p = (VERMES_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    return {
        ...
        "fallback_for_tools": Vermes.get("fallback_for_tools", []),
        "requires_tools": Vermes.get("requires_tools", []),
    }
...
def resolve_skill_config_values(
    ...
    if config_path.exists():
        try:
            ...
            logger.debug("skill_utils.py: resolve skill config values failed: %s", e)
