...
Owns one Codex thread per Vermes session. Drives `turn/start`, consumes
...
class CodexAppServerSession:
    """One Codex thread per Vermes session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Vermes' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Vermes' approval
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Vermes approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Vermes returns 'once', 'session', 'always', or 'deny'.
