...
A pre-run snapshot of ``~/.vermes/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.vermes/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.vermes/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.vermes/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.vermes/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.vermes/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.vermes/skills/``.
