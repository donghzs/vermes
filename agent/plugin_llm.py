...
Plugins built on Vermes Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Vermes
...
:class:`~vermes_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Vermes' main config
...
handles every provider, fallback chain, and per-task override Vermes
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`vermes_cli.plugins.PluginContext`
