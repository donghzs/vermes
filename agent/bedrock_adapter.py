"""AWS Bedrock Converse API adapter for Vermes Agent.
...
def _require_boto3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or install Vermes with Bedrock support: pip install -e '.[bedrock]'"
        )
...
def resolve_aws_auth_env_var(env: Optional[Dict[str, str]] = None) -> Optional[str]:
    ...
    try:
        ...
        logger.debug("bedrock_adapter.py: resolve aws auth env var failed: %s", e)
    return None
...
def has_aws_credentials(env: Optional[Dict[str, str]] = None) -> bool:
    ...
    try:
        ...
        logger.debug("bedrock_adapter.py: has aws credentials failed: %s", e)
    return False
...
def resolve_bedrock_region(env: Optional[Dict[str, str]] = None) -> str:
    ...
    try:
        ...
        logger.debug("bedrock_adapter.py: resolve bedrock region failed: %s", e)
    return "us-east-1"
...
def bedrock_model_ids_or_none() -> Optional[List[str]]:
    ...
    try:
        ...
        logger.debug("bedrock_adapter.py: bedrock model ids or none failed: %s", e)
    return None
