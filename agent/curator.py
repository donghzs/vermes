...
def _state_file() -> Path:
    return get_vermes_home() / "skills" / ".curator_state"
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    run it sooner can always invoke ``Vermes curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `Vermes curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    ...
    "to find: Vermes-config-*, Vermes-dashboard-*, gateway-*, codex-*, "
...
def _reports_root() -> Path:
    ...
    ``ensure_vermes_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_vermes_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every Vermes install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        keep an umbrella stable: Vermes curator pin document-tools
    ...
    lines.append("full report: Vermes curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: Vermes curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "safety and can be restored via `Vermes curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Restore any via `Vermes curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `Vermes curator restore <name>`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `Vermes curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `Vermes curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
