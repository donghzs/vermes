...
def _log_turn_exit(
    ...
    interrupted: bool,
    _turn_exit_reason: str | None,
    api_call_count: int,
) -> None:
    """Emit the turn-exit diagnostic log at INFO or WARNING level.

    When the last message is a tool result (agent was mid-work), logs at
    WARNING - this is the "just stops" scenario users report.
    """
...
def _apply_file_mutation_footer(
    ...
    """Append advisory footer for failed file mutations, if any.
...
def _any_keyword_in(text: str, keywords: set[str]) -> bool:
    """委托至 agent.claim_verifier.any_keyword_in（Phase 2.2 单一真相源）。"""
    from agent.claim_verifier import any_keyword_in as _impl
    return _impl(text, keywords)
...
def _apply_operator_claim_verifier(
    ...
    """Operator-chain verifier (Layer 1) — 委托至 agent.claim_verifier（Phase 2.2 单一真相源）。

    判定/拦截逻辑统一维护在 agent.claim_verifier.apply_operator_claim_verifier，
    此处仅作薄封装，保证 turn_finalizer 等既有调用方零改动。
    """
    from agent.claim_verifier import apply_operator_claim_verifier as _apply
    return _apply(agent, messages, final_response, interrupted)
...
def _prepare_api_messages(
    ...
    _ext_prefetch_cache: str | None,
    _plugin_user_context: str,
) -> list[dict]:
    """Prepare the messages list for an API call.

    Handles: /steer drain, tool-call argument sanitization, role
    alternation repair, ephemeral context injection (memory prefetch,
    plugin context, per-turn recall), reasoning content copying,
    system prompt + ephemeral prefill assembly, Anthropic prompt caching,
    orphaned tool result cleanup, thinking-only turn dropping, whitespace
    normalization, tool-call JSON canonicalization, and surrogate stripping.

    Modifies ``messages`` in place (steer drain may append to tool messages).
    Returns the prepared ``api_messages`` list (a copy of messages).
    """
...
def _initialize_turn(
    ...
    """Thin forwarder — logic lifted verbatim to
    ``agent.orchestration.turn_service.initialize_turn`` (A2 §7.5 stage 3,
    zero-behavior-change). Kept as a named local so all call sites inside
    ``run_conversation`` are unaffected.
    """
    from agent.orchestration.turn_service import initialize_turn
    return initialize_turn(
...
def _prepare_messages(
    ...
    """Thin forwarder — logic lifted verbatim to
    ``agent.orchestration.turn_service.prepare_messages`` (A2 §7.5 stage 3,
    zero-behavior-change). Kept as a named local so all call sites inside
    ``run_conversation`` are unaffected.
    """
    from agent.orchestration.turn_service import prepare_messages
...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        api_messages = _prepare_api_messages(
            agent, messages, active_system_prompt,
            current_turn_user_idx, _ext_prefetch_cache, _plugin_user_context,
        )
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if response_invalid:
                    ...
                    if thinking_spinner:
                        thinking_spinner.stop("( ́;ω;`) oops, retrying...")
                        thinking_spinner = None
                    if agent.thinking_callback:
                        agent.thinking_callback("")
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    from vermes_cli.scholarforge.tools import set_stream_callback
                    if agent.stream_delta_callback:
    ...
    # A2 §7.5 stage-3: route through the extracted turn_finalizer.finalize_turn
    # (was the inline _finalize_turn). Signature is keyword-only; args map 1:1
    # to the former inline call. `failed`/`turn_id` are legacy params the
    # inline path never set — default (False/None) preserves behavior.
