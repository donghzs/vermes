    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "中间轮次且不生成摘要。"
                    "请运行 `vermes setup` 或设置 OPENROUTER_API_KEY。"
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Vermes "
...
def compress_context(
    ...
    *,
    ...
    # long-lived ``vermes_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `vermes update`) to resync.",
    ...
    if agent._session_db:
        try:
            ...
            if in_place:
                ...
                # from ``vermes_logging._session_context`` (set once per turn in
                ...
                try:
                    from vermes_cli.goals import migrate_goal_to_session
...
def compaction_loop(
    ...
    Extracted from ``conversation_loop._compress_until_under_threshold`` as
    the first step of A2 CompactionEngine extraction.  The loop-level contract
    is locked by ``tests/agent/test_compress_until_under_threshold_contract.py``.
    """
