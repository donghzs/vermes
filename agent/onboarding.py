...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Vermes starts and finds ``~/.openclaw/``.

    Points users at ``Vermes claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``Vermes claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Vermes, run "
        "`Vermes claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `Vermes claw cleanup` (renames it to ~/.openclaw.pre-migration — "
