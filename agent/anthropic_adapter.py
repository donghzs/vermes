"""Anthropic Messages API adapter for Vermes Agent.

Translates between Vermes's internal OpenAI-style message format and
...
# Vermes effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Vermes historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Vermes)
    ...
    # 1. Vermes-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Vermes configs.
...
# ── Vermes-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.vermes/.anthropic_oauth.json (our own file).
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    logger.info()
    logger.info("Authorize Vermes with your Claude Pro/Max subscription.")
    logger.info()
    ...
    try:
        from vermes_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if _HERMES_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Vermes OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    # silently hides reasoning text that Vermes surfaces in its CLI. We
