...
Storage (``~/.vermes/auth/google_oauth.json``, chmod 0o600):
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    if creds is None:
        raise GoogleOAuthError(
            "No Google OAuth credentials found. Run `vermes auth add google-gemini-cli` first.",
...
<html><head><meta charset="utf-8"><title>Vermes — signed in</title>
<style>
...
<html><head><meta charset="utf-8"><title>Vermes — sign-in failed</title>
<style>
body {{ font: 16px/1.5 system-ui, sans-serif; margin: 10vh auto; max-width: 32rem; text-align: center; color: #222; }}
h1 {{ color: #b42318; }} p {{ color: #555; }}
</style></head>
<body><h1>Sign-in failed</h1><p>{message}</p>
<p>Return to your terminal — Vermes will walk you through a manual paste fallback.</p></body></html>
"""
