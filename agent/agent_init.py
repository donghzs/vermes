...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.vermes/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from vermes_cli.config import load_config as _load_pc_cfg
    ...
    if agent.verbose_logging:
        ...
        # on the fact that vermes_logging.setup_logging() does not
        ...
        # handler level inside vermes_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if not agent.quiet_mode:
                logger.info(f"🤖 AI Agent initialized with model: {agent.model} (AWS Bedrock + AnthropicBedrock SDK, {_br_region})")
        else:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from vermes_cli.auth import build_minimax_oauth_token_provider
            ...
            agent._client_kwargs = {}
            if not agent.quiet_mode:
                logger.info(f"🤖 AI Agent initialized with model: {agent.model} (Anthropic native)")
                ...
                if is_token_provider(effective_key):
                    logger.info("🔑 Using credentials: Microsoft Entra ID")
                elif isinstance(effective_key, str) and len(effective_key) > 12:
                    logger.info(f"🔑 Using token: {effective_key[:8]}...{effective_key[-4:]}")
        ...
        try:
            from vermes_cli.config import load_config as _load_br_cfg
        ...
        if not agent.quiet_mode:
            ...
            logger.info(f"🤖 AI Agent initialized with model: {agent.model} (AWS Bedrock, {agent._bedrock_region}{_gr_label})")
    else:
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `vermes model` to "
                        "select a provider, or run `vermes setup` for first-time "
                        "configuration."
                    )
        ...
        try:
            ...
            if not agent.quiet_mode:
                logger.info(f"🤖 AI Agent initialized with model: {agent.model}")
                if base_url:
                ...
                if is_token_provider(key_used):
                    ...
                    logger.info(f"🔑 Using API key: {key_used[:8]}...{key_used[-4:]}")
                else:
                    logger.info("⚠️  Warning: API key appears invalid or missing")
        except Exception as e:
    ...
    if agent._fallback_chain and not agent.quiet_mode:
        if len(agent._fallback_chain) == 1:
            ...
            logger.info(f"🔄 Fallback chain ({len(agent._fallback_chain)} providers): " +
    ...
    if agent.tools:
        ...
        if not agent.quiet_mode:
            logger.info(f"🛠️  Loaded {len(agent.tools)} tools: {', '.join(tool_names)}")
    ...
    if agent.tools and not agent.quiet_mode:
        ...
        if missing_reqs:
            logger.info(f"⚠️  Some tools may not work due to missing requirements: {missing_reqs}")
    ...
    if agent._use_prompt_caching and not agent.quiet_mode:
        ...
        logger.info(f"💾 Prompt caching: ENABLED ({source}, {agent._cache_ttl} TTL)")
    ...
    try:
        from vermes_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from vermes_cli.config import load_config as _load_agent_config
        ...
        try:
            ...
            if agent._memory_enabled or agent._user_profile_enabled:
                ...
                agent._memory_store = MemoryStore(
                    memory_char_limit=mem_config.get("memory_char_limit", 5000),
    ...
    agent._decision_review_done = False
    if not skip_memory:
        try:
    ...
    # Lets an admin append to or replace Vermes' built-in platform hint
    # for a single messaging platform without affecting other platforms.
    ...
    if not isinstance(_compression_cfg, dict):
        _compression_cfg = {}
    compression_threshold = float(_compression_cfg.get("threshold", 0.15))
    try:
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from vermes_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from vermes_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Vermes Agent.  Choose a model with at least "
    ...
    if not agent.quiet_mode:
        if compression_enabled:
            logger.info(f"📊 Context limit: {agent.context_compressor.context_length:,} tokens (compress at {int(compression_threshold*100)}% = {agent.context_compressor.threshold_tokens:,})")
        else:
            logger.info(f"📊 Context limit: {agent.context_compressor.context_length:,} tokens (auto-compression disabled)")
