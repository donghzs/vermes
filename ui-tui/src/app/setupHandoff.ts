import type { RunExternalProcess } from '@vermes/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`vermes ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching vermes: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`vermes ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
