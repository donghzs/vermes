...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.VERMES_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Vermes on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via VERMES_TUI_TRUECOLOR=1.
