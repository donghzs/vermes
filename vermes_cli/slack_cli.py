"""``Vermes slack ...`` CLI subcommands.

Today only ``Vermes slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ Vermes slack manifest > slack-manifest.json
    # or:
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``vermes_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$VERMES_HOME/slack-manifest.json``)
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("VERMES_HOME") or os.environ.get("VERMES_HOME") or str(Path.home() / ".vermes")) / "slack-manifest.json"
        else:
