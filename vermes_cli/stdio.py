...
on Japanese locales, etc.).  Vermes's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``vermes_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Vermes is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``VERMES_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("VERMES_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Vermes-managed Git install directories so
    ...
    # already-running shells don't see the change — which means Vermes
    ...
    # install without Vermes-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Vermes's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Vermes's
    ``vermes config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Vermes (User env var in Windows Settings, or
    export in a PowerShell profile) and Vermes picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Vermes-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\Vermes\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``Vermes`` from right after install) retain their old PATH.

    Any subprocess Vermes spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Vermes-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "Vermes", "git", "cmd"),
        os.path.join(local_appdata, "Vermes", "git", "bin"),
        os.path.join(local_appdata, "Vermes", "git", "usr", "bin"),
        # Vermes venv Scripts directory — host of the Vermes.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes Vermes, but harmless to include.
        os.path.join(local_appdata, "Vermes", "Vermes-agent", "venv", "Scripts"),
