"""``Vermes debug`` debug tools for Vermes.

Currently supports:
    Vermes debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[Vermes debug share: log content redacted at upload time. "
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``Vermes debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``Vermes debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `Vermes debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "Vermes-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``Vermes debug share`` invocation added ~20 MB of resident Python
    ...
    ``Vermes debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"Vermes debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "Vermes-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from vermes_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_vermes_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``Vermes dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``Vermes dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "Vermes debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    logger.info(f"To delete now:  Vermes debug delete <url>")
...
def run_debug_delete(args):
    ...
    if not urls:
        logger.info("Usage: Vermes debug delete <url> [<url> ...]")
        logger.info("  Deletes paste.rs pastes uploaded by 'Vermes debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``Vermes debug`` call.
    ...
    # best-effort — any failure is swallowed so ``Vermes debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        logger.info("Usage: Vermes debug <command>")
        logger.info()
