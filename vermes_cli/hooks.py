...
def hooks_command(args) -> None:
    ...
    if not sub:
        logger.info("Usage: hermes hooks {list|test|revoke|doctor}")
        logger.info("Run 'hermes hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        logger.info("No shell hooks configured in ~/.hermes/config.yaml.")
        logger.info("See `hermes hooks --help` or")
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        logger.info(f"      parsed (Hermes wire shape): {json.dumps(parsed)}")
    else:
