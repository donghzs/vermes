...
def _all_schemas() -> List[ChannelSchema]:
    ...
    ChannelSchema(
        ...
        tutorial=(
            ...
            "5. 保存后点击「启动网关」即可在飞书中与 Agent 对话"
        ),
        note="推荐使用 WebSocket 长连接模式，无需公网回调地址",
    ),
    ...
    ChannelSchema(
        ...
        tutorial=(
            ...
            "5. 保存后点击「启动网关」即可在钉钉中与 Agent 对话"
        ),
        note="推荐 Stream 模式，无需公网服务器",
    ),
    ...
    ChannelSchema(
        ...
        fields=[
            ChannelField(key="app_id", label="App ID", placeholder="QQ 机器人 AppID"),
            ChannelField(key="client_secret", label="Client Secret", placeholder="QQ 机器人 Secret"),
        ...
        tutorial=(
            ...
            "4. 保存后点击「启动网关」即可在 QQ 中与 Agent 对话"
        ),
    ),
    ...
    ChannelSchema(
        ...
        fields=[
            ChannelField(key="app_id", label="App ID", placeholder="元宝 AppID"),
            ChannelField(key="app_secret", label="App Secret", placeholder="元宝 AppSecret"),
    ...
    ChannelSchema(
        ...
        tutorial=(
            ...
            "4. 保存后点击「启动网关」即可在 Telegram 中与 Agent 对话"
        ),
        note="最简单的接入方式，3 分钟完成",
    ),
    ...
    ChannelSchema(
        ...
        tutorial=(
            ...
            "5. 保存后点击「启动网关」即可在 Discord 中与 Agent 对话"
        ),
    ),
    ...
    ChannelSchema(
        ...
        note="需要自建 Signal CLI REST API 服务",
    ),
    ...
    ChannelSchema(
        ...
        note="需要一台始终在线的 Mac 运行 BlueBubbles Server",
    ),
    ...
    ChannelSchema(
        ...
        note="Twitch 使用 IRC 协议，凭据格式与 IRC 相同",
    ),
