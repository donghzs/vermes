...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or vermes -c.",
    ...
    "Ctrl+Z suspends Vermes to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "vermes -c resumes your most recent CLI session. vermes -c \"project name\" resumes by title.",
    "vermes -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "vermes -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "vermes chat -t web,terminal enables only specific toolsets for a focused session.",
    "vermes chat -s github-pr-workflow preloads a skill at launch.",
    "vermes chat -q \"query\" runs a single non-interactive query and exits.",
    "vermes chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "vermes chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "vermes --yolo bypasses all dangerous command approval prompts for the entire session.",
    "vermes chat --source telegram tags the session for filtering in vermes sessions list.",
    "vermes -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "vermes doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "vermes dump outputs a compact setup summary — great for bug reports.",
    "vermes config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "vermes config edit opens config.yaml in your default editor.",
    "vermes config check scans for missing or stale configuration options.",
    "vermes sessions browse opens an interactive session picker with search.",
    "vermes sessions stats shows session counts by platform and database size.",
    "vermes sessions prune --older-than 30 cleans up old sessions.",
    "vermes skills search react --source skills-sh searches the skills.sh public directory.",
    "vermes skills check scans installed hub skills for upstream updates.",
    "vermes skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "vermes skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "vermes mcp add github --command npx adds MCP servers from the command line.",
    "vermes mcp serve runs Vermes itself as an MCP server for other agents.",
    "vermes auth add lets you add multiple API keys for credential pool rotation.",
    "vermes completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "vermes logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "vermes backup creates a zip backup of your entire Vermes home directory.",
    "vermes profile create coder creates an isolated profile that becomes its own command.",
    "vermes profile create work --clone copies your current config and keys to a new profile.",
    "vermes update syncs new bundled skills to ALL profiles automatically.",
    "vermes gateway install sets up Vermes as a system service (systemd/launchd).",
    "vermes memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "vermes webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: vermes tools disables unused tools, vermes skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "vermes models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Vermes tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'vermes profile create coder' creates the 'coder' command.",
    "vermes profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Vermes prints a resume command with session ID and stats.",
    "vermes sessions export backup.jsonl exports all sessions for backup or analysis.",
    "vermes -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "The agent has two memory stores: personal notes (~5000 chars) and user profile (~3000 chars).",
    ...
    # --- Gateway & Messaging ---
    "Vermes runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "vermes gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Vermes via a local macOS server.",
    ...
    "MCP servers auto-generate toolsets at runtime — vermes tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.vermes/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "vermes chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "vermes plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "vermes status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.vermes/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Vermes your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.vermes/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Vermes auto-flushes important facts to memory in the background.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'vermes profile create ls' would be rejected.",
    "vermes profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "vermes claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Vermes auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Vermes config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Vermes is working.',
    ...
    '/reload reloads ~/.vermes/.env into the running session — pick up new API keys without restarting.',
    ...
    'Drop a ~/.vermes/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'vermes curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "vermes curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'vermes curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'vermes auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'vermes dashboard --tui embeds the full Vermes TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.vermes/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.vermes/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.vermes/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.vermes/skills/ so drop-in skills appear without restarting the session.',
    ...
    'vermes -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'vermes chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'vermes chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'vermes chat --ignore-user-config skips ~/.vermes/config.yaml — reproducible bug reports and CI runs.',
    "vermes chat --source tool tags programmatic chats so they don't clutter vermes sessions list.",
    'vermes dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'vermes sessions rename <ID> "new title" renames any past session; vermes sessions delete <ID> removes one.',
    'vermes import restores a session export or profile archive produced by sessions export or profile export.',
    'vermes fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'vermes pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'vermes setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Vermes block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from vermes sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.vermes/dashboard-plugins/.',
]
