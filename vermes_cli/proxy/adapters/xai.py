...
# xAI's public API is OpenAI-compatible for the endpoints Vermes commonly
# uses. The Responses endpoint is included because Vermes' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Vermes-managed OAuth credentials."""

    auth_hint = "vermes auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`vermes auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`vermes auth reset xai-oauth` or re-authenticate with "
                    "`vermes auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `vermes auth add xai-oauth --type oauth`."
            )
