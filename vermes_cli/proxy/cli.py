"""CLI handlers for the ``Vermes proxy`` subcommand."""
...
from vermes_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    logger.info(
        "Vermes proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"Vermes login {adapter.name}")
        logger.info(
...
def cmd_proxy_status(args: Any) -> int:
    ...
    logger.info(
        "\nStart the proxy with: Vermes proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``Vermes proxy <subcommand>``."""
    ...
    logger.info(
        "Vermes proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  Vermes proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
