...
2. The persistence shape from the June 2026 ``vermes-security-alert`` campaign: a shell
   ...
   whose payload appended an attacker SSH key to ``authorized_keys``; Vermes
...
3. A hardcoded indicator-of-compromise (IOC) blocklist for that campaign — the
   attacker's ``vermes-security-alert`` SSH public key and source IPs. Any entry whose
...
# A shell payload that touches any of these is the June 2026 vermes-security-alert shape
...
# ── Indicators of compromise: June 2026 vermes-security-alert campaign ──────────────────
...
# multiple compromised public instances (r/vermes-agent, 854.media).
_IOC_SUBSTRINGS = (
    # Attacker SSH public key (the "vermes-security-alert" persistence key).
    "AAAAC3NzaC1lZDI1NTE5AAAAICBoh1oDC4DnsO1m5mJ4yfEKrQebaFh",
    "vermes-security-alert",
...
def validate_mcp_server_entry(name: str, entry: dict[str, Any]) -> list[str]:
    ...
    * a known vermes-security-alert IOC anywhere in command/args/env (hardcoded blocklist);
    ...
    * a shell interpreter whose inline script writes to an OS persistence
      surface (June 2026 vermes-security-alert SSH/PAM/sudoers/cron shape).
    """
    ...
    for ioc in _IOC_SUBSTRINGS:
        if ioc in flat:
            issues.append(
                f"MCP server '{name}' contains a known vermes-security-alert "
    ...
    if _PERSISTENCE_PATTERN.search(script):
        issues.append(
            ...
            f"shell rc) — this is the vermes-security-alert backdoor shape, not a real "
            f"MCP server"
        )
