...
def save_style_profile(project_id: int, style_prompt: str) -> bool:
    ...
    if not project_id or project_id <= 0 or not style_prompt:
        return False
    try:
        from vermes_cli.scholarforge.database import update_project
...
def auto_snapshot(project_id: int, label: str = "", note: str = "") -> int:
    ...
    try:
        from vermes_cli.scholarforge.database import create_project_snapshot, get_project
...
def restore_snapshot(snapshot_id: int) -> dict:
    ...
    """
    try:
        from vermes_cli.scholarforge.database import restore_snapshot as _restore
...
def list_snapshots(project_id: int) -> list[dict]:
    """列出项目所有快照。"""
    try:
        from vermes_cli.scholarforge.database import list_snapshots as _list
...
def get_snapshot_detail(snapshot_id: int) -> dict:
    """获取单个快照详情。"""
    try:
        from vermes_cli.scholarforge.database import get_snapshot as _get
...
def delete_snapshot(snapshot_id: int) -> bool:
    """删除单个快照。"""
    try:
        from vermes_cli.scholarforge.database import delete_snapshot as _delete
...
def mark_project_done(project_id: int) -> None:
    ...
    try:
        from agent.project_handoff import record_project_handoff
        from vermes_cli.scholarforge.database import get_project as _get_proj
