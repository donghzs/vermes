...
def temp_db():
    ...
    with patch("vermes_cli.scholarforge.database.DB_PATH", db_path):
        yield db_path
    import shutil
...
async def test_save_cards_basic(temp_db):
    ...
    with patch(
        ...
        from vermes_cli.scholarforge.literature_cards import save_cards
...
async def test_save_cards_from_query(temp_db):
    ...
    with patch("vermes_cli.scholarforge.search.search_papers", mock_search):
        with patch(
            "vermes_cli.scholarforge.tools._call_llm",
            AsyncMock(return_value=json.dumps([{"research_question": "test query"}])),
        ):
            from vermes_cli.scholarforge.literature_cards import save_cards_from_query
...
async def test_handle_save_cards_via_tools(temp_db):
    ...
    with patch(
        ...
        from vermes_cli.scholarforge.tools import _handle_scholarforge_save_cards
