...
class ProjectContext:
    ...
    def build_global_ref_list(self) -> str:
        ...
        for paper in self.papers:
            ...
            if num == 0:
                continue
            authors = ", ".join(paper.authors[:3]) if paper.authors else "〔作者缺失〕"
            if len(paper.authors) > 3:
                authors += " 等"
            year = paper.year or "〔年份缺失〕"
            venue = paper.venue or paper.source or "〔期刊缺失〕"
            ...
            if not (paper.venue or paper.source):
                missing_fields.append("期刊")
            if missing_fields:
                missing_count += 1
                logger.warning(f"Paper {paper.paper_id} missing fields: {missing_fields}")
        ...
        if missing_count > 0:
            logger.warning(f"build_global_ref_list: {missing_count}/{len(self.papers)} papers have missing fields")
        return "\n".join(lines)
...
def _quality_gate(content: str, ctx: "ProjectContext", agent_name: str = "") -> list[dict]:
    ...
    try:
        ...
        if ai_ratio > 0.5:
            try:
                from vermes_cli.scholarforge.plagcheck import suggest_deaigc_fixes
...
class WritingAgent(BaseAgent):
    ...
    async def run(self, user_input: str, section: str = "") -> AsyncGenerator[dict, None]:
        ...
        # 跨章节一致性:注入已生成章节的摘要,让后续章节知道前文定义了什么
        if self.ctx.section_contents:
            prev_summaries = []
            for sk, sv in self.ctx.section_contents.items():
                # 取每章前 200 字作为摘要
                summary = sv[:200].replace('\n', ' ').strip()
                prev_summaries.append(f'  - {sk}: {summary}...')
            context += f"\n\n【已生成章节摘要(后续章节必须与这些章节保持一致)】\n" + "\n".join(prev_summaries)
...
9. **跨章节一致性铁律**:
   - 如果本章是实验/结果章,必须使用方法章中定义的数据集和基线方法,不得引入新数据集
   - 如果本章是讨论/结论章,结论必须基于实验章的实际结果,不得编造未报告的数据
   - 所有技术术语、缩写、参数符号必须与前文保持一致
   - 如果前文未定义某个概念,本章首次使用时必须给出定义
...
请直接输出该章节的完整内容(不要重复本章标题):
        ...
        # ═══════════════════════════════════════════════════════
        # 后处理1b: 引用编号完整性检查 - 扫描正文所有 [n],标记未注册的
        # ═══════════════════════════════════════════════════════
        cited_nums = set(re.findall(r'\[(\d+)\]', self.ctx.draft))
        registered_nums = set(str(v) for v in self.ctx.global_citation_map.values())
        unregistered = cited_nums - registered_nums
        if unregistered:
            yield {"type": "thinking", "message": f"⚠️ 发现 {len(unregistered)} 个未注册的引用编号: {sorted(int(x) for x in unregistered)[:10]}..."}
            # 在参考文献列表末尾添加警告
            warning_lines = ["", "<!-- 引用完整性警告: 以下引用编号在正文中出现但未找到对应文献 -->"]
            for num in sorted(int(x) for x in unregistered):
                warning_lines.append(f"<!-- [{num}] 未找到对应文献(可能为 LLM 编造) -->")
            self.ctx.draft += "\n".join(warning_lines)
        # 发送完整论文到前端(用于导出面板)
        # ═══════════════════════════════════════════════════════
...
class RefinementAgent(BaseAgent):
    ...
    async def run(self, user_input: str) -> AsyncGenerator[dict, None]:
        ...
        if self.ctx.papers and valid_cited:
            try:
                from vermes_cli.scholarforge.citation_verifier import verify_citations
