...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    if not any(platforms.values()):
        ...
        logger.info("Set one up with `vermes gateway setup`, or run the gateway once so")
        logger.info("channel discovery can populate ~/.vermes/channel_directory.json.")
        return _SUCCESS_EXIT
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Vermes "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.vermes/.env + ~/.vermes/config.yaml) — no LLM, "
