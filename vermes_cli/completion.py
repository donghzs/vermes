"""Shell completion script generation for Vermes CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_vermes_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Vermes bash completion
# Add to ~/.bashrc:
#   eval "$(Vermes completion bash)"

_vermes_profiles() {{
    local profiles_dir="$HOME/.vermes/profiles"
...
_vermes_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_vermes_profiles)" -- "$cur"))
        return
    fi
...
complete -F _vermes_completion Vermes
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef Vermes
# Vermes zsh completion
# Add to ~/.zshrc:
...
_vermes() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_vermes_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'Vermes command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Vermes fish completion",
        "# Add to your config:",
        "#   Vermes completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __vermes_profiles",
        "    echo default",
        "    if test -d $HOME/.vermes/profiles",
        "        ls $HOME/.vermes/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c Vermes -f -s p -l profile"
        " -d 'Profile name' -xa '(__vermes_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__vermes_profiles)' -d 'Profile name'"
                )
