"""Migrate Vermes' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Vermes' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Vermes mcp_servers.<n>.command/args/env  → codex stdio transport
  Vermes mcp_servers.<n>.url/headers       → codex streamable_http transport
  Vermes mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Vermes mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Vermes-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Vermes' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Vermes config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Vermes' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Vermes)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Vermes' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Vermes' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Vermes
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def migrate(
    ...
    *,
    ...
    # Inject Vermes' own tool surface as an MCP server so the spawned
