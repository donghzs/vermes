...
- ``vermes_cli.providers``        -- canonical provider identity + overlays
- ``vermes_cli.model_normalize``  -- per-provider name formatting
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from vermes_cli.model_switch import
...
def switch_model(
    ...
    # Mirrors the same logic in vermes_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from vermes_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from vermes_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # https://donghzs.github.io/vermes/docs/api/model-catalog.json so
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`vermes_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from vermes_cli.models import fetch_openrouter_models
