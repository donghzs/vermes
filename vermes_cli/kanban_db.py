    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # See vermes_state._WAL_INCOMPAT_MARKERS for detection logic.
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from vermes_cli.profiles import normalize_profile_name
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from vermes_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
