"""
Interactive setup wizard for Vermes.
...
from vermes_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from vermes_cli.curses_ui import curses_radiolist
...
def prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    for i, choice in enumerate(choices):
        ...
        if i == default:
            logger.info(color(f"  {marker} {choice}", Colors.GREEN))
        else:
            logger.info(f"  {marker} {choice}")
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from vermes_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, VERMES_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'vermes setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from vermes_cli.plugins import _ensure_plugins_discovered
    ...
    try:
        ...
        from vermes_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'vermes setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'vermes setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'vermes setup terminal'"))
    ...
    for name, available, missing_var in tool_status:
        if available:
            logger.info(f"   {color('✓', Colors.GREEN)} {name}")
        else:
            logger.info(
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``vermes model``)
    ...
    provider added to ``vermes model`` is automatically available here.
    ...
    from vermes_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: vermes model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from vermes_cli.auth_commands import auth_add_command
    ...
    if _vision_needs_setup:
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'vermes setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``vermes model`` -> xAI Grok OAuth (SuperGrok Subscription).
    """
    try:
        from vermes_cli.auth import get_xai_oauth_auth_status
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'vermes setup tts')."""
    _setup_tts_provider(config)
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `vermes setup agent` later to customize.")
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `vermes update`)",
                True,
            ):
...
def _setup_qqbot():
    ...
    from vermes_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'vermes setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     vermes config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``vermes_cli.auth`` — lists every supported
    ...
    """
    try:
        from vermes_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from vermes_cli.config
    ...
    if section_key == "model":
        ...
        from vermes_cli.gateway import _all_platforms, _platform_status
...
def _print_migration_preview(report: dict):
    ...
    if migrated_items:
        logger.info(color("  Would import:", Colors.GREEN))
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      vermes setup           — full or quick (auto-detected)
      vermes setup model     — just model/provider
      vermes setup tts       — just text-to-speech
      vermes setup terminal  — just terminal backend
      vermes setup gateway   — just messaging platforms
      vermes setup tools     — just tool configuration
      vermes setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                logger.info(color(f"│     ✦ Vermes Setup — {label:<34s} │", Colors.MAGENTA))
                logger.info(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    logger.info(
        color(
            "│             ⚕ Vermes Setup Wizard                │", Colors.MAGENTA
        )
    )
    logger.info(
        color(
    ...
    logger.info(
        color(
            "│  Let's configure your Vermes installation.       │", Colors.MAGENTA
        )
    )
    logger.info(
        color(
    ...
    if is_existing:
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'vermes setup model|terminal|")
...
def _run_first_time_quick_setup(config: dict, VERMES_home, is_existing: bool):
    ...
    the user can customize later via ``vermes setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'vermes setup gateway'",
        ],
        0,
    )
