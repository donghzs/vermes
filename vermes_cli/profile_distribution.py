"""Profile distributions — shareable, packaged Vermes profiles via git.

A distribution is a Vermes profile published as a git repository (or
...
* ``Vermes profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``vermes skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``Vermes profile``, not a parallel tree):

    Vermes profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "vermes-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            VERMES_requires=str(data.get("VERMES_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.VERMES_requires:
            out["VERMES_requires"] = self.VERMES_requires
        if self.author:
...
def check_vermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Vermes {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``Vermes profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Vermes profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Vermes distribution."
        )

    # Version check up-front so we fail fast
    check_vermes_requires(manifest.VERMES_requires, VERMES_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.Vermes).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="VERMES_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `Vermes profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`Vermes profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="VERMES_dist_update_") as tmp:
        plan = plan_install(
