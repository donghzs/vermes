...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `vermes model`
    flow without maintaining a separate static list.
    """
    from vermes_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Vermes that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Vermes that doesn't ship
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from vermes_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  vermes model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `vermes model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `vermes model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``vermes model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``vermes auth add openai-codex`` without
    ``vermes model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    vermes-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from vermes_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — vermes-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from vermes_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from vermes_constants import AI_GATEWAY_BASE_URL
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``vermes model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from vermes_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from vermes_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Vermes' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``vermes_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Vermes should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``vermes auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.vermes/.env``.
    ...
    """
    try:
        from vermes_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from vermes_cli.auth import read_credential_pool
        from vermes_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Vermes release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Vermes release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from vermes_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Vermes' default Claude IDs elsewhere use
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Vermes will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Vermes cannot verify the model name."
