...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.vermes/.env contains provider auth/base URL settings."""
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from vermes_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from vermes_cli.auth import get_minimax_oauth_auth_status
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from vermes_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            logger.info(color(
                ...
                f"Check ~/.vermes/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`vermes doctor --ack {hit.advisory.id}`."
                )
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'vermes setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'vermes setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'vermes setup' to create one")
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from vermes_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from vermes_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'vermes config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from vermes_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.vermes/.env or run 'vermes setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'vermes setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'vermes config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from vermes_cli.config import DEFAULT_CONFIG, save_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'vermes setup' to migrate config")
                else:
                    issues.append("Run 'vermes doctor --fix' or 'vermes setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'vermes doctor --fix'")
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Vermes' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Vermes a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Vermes Persona\n\n"
                "<！-- Edit this file to customize how Vermes communicates. -->\n\n"
                "You are Vermes, a helpful AI assistant.\n",
                encoding="utf-8",
            )
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'vermes doctor --fix' to checkpoint")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `vermes doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'vermes config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``vermes doctor``.
    ...
    # never the legitimate source for `vermes doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'vermes setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: vermes skills list)")
    ...
    if should_fix and fixed_count > 0:
        ...
        logger.info()
        if not should_fix:
            logger.info(color("  Tip: run 'vermes doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        logger.info(color("─" * 60, Colors.GREEN))
