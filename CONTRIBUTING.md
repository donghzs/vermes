# Contributing to Vermes Agent

Thank you for contributing to Vermes Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Vermes to work everywhere.
...
Bundled skills (in `skills/`) ship with every Vermes install. They should be **broadly useful to most users**:
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.vermes/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(VERMES_home, config)` for setup-wizard integration
...
- Integrate with `vermes memory setup` via `post_setup()` — no need to touch core code
...
```bash
git clone --recurse-submodules https://github.com/donghzs/vermes.git
cd vermes-agent
...
```bash
mkdir -p ~/.vermes/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.vermes/config.yaml
touch ~/.vermes/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.vermes/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/vermes" ~/.local/bin/vermes
...
├── cli.py                    # vermesCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (vermes-cli, vermes-telegram, etc.)
├── VERMES_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── vermes_cli/               # CLI command implementations
...
### User configuration (stored in `~/.vermes/`)

| Path | Purpose |
|------|---------|
| `~/.vermes/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.vermes/.env` | API keys and secrets |
| `~/.vermes/auth.json` | OAuth credentials (Nous Portal) |
| `~/.vermes/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.vermes/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.vermes/state.db` | SQLite session database |
| `~/.vermes/sessions/` | JSON session logs |
| `~/.vermes/cron/` | Scheduled job data |
| `~/.vermes/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`VERMES_state.py`) with full-text search and unique session titles. JSON logs go to `~/.vermes/sessions/`.
...
(for example `_vermes_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.vermes`:
...
The user may skip setup and keep loading the skill. Vermes only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `vermes setup` or update `~/.vermes/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Vermes Agent" is the secondary collaborator. If the contributor's commit shows "Vermes Agent" as author because they used Vermes to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Vermes tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `vermes --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Vermes uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.vermes/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `vermes_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `vermes_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Vermes runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the vermes wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `vermes_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `vermes_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `vermes_cli/gateway_windows.py::
...
Vermes has terminal access. Security matters.
...
2. **Test manually**: Run `vermes` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/donghzs/vermes/issues)
- Include: OS, Python version, Vermes version (`vermes version`), full error traceback
...
- **Discord**: [discord.gg/donghzs](https://discord.gg/donghzs) — for questions, showcasing projects, and sharing skills
